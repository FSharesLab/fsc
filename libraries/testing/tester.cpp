...
fscio::chain::asset core_from_string(const std::string& s) {
  return fscio::chain::asset::from_string(s + " " CORE_SYMBOL_NAME);
}
  ...
  void base_tester::set_transaction_headers( transaction& trx, uint32_t expiration, uint32_t delay_sec ) const {
   ...
   transaction_trace_ptr base_tester::create_account( account_name a, account_name creator, bool multisig, bool include_code ) {
      ...
      if( include_code ) {
         ...
         owner_auth.accounts.push_back( permission_level_weight{ {a, config::fscio_code_name},
                                                                 static_cast<weight_type>(owner_auth.threshold) } );
         sort_permissions(owner_auth);
         active_auth.accounts.push_back( permission_level_weight{ {a, config::fscio_code_name},
      ...
      auto sync_dbs = [](base_tester& a, base_tester& b) {
         for( int i = 1; i <= a.control->head_block_num(); ++i ) {
            ...
            if( block ) { //&& !b.control->is_known_block(block->id()) ) {
               ...
               b.control->push_block(bs); //, fscio::chain::validation_steps::created_block);
            }
         }
      };
   ...
   void base_tester::push_genesis_block() {
      ...
      set_abi(config::system_account_name, fscio_bios_abi);
      //produce_block();
   }
   ...
   bool fscio_assert_message_is::operator()( const fscio_assert_message_exception& ex ) {
      auto message = ex.get_log().at( 0 ).get_message();
      bool match = (message == expected);
      if( !match ) {
         BOOST_TEST_MESSAGE( "LOG: expected: " << expected << ", actual: " << message );
   ...
   bool fscio_assert_message_starts_with::operator()( const fscio_assert_message_exception& ex ) {
      auto message = ex.get_log().at( 0 ).get_message();
      bool match = boost::algorithm::starts_with( message, expected );
      if( !match ) {
         BOOST_TEST_MESSAGE( "LOG: expected: " << expected << ", actual: " << message );
   ...
   bool fscio_assert_code_is::operator()( const fscio_assert_code_exception& ex ) {
      auto message = ex.get_log().at( 0 ).get_message();
      bool match = (message == expected);
      if( !match ) {
         BOOST_TEST_MESSAGE( "LOG: expected: " << expected << ", actual: " << message );
