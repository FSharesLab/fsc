...
#define REQUIRE_MATCHING_OBJECT(left, right) { auto a = fc::variant( left ); auto b = fc::variant( right ); BOOST_REQUIRE_EQUAL( true, a.is_object() ); \
   BOOST_REQUIRE_EQUAL( true, b.is_object() ); \
   auto filtered = ::fscio::testing::filter_fields( a.get_object(), b.get_object() ); \
...
fscio::chain::asset core_from_string(const std::string& s);
...
namespace fscio { namespace testing {
  ...
  /**
   * Utility predicate to check whether an fscio_assert message is equivalent to a given string
   */
  struct fscio_assert_message_is {
     fscio_assert_message_is( const string& msg )
           : expected( "assertion failure with message: " ) {
        expected.append( msg );
     }

     bool operator()( const fscio_assert_message_exception& ex );

     string expected;
  };

  /**
   * Utility predicate to check whether an fscio_assert message starts with a given string
   */
  struct fscio_assert_message_starts_with {
     fscio_assert_message_starts_with( const string& msg )
           : expected( "assertion failure with message: " ) {
        expected.append( msg );
     }
  ...
  /**
   * Utility predicate to check whether an fscio_assert_code error code is equivalent to a given number
   */
  struct fscio_assert_code_is {
     fscio_assert_code_is( uint64_t error_code )
           : expected( "assertion failure with error code: " ) {
        expected.append( std::to_string(error_code) );
     }

     bool operator()( const fscio_assert_code_exception& ex );
