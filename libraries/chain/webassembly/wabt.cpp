...
namespace wasm_constraints = fscio::chain::wasm_constraints;
...
class wabt_instantiated_module : public wasm_instantiated_module_interface {
   public:
      ...
      void apply(apply_context& context) override {
         ...
         FSC_ASSERT( res.result == interp::Result::Ok, wasm_execution_error, "wabt start function failure (${s})", ("s", ResultToString(res.result)) );

         res = _executor.RunExportByName(_instatiated_module, "apply", _params);
         FSC_ASSERT( res.result == interp::Result::Ok, wasm_execution_error, "wabt execution failure (${s})", ("s", ResultToString(res.result)) );
      }
...
std::unique_ptr<wasm_instantiated_module_interface> wabt_runtime::instantiate_module(const char* code_bytes, size_t code_size, std::vector<uint8_t> initial_memory) {
   ...
   FSC_ASSERT( Succeeded(res), wasm_execution_error, "Error building wabt interp: ${e}", ("e", wabt::FormatErrorsToString(errors, Location::Type::Binary)) );
