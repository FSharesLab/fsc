...
namespace fscio { namespace chain {
   ...
   void block_log::open(const fc::path& data_dir) {
      ...
      if (log_size) {
         ...
         FSC_ASSERT( my->version > 0, block_log_exception, "Block log was not setup properly" );
         FSC_ASSERT( my->version >= min_supported_version && my->version <= max_supported_version, block_log_unsupported_version,
         ...
         if (my->version > 1){
            ...
            FSC_ASSERT(my->first_block_num > 0, block_log_exception, "Block log is malformed, first recorded block number is 0 but must be greater than or equal to 1");
         } else {
   ...
   uint64_t block_log::append(const signed_block_ptr& b) {
      try {
         FSC_ASSERT( my->genesis_written_to_block_log, block_log_append_fail, "Cannot append to block log until the genesis is first written" );

         my->check_block_write();
         my->check_index_write();

         uint64_t pos = my->block_stream.tellp();
         FSC_ASSERT(my->index_stream.tellp() == sizeof(uint64_t) * (b->block_num() - my->first_block_num),
   ...
   signed_block_ptr block_log::read_block_by_num(uint32_t block_num)const {
      try {
         ...
         if (pos != npos) {
            ...
            FSC_ASSERT(b->block_num() == block_num, reversible_blocks_exception,
   ...
   fc::path block_log::repair_log( const fc::path& data_dir, uint32_t truncate_at_block ) {
      ...
      FSC_ASSERT( fc::is_directory(data_dir) && fc::is_regular_file(data_dir / "blocks.log"), block_log_not_found,
      ...
      FSC_ASSERT( blocks_dir_name.generic_string() != ".", block_log_exception, "Invalid path to blocks directory" );
      backup_dir = backup_dir / blocks_dir_name.generic_string().append("-").append( now );

      FSC_ASSERT( !fc::exists(backup_dir), block_log_backup_dir_exist,
      ...
      FSC_ASSERT( version > 0, block_log_exception, "Block log was not setup properly" );
      FSC_ASSERT( version >= min_supported_version && version <= max_supported_version, block_log_unsupported_version,
                 "Unsupported version of block log. Block log version is ${version} while code supports version(s) [${min},${max}]",
      ...
      if (version != 1) {
         ...
         FSC_ASSERT(actual_totem == expected_totem, block_log_exception,
