...
#include <fscio/chain/authority.hpp>
#include <fscio/chain/chain_config.hpp>
#include <fscio/chain/transaction.hpp>
#include <fscio/chain/asset.hpp>
...
namespace fscio { namespace chain {
   ...
   void abi_serializer::set_abi(const abi_def& abi, const fc::microseconds& max_serialization_time) {
      ...
      FSC_ASSERT(starts_with(abi.version, "fscio::abi/1."), unsupported_abi_version_exception, "ABI has an unsupported version");
      ...
      for( const auto& td : abi.types ) {
         FSC_ASSERT(_is_type(td.type, ctx), invalid_type_inside_abi, "invalid type ${type}", ("type",td.type));
         FSC_ASSERT(!_is_type(td.new_type_name, ctx), duplicate_abi_type_def_exception, "type already exists", ("new_type_name",td.new_type_name));
       ...
       *  an invalid ABI
       */
      FSC_ASSERT( typedefs.size() == abi.types.size(), duplicate_abi_type_def_exception, "duplicate type definition detected" );
      FSC_ASSERT( structs.size() == abi.structs.size(), duplicate_abi_struct_def_exception, "duplicate struct definition detected" );
      FSC_ASSERT( actions.size() == abi.actions.size(), duplicate_abi_action_def_exception, "duplicate action definition detected" );
      FSC_ASSERT( tables.size() == abi.tables.size(), duplicate_abi_table_def_exception, "duplicate table definition detected" );
      FSC_ASSERT( error_messages.size() == abi.error_messages.size(), duplicate_abi_err_msg_def_exception, "duplicate error message definition detected" );
      FSC_ASSERT( variants.size() == abi.variants.value.size(), duplicate_abi_variant_def_exception, "duplicate variant definition detected" );
   ...
   int abi_serializer::get_integer_size(const type_name& type) const {
      string stype = type;
      FSC_ASSERT( is_integer(type), invalid_type_inside_abi, "${stype} is not an integer type", ("stype",stype));
   ...
   const struct_def& abi_serializer::get_struct(const type_name& type)const {
      ...
      FSC_ASSERT( itr != structs.end(), invalid_type_inside_abi, "Unknown struct ${type}", ("type",type) );
      return itr->second;
   }
   ...
   void abi_serializer::validate( impl::abi_traverse_context& ctx )const {
      for( const auto& t : typedefs ) { try {
         ...
         while( itr != typedefs.end() ) {
            ctx.check_deadline();
            FSC_ASSERT( find(types_seen.begin(), types_seen.end(), itr->second) == types_seen.end(), abi_circular_def_exception, "Circular reference in type ${type}", ("type",t.first) );
            types_seen.emplace_back(itr->second);
            itr = typedefs.find(itr->second);
         }
      } FC_CAPTURE_AND_RETHROW( (t) ) }
      for( const auto& t : typedefs ) { try {
         FSC_ASSERT(_is_type(t.second, ctx), invalid_type_inside_abi, "${type}", ("type",t.second) );
      } FC_CAPTURE_AND_RETHROW( (t) ) }
      for( const auto& s : structs ) { try {
         if( s.second.base != type_name() ) {
            ...
            while( current.base != type_name() ) {
               ...
               FSC_ASSERT( find(types_seen.begin(), types_seen.end(), base.name) == types_seen.end(), abi_circular_def_exception, "Circular reference in struct ${type}", ("type",s.second.name) );
         ...
         for( const auto& field : s.second.fields ) { try {
            ctx.check_deadline();
            FSC_ASSERT(_is_type(_remove_bin_extension(field.type), ctx), invalid_type_inside_abi, "${type}", ("type",field.type) );
         } FC_CAPTURE_AND_RETHROW( (field) ) }
      } FC_CAPTURE_AND_RETHROW( (s) ) }
      for( const auto& s : variants ) { try {
         for( const auto& type : s.second.types ) { try {
            ctx.check_deadline();
            FSC_ASSERT(_is_type(type, ctx), invalid_type_inside_abi, "${type}", ("type",type) );
         } FC_CAPTURE_AND_RETHROW( (type) ) }
      } FC_CAPTURE_AND_RETHROW( (s) ) }
      for( const auto& a : actions ) { try {
        ctx.check_deadline();
        FSC_ASSERT(_is_type(a.second, ctx), invalid_type_inside_abi, "${type}", ("type",a.second) );
                                            ...
                                            fc::mutable_variant_object& obj, impl::binary_to_variant_context& ctx )const
   {
      ...
      FSC_ASSERT( s_itr != structs.end(), invalid_type_inside_abi, "Unknown type ${type}", ("type",ctx.maybe_shorten(type)) );
      ...
      for( uint32_t i = 0; i < st.fields.size(); ++i ) {
         ...
         if( !stream.remaining() ) {
            ...
            if( encountered_extension ) {
               FSC_THROW( abi_exception, "Encountered field '${f}' without binary extension designation while processing struct '${p}'",
                          ("f", ctx.maybe_shorten(field.name))("p", ctx.get_path_string()) );
            }
            FSC_THROW( unpack_exception, "Stream unexpectedly ended; unable to unpack field '${f}' of struct '${p}'",
                       ("f", ctx.maybe_shorten(field.name))("p", ctx.get_path_string()) );
                                                   ...
                                                   impl::binary_to_variant_context& ctx )const
   {
      ...
      if( btype != built_in_types.end() ) {
         ...
         } FSC_RETHROW_EXCEPTIONS( unpack_exception, "Unable to unpack ${class} type '${type}' while processing '${p}'",
      ...
      if ( is_array(rtype) ) {
         ...
         } FSC_RETHROW_EXCEPTIONS( unpack_exception, "Unable to unpack size of array '${p}'", ("p", ctx.get_path_string()) )
         ...
         for( decltype(size.value) i = 0; i < size; ++i ) {
            ...
            FSC_ASSERT( !v.is_null(), unpack_exception, "Invalid packed array '${p}'", ("p", ctx.get_path_string()) );
            vars.emplace_back(std::move(v));
         }
         // QUESTION: Why would the assert below ever fail?
         FSC_ASSERT( vars.size() == size.value,
                     unpack_exception,
         ...
         } FSC_RETHROW_EXCEPTIONS( unpack_exception, "Unable to unpack presence flag of optional '${p}'", ("p", ctx.get_path_string()) )
         ...
         if( v_itr != variants.end() ) {
            ...
            } FSC_RETHROW_EXCEPTIONS( unpack_exception, "Unable to unpack tag of variant '${p}'", ("p", ctx.get_path_string()) )
            FSC_ASSERT( (size_t)select < v_itr->second.types.size(), unpack_exception,
      ...
      FSC_ASSERT( mvo.size() > 0, unpack_exception, "Unable to unpack '${p}' from stream", ("p", ctx.get_path_string()) );
   ...
   { try {
      ...
      if( btype != built_in_types.end() ) {
         ...
         ctx.hint_variant_type_if_in_array( v_itr );
         auto& v = v_itr->second;
         FSC_ASSERT( var.is_array() && var.size() == 2, pack_exception,
                    "Expected input to be an array of two items while processing variant '${p}'", ("p", ctx.get_path_string()) );
         FSC_ASSERT( var[size_t(0)].is_string(), pack_exception,
         ...
         FSC_ASSERT( it != v.types.end(), pack_exception,
         ...
         if( var.is_object() ) {
            ...
            for( uint32_t i = 0; i < st.fields.size(); ++i ) {
               ...
               if( vo.contains( string(field.name).c_str() ) ) {
                  if( disallow_additional_fields )
                     FSC_THROW( pack_exception, "Unexpected field '${f}' found in input object while processing struct '${p}'",
                                ("f", ctx.maybe_shorten(field.name))("p", ctx.get_path_string()) );
                  {
                     auto h1 = ctx.push_to_path( impl::field_path_item{ .parent_struct_itr = s_itr, .field_ordinal = i } );
                  ...
                  FSC_THROW( pack_exception, "Missing field '${f}' in input object while processing struct '${p}'",
                             ("f", ctx.maybe_shorten(field.name))("p", ctx.get_path_string()) );
               }
            }
         } else if( var.is_array() ) {
            const auto& va = var.get_array();
            FSC_ASSERT( st.base == type_name(), invalid_type_inside_abi,
            ...
            for( uint32_t i = 0; i < st.fields.size(); ++i ) {
               ...
               if( va.size() > i ) {
                  ...
                  FSC_THROW( pack_exception, "Early end to input array specifying the fields of struct '${p}'; require input for field '${f}'",
                             ("p", ctx.get_path_string())("f", ctx.maybe_shorten(field.name)) );
               }
            }
         } else {
            FSC_THROW( pack_exception, "Unexpected input encountered while processing struct '${p}'", ("p",ctx.get_path_string()) );
         }
      } else {
         FSC_THROW( invalid_type_inside_abi, "Unknown type ${type}", ("type",ctx.maybe_shorten(type)) );
      }
   ...
   namespace impl {
      ...
      void abi_traverse_context::check_deadline()const {
         FSC_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception,
      ...
      fc::scoped_exit<std::function<void()>> abi_traverse_context::enter_scope() {
         ...
         ++recursion_depth;
         FSC_ASSERT( recursion_depth < abi_serializer::max_recursion_depth, abi_recursion_depth_exception,
      ...
      fc::scoped_exit<std::function<void()>> abi_traverse_context_with_path::push_to_path( const path_item& item ) {
         std::function<void()> callback = [this](){
            FSC_ASSERT( path.size() > 0, abi_exception,
      ...
      void abi_traverse_context_with_path::set_array_index_of_path_back( uint32_t i ) {
         FSC_ASSERT( path.size() > 0, abi_exception, "path is empty" );

         auto& b = path.back();

         FSC_ASSERT( b.contains<array_index_path_item>(), abi_exception, "trying to set array index without first pushing new array index item" );
