...
void transaction_header::validate()const {
   FSC_ASSERT( max_net_usage_words.value < UINT32_MAX / 8UL, transaction_exception,
...
{ try {
   ...
   for(const signature_type& sig : signatures) {
      ...
      FSC_ASSERT( now < deadline, tx_cpu_usage_exceeded, "transaction signature verification executed for too long",
      ...
      FSC_ASSERT( allow_duplicate_keys || successful_insertion, tx_duplicate_sig,
...
uint32_t packed_transaction::get_unprunable_size()const {
   ...
   FSC_ASSERT( size <= std::numeric_limits<uint32_t>::max(), tx_too_big, "packed_transaction is too big" );
...
struct read_limiter {
   ...
   {
      FSC_ASSERT(_total + count <= Limit, tx_decompression_error, "Exceeded maximum decompressed transaction size");
      _total += count;
...
bytes packed_transaction::get_raw_transaction() const
{
   try {
      switch(compression) {
         ...
         default:
            FSC_THROW(unknown_transaction_compression, "Unknown transaction compression algorithm");
      }
...
void packed_transaction::reflector_init()
{
   ...
   FSC_ASSERT( unpacked_trx.expiration == time_point_sec(), tx_decompression_error, "packed_transaction already unpacked" );
   local_unpack_transaction({});
   local_unpack_context_free_data();
}
...
{
   try {
      FSC_ASSERT(unpacked_trx.context_free_data.empty(), tx_decompression_error, "packed_transaction.context_free_data not empty");
      switch( compression ) {
         case none:
