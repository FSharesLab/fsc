...
#include <fscio/chain/authority_checker.hpp>
...
#include <fscio/chain/database_utils.hpp>
...
namespace fscio { namespace chain {
   ...
   namespace detail {
      ...
      struct snapshot_row_traits<permission_object> {
         ...
         static void from_snapshot_row(snapshot_permission_object&& row, permission_object& value, chainbase::database& db) {
            ...
            value.parent = 0;
            if (value.id == 0) {
               FSC_ASSERT(row.parent == permission_name(), snapshot_exception, "Unexpected parent name on reserved permission 0");
               FSC_ASSERT(row.name == permission_name(), snapshot_exception, "Unexpected permission name on reserved permission 0");
               FSC_ASSERT(row.owner == name(), snapshot_exception, "Unexpected owner name on reserved permission 0");
               FSC_ASSERT(row.auth.accounts.size() == 0,  snapshot_exception, "Unexpected auth accounts on reserved permission 0");
               FSC_ASSERT(row.auth.keys.size() == 0,  snapshot_exception, "Unexpected auth keys on reserved permission 0");
               FSC_ASSERT(row.auth.waits.size() == 0,  snapshot_exception, "Unexpected auth waits on reserved permission 0");
               FSC_ASSERT(row.auth.threshold == 0,  snapshot_exception, "Unexpected auth threshold on reserved permission 0");
               FSC_ASSERT(row.last_updated == time_point(),  snapshot_exception, "Unexpected auth last updated on reserved permission 0");
               value.parent = 0;
               ...
               FSC_ASSERT(parent.id != 0, snapshot_exception, "Unexpected mapping to reserved permission 0");
   ...
   { try {
      FSC_ASSERT( !level.actor.empty() && !level.permission.empty(), invalid_permission, "Invalid permission" );
      return _db.find<permission_object, by_owner>( boost::make_tuple(level.actor,level.permission) );
   } FSC_RETHROW_EXCEPTIONS( chain::permission_query_exception, "Failed to retrieve permission: ${level}", ("level", level) ) }
                                                                             ...
                                                                             )const
   {
      ...
      if( scope == config::system_account_name ) {
          FSC_ASSERT( act_name != updateauth::get_name() &&
      ...
      try {
         ...
         if( *linked_permission == config::fscio_any_name )
            return optional<permission_name>();
   ...
   {
      FSC_ASSERT( auths.size() == 1, irrelevant_auth_exception,
                  "updateauth action should only have one declared authorization" );
      const auto& auth = auths[0];
      FSC_ASSERT( auth.actor == update.account, irrelevant_auth_exception,
      ...
      FSC_ASSERT( get_permission(auth).satisfies( *min_permission,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      FSC_ASSERT( auth.actor == del.account, irrelevant_auth_exception,
                                                           ...
                                                           )const
   {
      ...
      const auto& auth = auths[0];
      FSC_ASSERT( auth.actor == link.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      FSC_ASSERT( link.type != updateauth::get_name(),  action_validate_exception,
                  "Cannot link fscio::updateauth to a minimum permission" );
      FSC_ASSERT( link.type != deleteauth::get_name(),  action_validate_exception,
                  "Cannot link fscio::deleteauth to a minimum permission" );
      FSC_ASSERT( link.type != linkauth::get_name(),    action_validate_exception,
                  "Cannot link fscio::linkauth to a minimum permission" );
      FSC_ASSERT( link.type != unlinkauth::get_name(),  action_validate_exception,
                  "Cannot link fscio::unlinkauth to a minimum permission" );
      FSC_ASSERT( link.type != canceldelay::get_name(), action_validate_exception,
                  "Cannot link fscio::canceldelay to a minimum permission" );

      const auto linked_permission_name = lookup_minimum_permission(link.account, link.code, link.type);

      if( !linked_permission_name ) // if action is linked to fscio.any permission
         return;

      FSC_ASSERT( get_permission(auth).satisfies( get_permission({link.account, *linked_permission_name}),
                                                  _db.get_index<permission_index>().indices()              ),
                  irrelevant_auth_exception,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      FSC_ASSERT( auth.actor == unlink.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      const auto unlinked_permission_name = lookup_linked_permission(unlink.account, unlink.code, unlink.type);
      FSC_ASSERT( unlinked_permission_name.valid(), transaction_exception,
      ...
      if( *unlinked_permission_name == config::fscio_any_name )
         return;

      FSC_ASSERT( get_permission(auth).satisfies( get_permission({unlink.account, *unlinked_permission_name}),
                                                  _db.get_index<permission_index>().indices()                  ),
                  irrelevant_auth_exception,
                                                                          ...
                                                                          )const
   {
      ...
      FSC_ASSERT( get_permission(auth).satisfies( get_permission(cancel.canceling_auth),
                                                  _db.get_index<permission_index>().indices() ),
                  irrelevant_auth_exception,
      ...
      FSC_ASSERT( itr != generated_index.end() && itr->sender == account_name() && itr->trx_id == trx_id,
                  tx_not_found,
      ...
      FSC_ASSERT( found, action_validate_exception,
                                             ...
                                             )const
   {
      ...
      for( const auto& act : actions ) {
         ...
         for( const auto& declared_auth : act.authorization ) {
            ...
            if( !special_case ) {
               ...
               if( min_permission_name ) { // since special cases were already handled, it should only be false if the permission is fscio.any
                  const auto& min_permission = get_permission({declared_auth.actor, *min_permission_name});
                  FSC_ASSERT( get_permission(declared_auth).satisfies( min_permission,
                                                                       _db.get_index<permission_index>().indices() ),
                              irrelevant_auth_exception,
      ...
      for( const auto& p : permissions_to_satisfy ) {
         ...
         FSC_ASSERT( checker.satisfied( p.first, p.second ), unsatisfied_authorization,
      ...
      if( !allow_unused_keys ) {
         FSC_ASSERT( checker.all_keys_used(), tx_irrelevant_sig,
                                             ...
                                             )const
   {
      ...
      FSC_ASSERT( checker.satisfied({account, permission}), unsatisfied_authorization,
                                                                     ...
                                                                     )const
   {
      ...
      for (const auto& act : trx.actions ) {
         for (const auto& declared_auth : act.authorization) {
            FSC_ASSERT( checker.satisfied(declared_auth), unsatisfied_authorization,
