#include <fscio/chain/block_header_state.hpp>
...
namespace fscio { namespace chain {
  ...
  block_header_state block_header_state::generate_next( block_timestamp_type when )const {
    ...
    if( when != block_timestamp_type() ) {
       FSC_ASSERT( when > header.timestamp, block_validate_exception, "next block must be in the future" );
    } else {
  ...
  void block_header_state::set_new_producers( producer_schedule_type pending ) {
      FSC_ASSERT( pending.version == active_schedule.version + 1, producer_schedule_exception, "wrong producer schedule version specified" );
      FSC_ASSERT( pending_schedule.producers.size() == 0, producer_schedule_exception,
  ...
  block_header_state block_header_state::next( const signed_block_header& h, bool skip_validate_signee )const {
    FSC_ASSERT( h.timestamp != block_timestamp_type(), block_validate_exception, "", ("h",h) );
    FSC_ASSERT( h.header_extensions.size() == 0, block_validate_exception, "no supported extensions" );

    FSC_ASSERT( h.timestamp > header.timestamp, block_validate_exception, "block must be later in time" );
    FSC_ASSERT( h.previous == id, unlinkable_block_exception, "block must link to current state" );
    auto result = generate_next( h.timestamp );
    FSC_ASSERT( result.header.producer == h.producer, wrong_producer, "wrong producer specified" );
    FSC_ASSERT( result.header.schedule_version == h.schedule_version, producer_schedule_exception, "schedule_version in signed block is corrupted" );

    auto itr = producer_to_last_produced.find(h.producer);
    if( itr != producer_to_last_produced.end() ) {
       FSC_ASSERT( itr->second < result.block_num - h.confirmed, producer_double_confirm, "producer ${prod} double-confirming known range", ("prod", h.producer) );
    }
    ...
    if( h.new_producers ) {
      FSC_ASSERT( !was_pending_promoted, producer_schedule_exception, "cannot set pending producer schedule in the same block in which pending was promoted to active" );
  ...
  void block_header_state::sign( const std::function<signature_type(const digest_type&)>& signer ) {
     ...
     FSC_ASSERT( block_signing_key == fc::crypto::public_key( header.producer_signature, d ), wrong_signing_key, "block is signed with unexpected key" );
  }
  ...
  void block_header_state::verify_signee( const public_key_type& signee )const {
     FSC_ASSERT( block_signing_key == signee, wrong_signing_key, "block not signed by expected key",
  ...
  void block_header_state::add_confirmation( const header_confirmation& conf ) {
     for( const auto& c : confirmations )
        FSC_ASSERT( c.producer != conf.producer, producer_double_confirm, "block already confirmed by this producer" );

     auto key = active_schedule.get_producer_key( conf.producer );
     FSC_ASSERT( key != public_key_type(), producer_not_in_schedule, "producer not in current schedule" );
     auto signer = fc::crypto::public_key( conf.producer_signature, sig_digest(), true );
     FSC_ASSERT( signer == key, wrong_signing_key, "confirmation not signed by expected key" );
