...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::fscio_code_name ) // virtual fscio.code permission does not really exist but is allowed
         continue;

      try {
         context.control.get_authorization_manager().get_permission({a.permission.actor, a.permission.permission});
      } catch( const permission_query_exception& ) {
         FSC_THROW( action_validate_exception,
 ...
 */
void apply_fscio_newaccount(apply_context& context) {
   auto create = context.act.data_as<newaccount>();
   try {
   context.require_authorization(create.creator);
//   context.require_write_lock( config::fscio_auth_scope );
   auto& authorization = context.control.get_mutable_authorization_manager();

   FSC_ASSERT( validate(create.owner), action_validate_exception, "Invalid owner authority");
   FSC_ASSERT( validate(create.active), action_validate_exception, "Invalid active authority");

   auto& db = context.db;

   auto name_str = name(create.name).to_string();

   FSC_ASSERT( !create.name.empty(), action_validate_exception, "account name cannot be empty" );
   FSC_ASSERT( name_str.size() <= 12, action_validate_exception, "account names can only be 12 chars long" );

   // Check if the creator is privileged
   const auto &creator = db.get<account_object, by_name>(create.creator);
   if( !creator.privileged ) {
      FSC_ASSERT( name_str.find( "fscio." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'fscio.'" );
   }

   auto existing_account = db.find<account_object, by_name>(create.name);
   FSC_ASSERT(existing_account == nullptr, account_name_exists_exception,
...
void apply_fscio_setcode(apply_context& context) {
   ...
   FSC_ASSERT( act.vmtype == 0, invalid_contract_vm_type, "code should be 0" );
   FSC_ASSERT( act.vmversion == 0, invalid_contract_vm_version, "version should be 0" );
   ...
   FSC_ASSERT( account.code_version != code_id, set_exact_code, "contract is already running this version of code" );
...
void apply_fscio_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_fscio_updateauth(apply_context& context) {
   ...
   FSC_ASSERT(!update.permission.empty(), action_validate_exception, "Cannot create authority with empty name");
   FSC_ASSERT( update.permission.to_string().find( "fscio." ) != 0, action_validate_exception,
               "Permission names that start with 'fscio.' are reserved" );
   FSC_ASSERT(update.permission != update.parent, action_validate_exception, "Cannot set an authority as its own parent");
   db.get<account_object, by_name>(update.account);
   FSC_ASSERT(validate(update.auth), action_validate_exception,
              "Invalid authority: ${auth}", ("auth", update.auth));
   if( update.permission == config::active_name )
      FSC_ASSERT(update.parent == config::owner_name, action_validate_exception, "Cannot change active authority's parent from owner", ("update.parent", update.parent) );
   if (update.permission == config::owner_name)
      FSC_ASSERT(update.parent.empty(), action_validate_exception, "Cannot change owner authority's parent");
   else
      FSC_ASSERT(!update.parent.empty(), action_validate_exception, "Only owner permission can have empty parent" );

   if( update.auth.waits.size() > 0 ) {
      auto max_delay = context.control.get_global_properties().configuration.max_transaction_delay;
      FSC_ASSERT( update.auth.waits.back().wait_sec <= max_delay, action_validate_exception,
   ...
   if( permission ) {
      FSC_ASSERT(parent_id == permission->parent, action_validate_exception,
...
void apply_fscio_deleteauth(apply_context& context) {
//   context.require_write_lock( config::fscio_auth_scope );
   ...
   FSC_ASSERT(remove.permission != config::active_name, action_validate_exception, "Cannot delete active authority");
   FSC_ASSERT(remove.permission != config::owner_name, action_validate_exception, "Cannot delete owner authority");
   ...
   { // Check for links to this permission
      ...
      FSC_ASSERT(range.first == range.second, action_validate_exception,
...
void apply_fscio_linkauth(apply_context& context) {
//   context.require_write_lock( config::fscio_auth_scope );

   auto requirement = context.act.data_as<linkauth>();
   try {
      FSC_ASSERT(!requirement.requirement.empty(), action_validate_exception, "Required permission cannot be empty");
      ...
      FSC_ASSERT(account != nullptr, account_query_exception,
      ...
      FSC_ASSERT(code != nullptr, account_query_exception,
                 "Failed to retrieve code for account: ${account}", ("account", requirement.code));
      if( requirement.requirement != config::fscio_any_name ) {
         const auto *permission = db.find<permission_object, by_name>(requirement.requirement);
         FSC_ASSERT(permission != nullptr, permission_query_exception,
      ...
      if( link ) {
         FSC_ASSERT(link->required_permission != requirement.requirement, action_validate_exception,
...
void apply_fscio_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::fscio_auth_scope );
   ...
   auto link = db.find<permission_link_object, by_action_name>(link_key);
   FSC_ASSERT(link != nullptr, action_validate_exception, "Attempting to unlink authority, but no link found");
   context.add_ram_usage(
      link->account,
      -(int64_t)(config::billable_size_v<permission_link_object>)
   );
...
void apply_fscio_canceldelay(apply_context& context) {
