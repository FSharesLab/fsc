...
#include <fscio/chain/chain_snapshot.hpp>
#include <fscio/chain/thread_utils.hpp>
...
struct controller_impl {
   ...
   void pop_block() {
      ...
      FSC_ASSERT( prev, block_validate_exception, "attempt to pop beyond last irreversible block" );
      ...
      if ( read_mode == db_read_mode::SPECULATIVE ) {
         FSC_ASSERT( head->block, block_validate_exception, "attempting to pop a block that was sparsely loaded from a snapshot");
...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   SET_APP_HANDLER( fscio, fscio, newaccount );
   SET_APP_HANDLER( fscio, fscio, setcode );
   SET_APP_HANDLER( fscio, fscio, setabi );
   SET_APP_HANDLER( fscio, fscio, updateauth );
   SET_APP_HANDLER( fscio, fscio, deleteauth );
   SET_APP_HANDLER( fscio, fscio, linkauth );
   SET_APP_HANDLER( fscio, fscio, unlinkauth );
/*
   SET_APP_HANDLER( fscio, fscio, postrecovery );
   SET_APP_HANDLER( fscio, fscio, passrecovery );
   SET_APP_HANDLER( fscio, fscio, vetorecovery );
*/

   SET_APP_HANDLER( fscio, fscio, canceldelay );
   ...
   void on_irreversible( const block_state_ptr& s ) {
      ...
      if (!log_head) {
         if (s->block) {
            FSC_ASSERT(s->block_num == blog.first_block_num(), block_log_exception, "block log has no blocks and is appending the wrong first block.  Expected ${expected}, but received: ${actual}",
                      ("expected", blog.first_block_num())("actual", s->block_num));
            append_to_blog = true;
         } else {
            FSC_ASSERT(s->block_num == blog.first_block_num() - 1, block_log_exception, "block log has no blocks and is not properly set up to start after the snapshot");
         }
      } else {
         auto lh_block_num = log_head->block_num();
         if (s->block_num > lh_block_num) {
            FSC_ASSERT(s->block_num - 1 == lh_block_num, unlinkable_block_exception, "unlinkable block", ("s->block_num", s->block_num)("lh_block_num", lh_block_num));
            FSC_ASSERT(s->block->previous == log_head->id(), unlinkable_block_exception, "irreversible doesn't link to block log head");
            append_to_blog = true;
         }
      }
      ...
      if (s->block) {
         if (read_mode == db_read_mode::IRREVERSIBLE) {
            ...
            if (!head || s->block_num == head->block_num + 1) {
               ...
               FSC_ASSERT(!head || head->block_num == 1, block_validate_exception, "Attempting to re-apply an irreversible block that was not the implied genesis block");
            }
   ...
   void init(std::function<bool()> shutdown, const snapshot_reader_ptr& snapshot) {
      ...
      if (snapshot) {
         FSC_ASSERT( !head, fork_database_exception, "" );
         snapshot->validate();
         ...
         if( !end ) {
            ...
            FSC_ASSERT( end->block_num() == head->block_num, fork_database_exception,
      ...
      if( objitr != ubi.rend() ) {
         FSC_ASSERT( objitr->blocknum == head->block_num, fork_database_exception,
         ...
         auto end = blog.read_head();
         FSC_ASSERT( !end || end->block_num() == head->block_num, fork_database_exception,
                    "fork database exists but reversible block database does not, replay blockchain",
                    ("blog_head",end->block_num())("head",head->block_num)  );
      }

      FSC_ASSERT( db.revision() >= head->block_num, fork_database_exception, "fork database is inconsistent with shared memory",
   ...
   void create_native_account( account_name name, const authority& owner, const authority& active, bool is_privileged = false ) {
      db.create<account_object>([&](auto& a) {
         ...
         if( name == config::system_account_name ) {
            a.set_abi(fscio_contract_abi(abi_def()));
         }
      });
   ...
   void commit_block( bool add_to_fork_db ) {
      ...
      try {
         if (add_to_fork_db) {
            ...
            head = fork_db.head();
            FSC_ASSERT(new_bsp == head, fork_database_exception, "committed block did not become the new head in fork database");
         }
   ...
   transaction_trace_ptr push_scheduled_transaction( const transaction_id_type& trxid, fc::time_point deadline, uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time = false ) {
      ...
      FSC_ASSERT( itr != idx.end(), unknown_transaction_exception, "unknown transaction" );
   ...
   { try {
      ...
      FSC_ASSERT( gtrx.delay_until <= self.pending_block_time(), transaction_exception, "this transaction isn't ready",
   ...
   const transaction_receipt& push_receipt( const T& trx, transaction_receipt_header::status_enum status,
      ...
      FSC_ASSERT( net_usage_words*8 == net_usage, transaction_exception, "net_usage is not divisible by 8" );
   ...
   transaction_trace_ptr push_transaction( const transaction_metadata_ptr& trx,
                                           ...
                                           bool explicit_billed_cpu_time = false )
   {
      FSC_ASSERT(deadline != fc::time_point(), transaction_exception, "deadline cannot be uninitialized");

      transaction_trace_ptr trace;
      try {
         auto start = fc::time_point::now();
         if( !explicit_billed_cpu_time ) {
            fc::microseconds already_consumed_time( FSC_PERCENT(trx->sig_cpu_usage.count(), conf.sig_cpu_bill_pct) );
   ...
   {
      FSC_ASSERT( !pending, block_validate_exception, "pending block already exists" );
      ...
      if (!self.skip_db_sessions(s)) {
         FSC_ASSERT( db.revision() == head->block_num, database_exception, "db revision is not on par with head block",
   ...
   void apply_block( const signed_block_ptr& b, controller::block_status s ) { try {
      try {
         FSC_ASSERT( b->block_extensions.size() == 0, block_validate_exception, "no supported extensions" );
         ...
         for( const auto& receipt : b->transactions ) {
            ...
            if( receipt.trx.contains<packed_transaction>() ) {
               ...
               FSC_ASSERT( false, block_validate_exception, "encountered unexpected receipt type" );
            }
            ...
            FSC_ASSERT( pending->_pending_block_state->block->transactions.size() > 0,
                        block_validate_exception, "expected a receipt",
                        ("block", *b)("expected_receipt", receipt)
                      );
            FSC_ASSERT( pending->_pending_block_state->block->transactions.size() == num_pending_receipts + 1,
            ...
            FSC_ASSERT( r == static_cast<const transaction_receipt_header&>(receipt),
         ...
         FSC_ASSERT(producer_block_id == pending->_pending_block_state->header.id(),
   ...
   std::future<block_state_ptr> create_block_state_future( const signed_block_ptr& b ) {
      FSC_ASSERT( b, block_validate_exception, "null block" );

      auto id = b->id();

      // no reason for a block_state if fork_db already knows about block
      auto existing = fork_db.get_block( id );
      FSC_ASSERT( !existing, fork_database_exception, "we already know about this block: ${id}", ("id", id) );

      auto prev = fork_db.get_block( b->previous );
      FSC_ASSERT( prev, unlinkable_block_exception, "unlinkable block ${id}", ("id", id)("previous", b->previous) );
   ...
   void push_block( std::future<block_state_ptr>& block_state_future ) {
      ...
      FSC_ASSERT(!pending, block_validate_exception, "it is not valid to push a block when there is a pending block");
   ...
   void replay_push_block( const signed_block_ptr& b, controller::block_status s ) {
      ...
      try {
         FSC_ASSERT( b, block_validate_exception, "trying to push empty block" );
         FSC_ASSERT( (s == controller::block_status::irreversible || s == controller::block_status::validated),
   ...
   void maybe_switch_forks( controller::block_status s ) {
      ...
      if( new_head->header.previous == head->id ) {
         ...
         FSC_ASSERT( self.head_block_id() == branches.second.back()->header.previous, fork_database_exception,
   ...
   void finalize_block()
   {
      FSC_ASSERT(pending, block_validate_exception, "it is not valid to finalize when there is no pending block");
      try {
      ...
      */
      ...
      uint64_t CPU_TARGET = FSC_PERCENT(chain_config.max_block_cpu_usage, chain_config.target_block_cpu_usage_pct);
      resource_limits.set_block_parameters(
         ...
         {FSC_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, max_virtual_mult, {99, 100}, {1000, 999}}
      );
   ...
   void check_actor_list( const flat_set<account_name>& actors )const {
      ...
      if( conf.actor_whitelist.size() > 0 ) {
         ...
         FSC_ASSERT( is_subset,  actor_whitelist_exception,
         ...
         FSC_ASSERT( !intersects, actor_blacklist_exception,
   ...
   void check_contract_list( account_name code )const {
      if( conf.contract_whitelist.size() > 0 ) {
         FSC_ASSERT( conf.contract_whitelist.find( code ) != conf.contract_whitelist.end(),
                     contract_whitelist_exception,
                     "account '${code}' is not on the contract whitelist", ("code", code)
                   );
      } else if( conf.contract_blacklist.size() > 0 ) {
         FSC_ASSERT( conf.contract_blacklist.find( code ) == conf.contract_blacklist.end(),
   ...
   void check_action_list( account_name code, action_name action )const {
      if( conf.action_blacklist.size() > 0 ) {
         FSC_ASSERT( conf.action_blacklist.find( std::make_pair(code, action) ) == conf.action_blacklist.end(),
   ...
   void check_key_list( const public_key_type& key )const {
      if( conf.key_blacklist.size() > 0 ) {
         FSC_ASSERT( conf.key_blacklist.find( key ) == conf.key_blacklist.end(),
   ...
   void validate_tapos( const transaction& trx )const {
      ...
      FSC_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), invalid_ref_block_exception,
...
transaction_trace_ptr controller::push_transaction( const transaction_metadata_ptr& trx, fc::time_point deadline, uint32_t billed_cpu_time_us ) {
   validate_db_available_size();
   FSC_ASSERT( get_read_mode() != chain::db_read_mode::READ_ONLY, transaction_type_exception, "push transaction not allowed in read-only mode" );
   FSC_ASSERT( trx && !trx->implicit && !trx->scheduled, transaction_type_exception, "Implicit/Scheduled transaction not allowed" );
...
void controller::set_action_blacklist( const flat_set< pair<account_name, action_name> >& new_action_blacklist ) {
   for (auto& act: new_action_blacklist) {
      FSC_ASSERT(act.first != account_name(), name_type_exception, "Action blacklist - contract name should not be empty");
      FSC_ASSERT(act.second != action_name(), action_type_exception, "Action blacklist - action name should not be empty");
   }
...
time_point controller::pending_block_time()const {
   FSC_ASSERT( my->pending, block_validate_exception, "no pending block" );
...
block_id_type controller::get_block_id_for_num( uint32_t block_num )const { try {
   ...
   FSC_ASSERT( BOOST_LIKELY( signed_blk != nullptr ), unknown_block_exception,
...
void controller::write_snapshot( const snapshot_writer_ptr& snapshot ) const {
   FSC_ASSERT( !my->pending, block_validate_exception, "cannot take a consistent snapshot with a pending block" );
...
unapplied_transactions_type& controller::get_unapplied_transactions() {
   if ( my->read_mode != db_read_mode::SPECULATIVE ) {
      FSC_ASSERT( my->unapplied_transactions.empty(), transaction_exception,
...
void controller::validate_expiration( const transaction& trx )const { try {
   ...
   FSC_ASSERT( time_point(trx.expiration) >= pending_block_time(),
               expired_tx_exception,
               "transaction has expired, "
               "expiration is ${trx.expiration} and pending block time is ${pending_block_time}",
               ("trx.expiration",trx.expiration)("pending_block_time",pending_block_time()));
   FSC_ASSERT( time_point(trx.expiration) <= pending_block_time() + fc::seconds(chain_configuration.max_transaction_lifetime),
...
void controller::validate_db_available_size() const {
   ...
   FSC_ASSERT(free >= guard, database_guard_exception, "database free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
void controller::validate_reversible_available_size() const {
   ...
   FSC_ASSERT(free >= guard, reversible_guard_exception, "reversible free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
