...
datastream<ST>& operator << (datastream<ST>& s, const fscio::chain::may_not_exist<T>& v) {
   raw::pack(s, v.value);
   return s;
}

template<typename ST, typename T>
datastream<ST>& operator >> (datastream<ST>& s, fscio::chain::may_not_exist<T>& v) {
   if (s.remaining())
...
template<typename T>
void to_variant(const fscio::chain::may_not_exist<T>& e, fc::variant& v) {
   to_variant( e.value, v);
}

template<typename T>
void from_variant(const fc::variant& v, fscio::chain::may_not_exist<T>& e) {
   from_variant( v, e.value );
}

} // namespace fc

FC_REFLECT( fscio::chain::type_def                         , (new_type_name)(type) )
FC_REFLECT( fscio::chain::field_def                        , (name)(type) )
FC_REFLECT( fscio::chain::struct_def                       , (name)(base)(fields) )
FC_REFLECT( fscio::chain::action_def                       , (name)(type)(ricardian_contract) )
FC_REFLECT( fscio::chain::table_def                        , (name)(index_type)(key_names)(key_types)(type) )
FC_REFLECT( fscio::chain::clause_pair                      , (id)(body) )
FC_REFLECT( fscio::chain::error_message                    , (error_code)(error_msg) )
FC_REFLECT( fscio::chain::variant_def                      , (name)(types) )
FC_REFLECT( fscio::chain::abi_def                          , (version)(types)(structs)(actions)(tables)
