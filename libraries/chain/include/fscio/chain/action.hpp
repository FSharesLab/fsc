...
namespace fscio { namespace chain {
    ...
    *  In the context of fscio, every action is dispatched to the handler defined
   ...
   struct action {
      ...
      template<typename T>
      T data_as()const {
         FSC_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
         FSC_ASSERT( name == T::get_name(), action_type_exception, "action name is not consistent with action struct"  );
...
FC_REFLECT( fscio::chain::permission_level, (actor)(permission) )
FC_REFLECT( fscio::chain::action, (account)(name)(authorization)(data) )
