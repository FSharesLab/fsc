...
#include <fscio/chain/webassembly/runtime_interface.hpp>
...
namespace fscio { namespace chain { namespace webassembly { namespace wabt_runtime {

using namespace fc;
using namespace wabt;
using namespace wabt::interp;
using namespace fscio::chain::webassembly::common;
...
struct wabt_apply_instance_vars {
   ...
   char* get_validated_pointer(uint32_t offset, uint32_t size) {
      FSC_ASSERT(memory, wasm_execution_error, "access violation");
      FSC_ASSERT(offset + size <= memory->data.size() && offset + size >= offset, wasm_execution_error, "access violation");
...
class wabt_runtime : public fscio::chain::wasm_runtime_interface {
   public:
      wabt_runtime();
...
{
   FSC_ASSERT( length < INT_MAX/(uint32_t)sizeof(T), binaryen_exception, "length will overflow" );
...
inline auto convert_native_to_literal(const wabt_apply_instance_vars& vars, char* ptr) {
   ...
   FSC_ASSERT(ptr >= base && ptr < top_of_memory, wasm_execution_error, "returning pointer not in linear memory");
   Value v;
   v.i32 = (int)(ptr - base);
...
struct intrinsic_invoker_impl<Ret, std::tuple<T &, Inputs...>> {
   ...
   static auto translate_one(wabt_apply_instance_vars& vars, Inputs... rest, const TypedValues& args, int offset) -> std::enable_if_t<std::is_const<U>::value, Ret> {
      ...
      uint32_t ptr = args.at((uint32_t)offset).get_i32();
      FSC_ASSERT(ptr != 0, binaryen_exception, "references cannot be created for null pointers");
      T* base = array_ptr_impl<T>(vars, ptr, 1);
      if ( reinterpret_cast<uintptr_t>(base) % alignof(T) != 0 ) {
         if(vars.ctx.control.contracts_console())
...
#define _REGISTER_WABT_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   static fscio::chain::webassembly::wabt_runtime::intrinsic_registrator _INTRINSIC_NAME(__wabt_intrinsic_fn, __COUNTER__) (\
      MOD,\
      NAME,\
      fscio::chain::webassembly::wabt_runtime::wabt_function_type_provider<WASM_SIG>::type(),\
      fscio::chain::webassembly::wabt_runtime::intrinsic_function_invoker_wrapper<SIG>::type::fn<&CLS::METHOD>()\
   );\

} } } }// fscio::chain::webassembly::wabt_runtime
