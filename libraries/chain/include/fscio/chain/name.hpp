...
#define N(X) fscio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<fscio::chain::name> : private hash<uint64_t> {
      typedef fscio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const fscio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, fscio::chain::name& check);
} // fc


FC_REFLECT( fscio::chain::name, (value) )
