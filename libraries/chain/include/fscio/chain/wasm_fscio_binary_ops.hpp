...
namespace fscio { namespace chain { namespace wasm_ops {
...
struct field_specific_params {
   ...
   static void pack(instruction_stream* stream, Field& f) { return fscio::chain::wasm_ops::pack(stream, f); }
   static auto to_string(Field& f) { return std::string(" ")+
                                       fscio::chain::wasm_ops::to_string(f); }
};
template <>
...
template <class Op_Types>
struct FSCIO_OperatorDecoderStream
{
   FSCIO_OperatorDecoderStream(const std::vector<U8>& codeBytes)
   ...
   instr* decodeOp() {
      FSC_ASSERT(nextByte + sizeof(IR::Opcode) <= end, wasm_exception, "");
      ...
      #define VISIT_OPCODE(opcode,name,nameString,Imm,...) \
         case IR::Opcode::name: \
         { \
            FSC_ASSERT(nextByte + sizeof(IR::OpcodeAndImm<IR::Imm>) <= end, wasm_exception, ""); \
...
template <class Op_Types>
const std::vector<instr*>* FSCIO_OperatorDecoderStream<Op_Types>::_cached_ops;

}}} // namespace fscio, chain, wasm_ops

FC_REFLECT_TEMPLATE( (typename T), fscio::chain::wasm_ops::block< T >, (code)(rt) )
FC_REFLECT( fscio::chain::wasm_ops::memarg, (a)(o) )
FC_REFLECT( fscio::chain::wasm_ops::blocktype, (result) )
FC_REFLECT( fscio::chain::wasm_ops::memoryoptype, (end) )
