...
namespace fscio { namespace chain { namespace wasm_validations {
   ...
   struct nested_validator {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         if (!disabled) {
            ...
            depth++;
            FSC_ASSERT(depth < 1024, wasm_execution_error, "Nested depth exceeded");
         }
      }
   };
   ...
   class wasm_binary_validation {
      ...
      public:
         wasm_binary_validation( const fscio::chain::controller& control, IR::Module& mod ) : _module( &mod ) {
         ...
         void validate() {
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::FSCIO_OperatorDecoderStream<op_constrainers> decoder(fd.code);
               while ( decoder ) {
