...
#include <fscio/chain/wasm_fscio_binary_ops.hpp>
#include <fscio/chain/wasm_fscio_constraints.hpp>
#include <fscio/chain/webassembly/common.hpp>
...
namespace fscio { namespace chain { namespace wasm_injections {
   using namespace IR;
   ...
   struct injector_utils {
      ...
      static void add_import(Module& module, const char* func_name, int32_t& index ) {
         if (module.functions.imports.size() == 0 || registered_injected.find(func_name) == registered_injected.end() ) {
            ...
            decltype(module.functions.imports) new_import = { {{func_type_index}, FSCIO_INJECTED_MODULE_NAME, std::move(func_name)} };
...
#undef CLEAR
   ...
   struct call_depth_check_and_insert_checktime {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         if ( global_idx == -1 ) {
            arg.module->globals.defs.push_back({{ValueType::i32, true}, {(I32) fscio::chain::wasm_constraints::maximum_call_depth}});
         }
...
#undef INSERT_INJECTED
   ...
   struct f32_promote_injector {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         int32_t idx;
         injector_utils::add_import<ResultType::f64, ValueType::f32>( *(arg.module), u8"_fscio_f32_promote", idx );
   ...
   struct f64_demote_injector {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         int32_t idx;
         injector_utils::add_import<ResultType::f32, ValueType::f64>( *(arg.module), u8"_fscio_f64_demote", idx );
         wasm_ops::op_types<>::call_t f32promote;
         f32promote.field = idx;
         f32promote.pack(arg.new_code);
      }
      ...
      public:
         ...
         void inject() {
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::FSCIO_OperatorDecoderStream<pre_op_injectors> pre_decoder(fd.code);
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::FSCIO_OperatorDecoderStream<post_op_injectors> post_decoder(fd.code);
...
}}} // namespace wasm_constraints, chain, fscio
