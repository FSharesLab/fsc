...
#include <fscio/chain/symbol.hpp>
...
struct asset
{
   ...
   explicit asset(share_type a = 0, symbol id = symbol(CORE_SYMBOL)) :amount(a), sym(id) {
      FSC_ASSERT( is_amount_within_range(), asset_type_exception, "magnitude of asset amount must be less than 2^62" );
      FSC_ASSERT( sym.valid(), asset_type_exception, "invalid symbol" );
   }
   ...
   {
      FSC_ASSERT(get_symbol() == o.get_symbol(), asset_type_exception, "addition between two different asset is not allowed");
      amount += o.amount;
      return *this;
   }
   ...
   asset& operator -= (const asset& o)
   {
      FSC_ASSERT(get_symbol() == o.get_symbol(), asset_type_exception, "subtraction between two different asset is not allowed");
      amount -= o.amount;
      return *this;
   }
   ...
   friend bool operator < (const asset& a, const asset& b)
   {
      FSC_ASSERT(a.get_symbol() == b.get_symbol(), asset_type_exception, "logical operation between two different asset is not allowed");
   ...
   friend asset operator - (const asset& a, const asset& b) {
      FSC_ASSERT(a.get_symbol() == b.get_symbol(), asset_type_exception, "subtraction between two different asset is not allowed");
      return asset(a.amount - b.amount, a.get_symbol());
   }

   friend asset operator + (const asset& a, const asset& b) {
      FSC_ASSERT(a.get_symbol() == b.get_symbol(), asset_type_exception, "addition between two different asset is not allowed");
      return asset(a.amount + b.amount, a.get_symbol());
   }
...
namespace fc {
inline void to_variant(const fscio::chain::asset& var, fc::variant& vo) { vo = var.to_string(); }
inline void from_variant(const fc::variant& var, fscio::chain::asset& vo) {
   vo = fscio::chain::asset::from_string(var.get_string());
}
}

FC_REFLECT(fscio::chain::asset, (amount)(sym))
FC_REFLECT(fscio::chain::extended_asset, (quantity)(contract) )
