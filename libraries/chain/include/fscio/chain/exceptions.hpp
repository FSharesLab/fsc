...
#define FSC_ASSERT( expr, exc_type, FORMAT, ... )                \
   FC_MULTILINE_MACRO_BEGIN                                           \
   if( !(expr) )                                                      \
      FC_THROW_EXCEPTION( exc_type, FORMAT, __VA_ARGS__ );            \
   FC_MULTILINE_MACRO_END

#define FSC_THROW( exc_type, FORMAT, ... ) \
    throw exc_type( FC_LOG_MESSAGE( error, FORMAT, __VA_ARGS__ ) );

/**
 * Macro inspired from FC_RETHROW_EXCEPTIONS
 * The main difference here is that if the exception caught isn't of type "fscio::chain::chain_exception"
 * This macro will rethrow the exception as the specified "exception_type"
 */
#define FSC_RETHROW_EXCEPTIONS(exception_type, FORMAT, ... ) \
   catch (fscio::chain::chain_exception& e) { \
 ...
 * This macro will rethrow the exception as the specified "exception_type"
 */
#define FSC_CAPTURE_AND_RETHROW( exception_type, ... ) \
   catch (fscio::chain::chain_exception& e) { \
...
#define FSC_RECODE_EXC( cause_type, effect_type ) \
...
namespace fscio { namespace chain {
   ...
   FC_DECLARE_DERIVED_EXCEPTION( action_validate_exception, chain_exception,
      ...
      FC_DECLARE_DERIVED_EXCEPTION( fscio_assert_message_exception, action_validate_exception,
                                    3050003, "fscio_assert_message assertion failure" )
      FC_DECLARE_DERIVED_EXCEPTION( fscio_assert_code_exception, action_validate_exception,
                                    3050004, "fscio_assert_code assertion failure" )
