...
namespace fscio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(fscio);
const static uint64_t null_account_name      = N(fscio.null);
const static uint64_t producers_account_name = N(fscio.prods);
...
const static uint64_t fscio_auth_scope       = N(fscio.auth);
const static uint64_t fscio_all_scope        = N(fscio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t fscio_any_name = N(fscio.any);
const static uint64_t fscio_code_name = N(fscio.code);
...
const static fscio::chain::wasm_interface::vm_type default_wasm_runtime = fscio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace fscio::chain::config

constexpr uint64_t FSC_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / fscio::chain::config::percent_100;
}

template<typename Number>
Number FSC_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + fscio::chain::config::percent_100 - fscio::chain::config::percent_1)  / fscio::chain::config::percent_100;
}
