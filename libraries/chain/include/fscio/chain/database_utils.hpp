...
namespace fc {
   ...
   inline
   void to_variant( const fscio::chain::shared_string& s, variant& v ) {
      v = variant(std::string(s.begin(), s.end()));
   }

   inline
   void from_variant( const variant& v, fscio::chain::shared_string& s ) {
      string _s;
      from_variant(v, _s);
      s = fscio::chain::shared_string(_s.begin(), _s.end(), s.get_allocator());
   }

   inline
   void to_variant( const fscio::chain::shared_blob& b, variant& v ) {
      v = variant(base64_encode(b.data(), b.size()));
   }

   inline
   void from_variant( const variant& v, fscio::chain::shared_blob& b ) {
      string _s = base64_decode(v.as_string());
      b = fscio::chain::shared_blob(_s.begin(), _s.end(), b.get_allocator());
   }
   ...
   template<typename T>
   void to_variant( const fscio::chain::shared_vector<T>& sv, variant& v ) {
      to_variant(std::vector<T>(sv.begin(), sv.end()), v);
   }

   template<typename T>
   void from_variant( const variant& v, fscio::chain::shared_vector<T>& sv ) {
      std::vector<T> _v;
      from_variant(v, _v);
      sv = fscio::chain::shared_vector<T>(_v.begin(), _v.end(), sv.get_allocator());
   }
}
...
DataStream& operator << ( DataStream& ds, const float128_t& v ) {
   fc::raw::pack(ds, *reinterpret_cast<const fscio::chain::uint128_t*>(&v));
   return ds;
}

template<typename DataStream>
DataStream& operator >> ( DataStream& ds, float128_t& v ) {
   fc::raw::unpack(ds, *reinterpret_cast<fscio::chain::uint128_t*>(&v));
   return ds;
}
