...
#include <fscio/chain/webassembly/wavm.hpp>
#include <fscio/chain/webassembly/wabt.hpp>
...
using namespace fc;
using namespace fscio::chain::webassembly;
using namespace IR;
   ...
   struct wasm_interface_impl {
      wasm_interface_impl(wasm_interface::vm_type vm) {
         if(vm == wasm_interface::vm_type::wavm)
            ...
            FSC_THROW(wasm_exception, "wasm_interface_impl fall through");
      }
      ...
      std::vector<uint8_t> parse_initial_memory(const Module& module) {
         ...
         for(const DataSegment& data_segment : module.dataSegments) {
            FSC_ASSERT(data_segment.baseOffset.type == InitializerExpression::Type::i32_const, wasm_exception, "");
            FSC_ASSERT(module.memories.defs.size(), wasm_exception, "");
...
#define _REGISTER_INJECTED_INTRINSIC(R, CLS, INFO)\
   BOOST_PP_CAT(BOOST_PP_OVERLOAD(_REGISTER_INTRINSIC, _UNWRAP_SEQ INFO) _EXPAND_ARGS(CLS, FSCIO_INJECTED_MODULE_NAME, INFO), BOOST_PP_EMPTY())
