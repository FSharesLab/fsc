...
namespace impl {
   ...
   struct abi_from_variant {
      ...
      static void extract( const variant& v, action& act, Resolver resolver, abi_traverse_context& ctx )
      {
         auto h = ctx.enter_scope();
         const variant_object& vo = v.get_object();
         FSC_ASSERT(vo.contains("account"), packed_transaction_type_exception, "Missing account");
         FSC_ASSERT(vo.contains("name"), packed_transaction_type_exception, "Missing name");
         ...
         FSC_ASSERT(valid_empty_data || !act.data.empty(), packed_transaction_type_exception,
      ...
      static void extract( const variant& v, packed_transaction& ptrx, Resolver resolver, abi_traverse_context& ctx )
      {
         auto h = ctx.enter_scope();
         const variant_object& vo = v.get_object();
         FSC_ASSERT(vo.contains("signatures"), packed_transaction_type_exception, "Missing signatures");
         FSC_ASSERT(vo.contains("compression"), packed_transaction_type_exception, "Missing compression");
         ...
         if( vo.contains("packed_trx") && vo["packed_trx"].is_string() && !vo["packed_trx"].as_string().empty() ) {
            ...
            FSC_ASSERT(vo.contains("transaction"), packed_transaction_type_exception, "Missing transaction");
            if( use_packed_cfd ) {
               transaction trx;
