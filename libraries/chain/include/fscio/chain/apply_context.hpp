      ...
      class iterator_cache {
         public:
            ...
            const table_id_object& get_table( table_id_object::id_type i )const {
               ...
               FSC_ASSERT( itr != _table_cache.end(), table_not_in_cache, "an invariant was broken, table should be in cache" );
            ...
            const table_id_object* find_table_by_end_iterator( int ei )const {
               FSC_ASSERT( ei < -1, invalid_table_iterator, "not an end iterator" );
            ...
            const T& get( int iterator ) {
               FSC_ASSERT( iterator != -1, invalid_table_iterator, "invalid iterator" );
               FSC_ASSERT( iterator >= 0, table_operation_not_permitted, "dereference of end iterator" );
               FSC_ASSERT( iterator < _iterator_to_object.size(), invalid_table_iterator, "iterator out of range" );
               auto result = _iterator_to_object[iterator];
               FSC_ASSERT( result, table_operation_not_permitted, "dereference of deleted object" );
               return *result;
            }

            void remove( int iterator ) {
               FSC_ASSERT( iterator != -1, invalid_table_iterator, "invalid iterator" );
               FSC_ASSERT( iterator >= 0, table_operation_not_permitted, "cannot call remove on end iterators" );
               FSC_ASSERT( iterator < _iterator_to_object.size(), invalid_table_iterator, "iterator out of range" );
...
//               context.require_write_lock( scope );
            ...
            void remove( int iterator ) {
               ...
               FSC_ASSERT( table_obj.code == context.receiver, table_access_violation, "db access violation" );
...
//FC_REFLECT(fscio::chain::apply_context::apply_results, (applied_actions)(deferred_transaction_requests)(deferred_transactions_count))
