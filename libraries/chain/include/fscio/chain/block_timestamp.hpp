   ...
   class block_timestamp {
      public:
         ...
         block_timestamp next() const {
            FSC_ASSERT( std::numeric_limits<uint32_t>::max() - slot >= 1, fc::overflow_exception, "block timestamp overflow" );
...
FC_REFLECT(fscio::chain::block_timestamp_type, (slot))

namespace fc {
  template<uint16_t IntervalMs, uint64_t EpochMs>
  void to_variant(const fscio::chain::block_timestamp<IntervalMs,EpochMs>& t, fc::variant& v) {
     to_variant( (fc::time_point)t, v);
  }

  template<uint16_t IntervalMs, uint64_t EpochMs>
  void from_variant(const fc::variant& v, fscio::chain::block_timestamp<IntervalMs,EpochMs>& t) {
