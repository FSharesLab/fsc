...
namespace fscio { namespace chain {
   ...
   void fork_database::set( block_state_ptr s ) {
      ...
      FSC_ASSERT( s->id == s->header.id(), fork_database_exception, 
      ...
      FSC_ASSERT( result.second, fork_database_exception, "unable to insert block state, duplicate state detected" );
      if( !my->head ) {
         my->head =  s;
   ...
   block_state_ptr fork_database::add( const block_state_ptr& n, bool skip_validate_previous ) {
      FSC_ASSERT( n, fork_database_exception, "attempt to add null block state" );
      FSC_ASSERT( my->head, fork_db_block_not_found, "no head block set" );

      if( !skip_validate_previous ) {
         auto prior = my->index.find( n->block->previous );
         FSC_ASSERT( prior != my->index.end(), unlinkable_block_exception,
                     "unlinkable block", ("id", n->block->id())("previous", n->block->previous) );
      }

      auto inserted = my->index.insert(n);
      FSC_ASSERT( inserted.second, fork_database_exception, "duplicate block added?" );
   ...
   block_state_ptr fork_database::add( signed_block_ptr b, bool skip_validate_signee ) {
      FSC_ASSERT( b, fork_database_exception, "attempt to add null block" );
      FSC_ASSERT( my->head, fork_db_block_not_found, "no head block set" );

      const auto& by_id_idx = my->index.get<by_block_id>();
      auto existing = by_id_idx.find( b->id() );
      FSC_ASSERT( existing == by_id_idx.end(), fork_database_exception, "we already know about this block" );

      auto prior = by_id_idx.find( b->previous );
      FSC_ASSERT( prior != by_id_idx.end(), unlinkable_block_exception, "unlinkable block", ("id", string(b->id()))("previous", string(b->previous)) );

      auto result = std::make_shared<block_state>( **prior, move(b), skip_validate_signee );
      FSC_ASSERT( result, fork_database_exception , "fail to add new block state" );
   ...
   pair< branch_type, branch_type >  fork_database::fetch_branch_from( const block_id_type& first,
      ...
      while( first_branch->block_num > second_branch->block_num )
      {
         ...
         FSC_ASSERT( first_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", string(first_branch->header.previous)) );
      }
      ...
      while( second_branch->block_num > first_branch->block_num )
      {
         ...
         FSC_ASSERT( second_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", string(second_branch->header.previous)) );
      }
      ...
      while( first_branch->header.previous != second_branch->header.previous )
      {
         ...
         first_branch = get_block( first_branch->header.previous );
         second_branch = get_block( second_branch->header.previous );
         FSC_ASSERT( first_branch && second_branch, fork_db_block_not_found, 
   ...
   void fork_database::mark_in_current_chain( const block_state_ptr& h, bool in_current_chain ) {
      ...
      FSC_ASSERT( itr != by_id_idx.end(), fork_db_block_not_found, "could not find block in fork database" );
   ...
   void fork_database::add( const header_confirmation& c ) {
      ...
      FSC_ASSERT( b, fork_db_block_not_found, "unable to find block id ${id}", ("id",c.block_id));
