...
#include <fscio/chain/apply_context.hpp>
#include <fscio/chain/controller.hpp>
#include <fscio/chain/transaction_context.hpp>
...
#include <fscio/chain/wasm_interface_private.hpp>
#include <fscio/chain/wasm_fscio_validation.hpp>
#include <fscio/chain/wasm_fscio_injection.hpp>
...
namespace fscio { namespace chain {
   ...
   void wasm_interface::validate(const controller& control, const bytes& code) {
      ...
      try {
         ...
         FSC_ASSERT(false, wasm_serialization_error, e.message.c_str());
   ...
   public:
      ...
      :context(ctx)
      {
         if( context.context_free )
            FSC_ASSERT( context_free, unaccessible_api, "only context free api's can be used in this context" );
...
class context_free_api : public context_aware_api {
   public:
      ...
      :context_aware_api(ctx, true) {
         ...
         FSC_ASSERT( context.context_free, unaccessible_api, "this API may only be called from context_free apply" );
      }
...
class privileged_api : public context_aware_api {
   public:
      ...
      {
         FSC_ASSERT( context.privileged, unaccessible_api, "${code} does not have permission to call this API", ("code",context.receiver) );
      }
      ...
      void activate_feature( int64_t feature_name ) {
         FSC_ASSERT( false, unsupported_feature, "Unsupported Hardfork Detected" );
      }
      ...
      void set_resource_limits( account_name account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight) {
         FSC_ASSERT(ram_bytes >= -1, wasm_execution_error, "invalid value for ram resource limit expected [-1,INT64_MAX]");
         FSC_ASSERT(net_weight >= -1, wasm_execution_error, "invalid value for net resource weight expected [-1,INT64_MAX]");
         FSC_ASSERT(cpu_weight >= -1, wasm_execution_error, "invalid value for cpu resource weight expected [-1,INT64_MAX]");
      ...
      int64_t set_proposed_producers( array_ptr<char> packed_producer_schedule, size_t datalen) {
         ...
         FSC_ASSERT(producers.size() <= config::max_producers, wasm_execution_error, "Producer schedule exceeds the maximum producer count for this chain");
         // check that producers are unique
         std::set<account_name> unique_producers;
         for (const auto& p: producers) {
            FSC_ASSERT( context.is_account(p.producer_name), wasm_execution_error, "producer schedule includes a nonexisting account" );
            FSC_ASSERT( p.block_signing_key.valid(), wasm_execution_error, "producer schedule includes an invalid key" );
            unique_producers.insert(p.producer_name);
         }
         FSC_ASSERT( producers.size() == unique_producers.size(), wasm_execution_error, "duplicate producer name in producer schedule" );
   ...
   public:
      ...
      int32_t _fscio_f32_trunc_i32s( float af ) {
         ...
         if (_fscio_f32_ge(af, 2147483648.0f) || _fscio_f32_lt(af, -2147483648.0f))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f32.convert_s/i32 overflow" );

         if (is_nan(a))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f32.convert_s/i32 unrepresentable");
      ...
      int32_t _fscio_f64_trunc_i32s( double af ) {
         ...
         if (_fscio_f64_ge(af, 2147483648.0) || _fscio_f64_lt(af, -2147483648.0))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f64.convert_s/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f64.convert_s/i32 unrepresentable");
      ...
      uint32_t _fscio_f32_trunc_i32u( float af ) {
         ...
         if (_fscio_f32_ge(af, 4294967296.0f) || _fscio_f32_le(af, -1.0f))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f32.convert_u/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f32.convert_u/i32 unrepresentable");
      ...
      uint32_t _fscio_f64_trunc_i32u( double af ) {
         ...
         if (_fscio_f64_ge(af, 4294967296.0) || _fscio_f64_le(af, -1.0))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f64.convert_u/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f64.convert_u/i32 unrepresentable");
      ...
      int64_t _fscio_f32_trunc_i64s( float af ) {
         ...
         if (_fscio_f32_ge(af, 9223372036854775808.0f) || _fscio_f32_lt(af, -9223372036854775808.0f))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f32.convert_s/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f32.convert_s/i64 unrepresentable");
      ...
      int64_t _fscio_f64_trunc_i64s( double af ) {
         ...
         if (_fscio_f64_ge(af, 9223372036854775808.0) || _fscio_f64_lt(af, -9223372036854775808.0))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f64.convert_s/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f64.convert_s/i64 unrepresentable");
      ...
      uint64_t _fscio_f32_trunc_i64u( float af ) {
         ...
         if (_fscio_f32_ge(af, 18446744073709551616.0f) || _fscio_f32_le(af, -1.0f))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f32.convert_u/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f32.convert_u/i64 unrepresentable");
      ...
      uint64_t _fscio_f64_trunc_i64u( double af ) {
         ...
         if (_fscio_f64_ge(af, 18446744073709551616.0) || _fscio_f64_le(af, -1.0))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f64.convert_u/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( fscio::chain::wasm_execution_error, "Error, f64.convert_u/i64 unrepresentable");
...
class crypto_api : public context_aware_api {
   public:
      ...
      void assert_recover_key( const fc::sha256& digest,
         ...
         FSC_ASSERT( check == p, crypto_api_exception, "Error expected key different than recovered key" );
      }
      ...
      template<class Encoder> auto encode(char* data, size_t datalen) {
         Encoder e;
         const size_t bs = fscio::chain::config::hashing_checktime_block_size;
         while ( datalen > bs ) {
            e.write( data, bs );
            data += bs;
            datalen -= bs;
      ...
      void assert_sha256(array_ptr<char> data, size_t datalen, const fc::sha256& hash_val) {
         ...
         FSC_ASSERT( result == hash_val, crypto_api_exception, "hash mismatch" );
      }
...
class permission_api : public context_aware_api {
   public:
      ...
      {
         FSC_ASSERT( delay_us <= static_cast<uint64_t>(std::numeric_limits<int64_t>::max()),
      ...
      int64_t get_account_creation_time( account_name account ) {
         ...
         FSC_ASSERT( acct != nullptr, action_validate_exception,
...
public:
   ...
   void abort() {
      FSC_ASSERT( false, abort_called, "abort() called");
   }

   // Kept as intrinsic rather than implementing on WASM side (using fscio_assert_message and strlen) because strlen is faster on native side.
   void fscio_assert( bool condition, null_terminated_ptr msg ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         std::string message( msg );
         FSC_THROW( fscio_assert_message_exception, "assertion failure with message: ${s}", ("s",message) );
      }
   }

   void fscio_assert_message( bool condition, array_ptr<const char> msg, size_t msg_len ) {
      if( BOOST_UNLIKELY( !condition ) ) {
   ...
   void fscio_assert_code( bool condition, uint64_t error_code ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         FSC_THROW( fscio_assert_code_exception,
...
#define DB_API_METHOD_WRAPPERS_FLOAT_SECONDARY(IDX, TYPE)\
      int db_##IDX##_store( uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, const TYPE& secondary ) {\
         FSC_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
         return context.IDX.store( scope, table, payer, id, secondary );\
      }\
      void db_##IDX##_update( int iterator, uint64_t payer, const TYPE& secondary ) {\
         FSC_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
...
class memory_api : public context_aware_api {
   public:
      ...
      char* memcpy( array_ptr<char> dest, array_ptr<const char> src, size_t length) {
         FSC_ASSERT((std::abs((ptrdiff_t)dest.value - (ptrdiff_t)src.value)) >= length,
...
class transaction_api : public context_aware_api {
   public:
      ...
      void send_inline( array_ptr<char> data, size_t data_len ) {
         ...
         FSC_ASSERT( data_len < context.control.get_global_properties().configuration.max_inline_action_size, inline_action_too_big,
...
class compiler_builtins : public context_aware_api {
   public:
      ...
      void __divti3(__int128& ret, uint64_t la, uint64_t ha, uint64_t lb, uint64_t hb) {
         ...
         FSC_ASSERT(rhs != 0, arithmetic_exception, "divide by zero");
...
/*
 * This api will be removed with fix for `fsc #2561`
 */
...
REGISTER_INTRINSICS(context_free_system_api,
   (abort,                void()              )
   (fscio_assert,         void(int, int)      )
   (fscio_assert_message, void(int, int, int) )
   (fscio_assert_code,    void(int, int64_t)  )
   (fscio_exit,           void(int)           )
);
...
REGISTER_INJECTED_INTRINSICS(softfloat_api,
      (_fscio_f32_add,       float(float, float)    )
      (_fscio_f32_sub,       float(float, float)    )
      (_fscio_f32_mul,       float(float, float)    )
      (_fscio_f32_div,       float(float, float)    )
      (_fscio_f32_min,       float(float, float)    )
      (_fscio_f32_max,       float(float, float)    )
      (_fscio_f32_copysign,  float(float, float)    )
      (_fscio_f32_abs,       float(float)           )
      (_fscio_f32_neg,       float(float)           )
      (_fscio_f32_sqrt,      float(float)           )
      (_fscio_f32_ceil,      float(float)           )
      (_fscio_f32_floor,     float(float)           )
      (_fscio_f32_trunc,     float(float)           )
      (_fscio_f32_nearest,   float(float)           )
      (_fscio_f32_eq,        int(float, float)      )
      (_fscio_f32_ne,        int(float, float)      )
      (_fscio_f32_lt,        int(float, float)      )
      (_fscio_f32_le,        int(float, float)      )
      (_fscio_f32_gt,        int(float, float)      )
      (_fscio_f32_ge,        int(float, float)      )
      (_fscio_f64_add,       double(double, double) )
      (_fscio_f64_sub,       double(double, double) )
      (_fscio_f64_mul,       double(double, double) )
      (_fscio_f64_div,       double(double, double) )
      (_fscio_f64_min,       double(double, double) )
      (_fscio_f64_max,       double(double, double) )
      (_fscio_f64_copysign,  double(double, double) )
      (_fscio_f64_abs,       double(double)         )
      (_fscio_f64_neg,       double(double)         )
      (_fscio_f64_sqrt,      double(double)         )
      (_fscio_f64_ceil,      double(double)         )
      (_fscio_f64_floor,     double(double)         )
      (_fscio_f64_trunc,     double(double)         )
      (_fscio_f64_nearest,   double(double)         )
      (_fscio_f64_eq,        int(double, double)    )
      (_fscio_f64_ne,        int(double, double)    )
      (_fscio_f64_lt,        int(double, double)    )
      (_fscio_f64_le,        int(double, double)    )
      (_fscio_f64_gt,        int(double, double)    )
      (_fscio_f64_ge,        int(double, double)    )
      (_fscio_f32_promote,    double(float)         )
      (_fscio_f64_demote,     float(double)         )
      (_fscio_f32_trunc_i32s, int(float)            )
      (_fscio_f64_trunc_i32s, int(double)           )
      (_fscio_f32_trunc_i32u, int(float)            )
      (_fscio_f64_trunc_i32u, int(double)           )
      (_fscio_f32_trunc_i64s, int64_t(float)        )
      (_fscio_f64_trunc_i64s, int64_t(double)       )
      (_fscio_f32_trunc_i64u, int64_t(float)        )
      (_fscio_f64_trunc_i64u, int64_t(double)       )
      (_fscio_i32_to_f32,     float(int32_t)        )
      (_fscio_i64_to_f32,     float(int64_t)        )
      (_fscio_ui32_to_f32,    float(int32_t)        )
      (_fscio_ui64_to_f32,    float(int64_t)        )
      (_fscio_i32_to_f64,     double(int32_t)       )
      (_fscio_i64_to_f64,     double(int64_t)       )
      (_fscio_ui32_to_f64,    double(int32_t)       )
      (_fscio_ui64_to_f64,    double(int64_t)       )
);
...
std::istream& operator>>(std::istream& in, wasm_interface::vm_type& runtime) {
   ...
   in >> s;
   if (s == "wavm")
      runtime = fscio::chain::wasm_interface::vm_type::wavm;
   else if (s == "wabt")
      runtime = fscio::chain::wasm_interface::vm_type::wabt;
   else
