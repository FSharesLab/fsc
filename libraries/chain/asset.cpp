...
asset asset::from_string(const string& from)
{
   try {
      ...
      FSC_ASSERT((space_pos != string::npos), asset_type_exception, "Asset's amount and symbol should be separated with space");
      ...
      if (dot_pos != string::npos) {
         FSC_ASSERT((dot_pos != amount_str.size() - 1), asset_type_exception, "Missing decimal fraction after decimal point");
      }

      // Parse symbol
      string precision_digit_str;
      if (dot_pos != string::npos) {
         precision_digit_str = fscio::chain::to_string(amount_str.size() - dot_pos - 1);
      } else {
