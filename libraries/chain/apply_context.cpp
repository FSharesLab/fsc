...
void apply_context::exec( action_trace& trace )
{
   ...
   if( _cfa_inline_actions.size() > 0 || _inline_actions.size() > 0 ) {
      FSC_ASSERT( recurse_depth < control.get_global_properties().configuration.max_inline_action_depth,
...
void apply_context::require_authorization( const account_name& account ) {
   ...
   FSC_ASSERT( false, missing_auth_exception, "missing authority of ${account}", ("account",account));
}
...
void apply_context::require_authorization(const account_name& account,
  ...
  FSC_ASSERT( false, missing_auth_exception, "missing authority of ${account}/${permission}",
...
void apply_context::execute_inline( action&& a ) {
   ...
   FSC_ASSERT( code != nullptr, action_validate_exception,
   ...
   for( const auto& auth : a.authorization ) {
      ...
      FSC_ASSERT( actor != nullptr, action_validate_exception,
                  "inline action's authorizing actor ${account} does not exist", ("account", auth.actor) );
      FSC_ASSERT( control.get_authorization_manager().find_permission(auth) != nullptr, action_validate_exception,
      ...
      try {
         control.get_authorization_manager()
                .check_authorization( {a},
                                      {},
                                      {{receiver, config::fscio_code_name}},
         ...
         if( disallow_send_to_self_bypass || !send_to_self ) {
            ...
            FSC_THROW(subjective_block_production_exception, "Unexpected exception occurred validating inline action sent to self");
         }
      }
   }
...
void apply_context::execute_context_free_inline( action&& a ) {
   ...
   FSC_ASSERT( a.authorization.size() == 0, action_validate_exception,
...
void apply_context::schedule_deferred_transaction( const uint128_t& sender_id, account_name payer, transaction&& trx, bool replace_existing ) {
   FSC_ASSERT( trx.context_free_actions.size() == 0, cfa_inside_generated_tx, "context free actions are not currently allowed in generated transactions" );
   ...
   if( !control.skip_auth_check() && !privileged ) { // Do not need to check authorization if replayng irreversible block or if contract is privileged
      ...
      try {
         ...
         if( disallow_send_to_self_bypass || !is_sending_only_to_self(receiver) ) {
            ...
            FSC_THROW(subjective_block_production_exception, "Unexpected exception occurred validating sent deferred transaction consisting only of actions to self");
         }
      }
   }
   ...
   if ( auto ptr = db.find<generated_transaction_object,by_sender_id>(boost::make_tuple(receiver, sender_id)) ) {
      FSC_ASSERT( replace_existing, deferred_tx_duplicate, "deferred transaction with the same sender_id and payer already exists" );

      // TODO: Remove the following subjective check when the deferred trx replacement RAM bug has been fixed with a hard fork.
      FSC_ASSERT( !control.is_producing_block(), subjective_block_production_exception,
   ...
   FSC_ASSERT( control.is_ram_billing_in_notify_allowed() || (receiver == act.account) || (receiver == payer) || privileged,
...
void apply_context::update_db_usage( const account_name& payer, int64_t delta ) {
   if( delta > 0 ) {
      if( !(privileged || payer == account_name(receiver)) ) {
         FSC_ASSERT( control.is_ram_billing_in_notify_allowed() || (receiver == act.account),
                     subjective_block_production_exception, "Cannot charge RAM to other accounts during notify." );
...
int apply_context::get_action( uint32_t type, uint32_t index, char* buffer, size_t buffer_size )const
{
   ...
   FSC_ASSERT(act_ptr, action_not_found_exception, "action is not found" );
...
//   require_write_lock( scope );
   ...
   FSC_ASSERT( payer != account_name(), invalid_table_payer, "must specify a valid account to pay for new record" );
...
void apply_context::db_update_i64( int iterator, account_name payer, const char* buffer, size_t buffer_size ) {
   ...
   FSC_ASSERT( table_obj.code == receiver, table_access_violation, "db access violation" );
...
int apply_context::db_previous_i64( int iterator, uint64_t& primary ) {
   ...
   if( iterator < -1 ) // is end iterator
   {
      ...
      FSC_ASSERT( tab, invalid_table_iterator, "not a valid end iterator" );
