#include "../../../chain/include/fscio/chain/wasm_fscio_constraints.hpp"
...
namespace WASM
{
	...
	void serializeFunctionBody(InputStream& sectionStream,Module& module,FunctionDef& functionDef)
	{
		...
		constexpr size_t max_size = fscio::chain::wasm_constraints::maximum_code_size;
		...
		for(Uptr setIndex = 0;setIndex < numLocalSets;++setIndex)
		{
			...
			if( locals_accum > fscio::chain::wasm_constraints::maximum_func_local_bytes )
	...
	void serializeImportSection(Stream& moduleStream,Module& module)
	{
		serializeSection(moduleStream,SectionType::import,[&module](Stream& sectionStream)
		{
			...
			constexpr size_t max_size = fscio::chain::wasm_constraints::maximum_section_elements;
			if(Stream::isInput)
			{
