...
#include <net/if.h>
#include <fscio/chain/genesis_state.hpp>
...
void
fscd_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string fscio_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), fscio_home(), instances(1) {}
};
...
struct launcher_def {
   ...
   string fscd_extra_args;
   std::map<uint,string> specific_nodfsc_args;
   testnet_def network;
   string gelf_endpoint;
   ...
   fscio::chain::genesis_state genesis_from_file;

   void assign_name (fscd_def &node, bool is_bios);
   ...
   void prep_remote_config_dir (fscd_def &node, host_def *host);
   void launch (fscd_def &node, string &gts);
   void kill (launch_modes mode, string sig_opt);
   static string get_node_num(uint16_t node_num);
   pair<host_def, fscd_def> find_node(uint16_t node_num);
   vector<pair<host_def, fscd_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "nodfsc does not require transaction signatures.")
    ("nodfsc", bpo::value<string>(&fscd_extra_args), "forward nodfsc command line argument(s) to each instance of nodfsc, enclose arg(s) in quotes")
    ("specific-num", bpo::value<vector<uint>>()->composing(), "forward nodfsc command line argument(s) (using \"--specific-nodfsc\" flag) to this specific instance of nodfsc. This parameter can be entered multiple times and requires a paired \"--specific-nodfsc\" flag")
    ("specific-nodfsc", bpo::value<vector<string>>()->composing(), "forward nodfsc command line argument(s) to its paired specific instance of nodfsc(using \"--specific-num\"), enclose arg(s) in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many nodfsc instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if (vmap.count("specific-num")) {
    ...
    const auto specific_args = vmap["specific-nodfsc"].as<vector<string>>();
    if (specific_nums.size() != specific_args.size()) {
      cerr << "ERROR: every specific-num argument must be paired with a specific-nodfsc argument" << endl;
      exit (-1);
    }
    ...
    for(uint i = 0; i < specific_nums.size(); ++i)
    {
      ...
      specific_nodfsc_args[num] = specific_args[i];
    }
  }
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &fscd : binding.instances) {
          fscd.host = binding.host_name;
          fscd.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(fscd.p2p_port);

          aliases.push_back (fscd.name);
        }
      }
  ...
  char* erd_env_var = getenv ("FSCIO_HOME");
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable FSCIO_HOME is set to the build path." << endl;
    exit (-1);
  }
...
void
launcher_def::assign_name (fscd_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.fscio_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      fscd_def fscd;
      assign_name(fscd, i == 0);
      aliases.push_back(fscd.name);
      fscd.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(fscd));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->fscio_home =
          (local_id.contains (lhost->host_name) || servers.default_fscio_home.empty()) ?
          erd : servers.default_fscio_home;
        host_ndx++;
      } // ph_count == 0

      fscd_def fscd;
      assign_name(fscd, do_bios);
      fscd.has_db = false;
      ...
      aliases.push_back(fscd.name);
      fscd.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(fscd));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  fscd_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->fscio_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->fscio_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->fscio_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->fscio_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->fscio_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = fscio::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = fscio::mongo_db_plugin\n";
  }
  if ( p2p == p2p_plugin::NET ) {
    cfg << "plugin = fscio::net_plugin\n";
  } else {
    cfg << "plugin = fscio::bnet_plugin\n";
  }
  cfg << "plugin = fscio::chain_api_plugin\n"
...
launcher_def::init_genesis () {
   ...
   if (!bfs::exists(genesis_path)) {
      ...
      fscio::chain::genesis_state default_genesis;
      fc::json::save_to_file( default_genesis, genesis_path, true );
   }
   string bioskey = string(network.nodes["bios"].keys[0].get_public_key());

   fc::json::from_file(genesis_path).as<fscio::chain::genesis_state>(genesis_from_file);
...
void
launcher_def::prep_remote_config_dir (fscd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->fscio_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->fscio_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->fscio_home;

  cmd = "cd " + host->fscio_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->fscio_home
...
void
launcher_def::launch (fscd_def &instance, string &gts) {
  ...
  string fscdcmd = "programs/nodfsc/nodfsc ";
  if (skip_transaction_signatures) {
    fscdcmd += "--skip-transaction-signatures ";
  }
  if (!fscd_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +fscio::mongo_db_plugin");
       string args = std::regex_replace (fscd_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       fscdcmd += args + " ";
    }
    else {
       fscdcmd += fscd_extra_args + " ";
    }
  }
  if (instance.name != "bios" && !specific_nodfsc_args.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodfsc_args.count(node_num)) {
        fscdcmd += specific_nodfsc_args[node_num] + " ";
     }
  }

  if( add_enable_stale_production ) {
    fscdcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  fscdcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  fscdcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    fscdcmd += " --genesis-timestamp " + gts;
  }

  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->fscio_home + "; nohup " + fscdcmd + " > "
    ...
    string cmd = "cd " + host->fscio_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << fscdcmd << endl;

    bp::child c(fscdcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << fscdcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(fscd_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, fscd_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, fscd_def>> node_list;
...
launcher_def::do_command(const host_def& host, const string& name,
   ...
   if (!host.is_local()) {
      string rcmd = "cd " + host.fscio_home + "; ";
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const fscd_def& node = node_pair.second;
      const string node_num = node.get_node_num();
      cout << "Bouncing " << node.name << endl;
      string cmd = "./scripts/fscio-tn_bounce.sh " + fscd_extra_args;
      if (node_num != "bios" && !specific_nodfsc_args.empty()) {
         const auto node_num_i = boost::lexical_cast<uint16_t,string>(node_num);
         if (specific_nodfsc_args.count(node_num_i)) {
            cmd += " " + specific_nodfsc_args[node_num_i];
         }
      }

      do_command(host, node.name, { { "FSCIO_HOME", host.fscio_home }, { "FSCIO_NODE", node_num } }, cmd);
   }
}
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/fscio-tn_down.sh ";
      do_command(host, node.name,
                 { { "FSCIO_HOME", host.fscio_home }, { "FSCIO_NODE", node_num }, { "FSCIO_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
...
launcher_def::roll (const string& host_names) {
   ...
   for (string host_name: hosts) {
      ...
      string cmd = "./scripts/fscio-tn_roll.sh ";
      do_command(*host, host_name, { { "FSCIO_HOME", host->fscio_home } }, cmd);
   }
}
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the fscio-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the fscio-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the fscio-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << fscio::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(fscio_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (fscio_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_fscio_home) (ssh) )
