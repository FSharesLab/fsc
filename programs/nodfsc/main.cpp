/**
 *  @file
 *  @copyright defined in fscio/LICENSE.txt
 */
...
{
   try {
      app().set_version(fscio::nodfsc::config::version);

      auto root = fc::app_path();
      app().set_default_data_dir(root / "fscio/nodfsc/data" );
      app().set_default_config_dir(root / "fscio/nodfsc/config" );
      ...
      initialize_logging();
      ilog("nodfsc version ${ver}", ("ver", app().version_string()));
      ilog("fscio root is ${root}", ("root", root.string()));
      ilog("nodfsc using configuration file ${c}", ("c", app().full_config_file_path().string()));
      ilog("nodfsc data directory is ${d}", ("d", app().data_dir().string()));
      app().startup();
      app().exec();
