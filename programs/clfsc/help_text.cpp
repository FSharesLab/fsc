...
const char* history_plugin_advice_text = _("\033[32mPlease ensure that the \033[2mfscio::history_plugin\033[22m is enabled on the RPC node you are connecting to and that an account involved in this transaction was configured in the \033[2mfilter-on\033[22m setting.\033[0m\n");
...
const char* error_advice_public_key_type_exception = R"=====(Public key should be encoded in base58 and starts with FSC prefix)=====";
...
const char* error_advice_authority_type_exception = R"=====(Ensure that your authority JSON is valid follows the following format!
{
  ...
  "keys": [         <keys must be alpha-numerically sorted by their string representations and unique>
    ...
    {
      "key":        <STRING: FSC.IO compatible Public Key>,
...
const char* error_advice_tx_duplicate =  "You can try embedding fscio nonce action inside your transaction to ensure uniqueness.";

const char* error_advice_invalid_action_args_exception = R"=====(Ensure that your arguments follow the contract abi!
You can check the contract's abi by using 'clfsc get code' command.)=====";
...
If you are currently using 'clfsc push action' command, try to add the relevant authority using -p option.)=====";
const char* error_advice_irrelevant_auth_exception =  "Please remove the unnecessary authority from your action!";

const char* error_advice_missing_chain_api_plugin_exception =  "Ensure that you have \033[2mfscio::chain_api_plugin\033[0m\033[32m added to your node's configuration!";
const char* error_advice_missing_wallet_api_plugin_exception =  "Ensure that you have \033[2mfscio::wallet_api_plugin\033[0m\033[32m added to your node's configuration!\n"\
                                    "Otherwise specify your wallet location with \033[2m--wallet-url\033[0m\033[32m argument!";
const char* error_advice_missing_history_api_plugin_exception =  "Ensure that you have \033[2mfscio::history_api_plugin\033[0m\033[32m added to your node's configuration!";
const char* error_advice_missing_net_api_plugin_exception =  "Ensure that you have \033[2mfscio::net_api_plugin\033[0m\033[32m added to your node's configuration!";
...
bool print_recognized_errors(const fc::exception& e, const bool verbose_errors) {
   // fsc recognized error code is from 3000000
   // refer to libraries/chain/include/fscio/chain/exceptions.hpp
