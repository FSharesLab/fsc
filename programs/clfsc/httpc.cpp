...
namespace fscio { namespace client { namespace http {
   ...
   std::string do_txrx(T& socket, boost::asio::streambuf& request_buff, unsigned int& status_code) {
      ...
      FSC_ASSERT( status_code != 400, invalid_http_request, "The server has rejected the request as invalid!");

      std::string status_message;
      std::getline(response_stream, status_message);
      FSC_ASSERT( !(!response_stream || http_version.substr(0, 5) != "HTTP/"), invalid_http_response, "Invalid Response" );
      ...
      FSC_ASSERT(response_content_length >= 0, invalid_http_response, "Invalid content-length response");
   ...
   parsed_url parse_url( const string& server_url ) {
      ...
      if(res.scheme != "http" && res.scheme != "https")
         FSC_THROW(fail_to_resolve_host, "Unrecognized URL scheme (${s}) in URL \"${u}\"", ("s", res.scheme)("u", server_url));
      if(res.server.empty())
         FSC_THROW(fail_to_resolve_host, "No server parsed from URL \"${u}\"", ("u", server_url));
      if(res.port.empty())
   ...
   resolved_url resolve_url( const http_context& context, const parsed_url& url ) {
      ...
      if (ec) {
         FSC_THROW(fail_to_resolve_host, "Error resolving \"${server}:${port}\" : ${m}", ("server", url.server)("port",url.port)("m",ec.message()));
      }
      ...
      for(const auto& r : result) {
         ...
         if (resolved_port) {
            FSC_ASSERT(*resolved_port == port, resolved_to_multiple_ports, "Service name \"${port}\" resolved to multiple ports and this is not supported!", ("port",url.port));
         } else {
            resolved_port = port;
         }
      }
   ...
   if( status_code == 200 || status_code == 201 || status_code == 202 ) {
      ...
      auto &&error_info = response_result.as<fscio::error_results>().error;
   ...
   FSC_ASSERT( status_code == 200, http_request_fail, "Error code ${c}\n: ${msg}\n", ("c", status_code)("msg", re) );
   return response_result;
   }
}}}
