#!/bin/bash
#
# fscio-tn_roll is used to have all of the instances of the FSC daemon on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: fscio-tn_roll.sh [arglist]
...
# the command line.
#
# In most cases, simply running ./fscio-tn_roll.sh is sufficient.
#

if [ -z "$FSCIO_HOME" ]; then
    echo FSCIO_HOME not set - $0 unable to proceed.
    exit -1
fi
...
prog=""
RD=""
for p in fscd fsciod nodfsc; do
    prog=$p
    RD=bin
    if [ -f $RD/$prog ]; then
        break;
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for fscd or fsciod or nodfsc
    exit 1
fi
...
if [ $DD = "all" ]; then
    for FSCIO_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $FSCIO_HOME/scripts/fscio-tn_up.sh "$*"
    done
else
    bash $FSCIO_HOME/scripts/fscio-tn_up.sh "$*"
fi
unset FSCIO_RESTART_DATA_DIR
