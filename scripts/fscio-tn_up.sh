#!/bin/bash
#
# fscio-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# fscio-tn_bounce.sh and fscio-tn_roll.sh scripts.

connected="0"

rundir=programs/nodfsc
prog=nodfsc
...
datadir=var/lib/node_$FSCIO_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/fscio/node_$FSCIO_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/fscio/node_$FSCIO_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
