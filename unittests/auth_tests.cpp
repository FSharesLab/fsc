...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for fscio reqauth action with alice's spending key
   chain.link_authority("alice", "fscio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "fscio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with fscio reqauth
   chain.unlink_authority("alice", "fscio", "reqauth");
   ...
   // Link authority for any fscio action with alice's scud key
   chain.link_authority("alice", "fscio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "fscio", "first",  "reqauth");
...
try {
   ...
   // Creating account with fscio. prefix with privileged account
   chain.create_account("fscio.test1");

   // Creating account with fscio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("fscio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'fscio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "fscio", "fscio.any", "reqauth" );
   chain.link_authority( "bob", "fscio", "fscio.any", "reqauth" );

   /// this should succeed because fscio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = fscio::chain::resource_limits::resource_usage_object;
   using by_owner = fscio::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link fscio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };
