...
BOOST_AUTO_TEST_CASE( tic_tac_toe_game ) try {
   ...
   BOOST_CHECK_EXCEPTION(chain.push_action(N(tic.tac.toe), N(move), N(player1), mutable_variant_object()
         ...
         ("row", 0)
         ("column", 1)
      ), fscio_assert_message_exception, fscio_assert_message_starts_with("it's not your turn yet"));
   ...
   BOOST_CHECK_EXCEPTION(chain.push_action(N(tic.tac.toe), N(move), N(player2), mutable_variant_object()
         ...
         ("row", 1)
         ("column", 1)
      ), fscio_assert_message_exception, fscio_assert_message_starts_with("not a valid movement"));
   ...
   BOOST_CHECK_EXCEPTION(chain.push_action(N(tic.tac.toe), N(move), N(player2), mutable_variant_object()
         ...
         ("row", 2)
         ("column", 0)
      ), fscio_assert_message_exception, fscio_assert_message_starts_with("the game has ended"));
   ...
   BOOST_CHECK_EXCEPTION(chain.push_action(N(tic.tac.toe), N(move), N(player2), mutable_variant_object()
         ...
         ("row", 2)
         ("column", 0)
      ), fscio_assert_message_exception, fscio_assert_message_starts_with("game doesn't exists"));
