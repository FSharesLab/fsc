...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(fscio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(fscio.token), fscio_token_wast );
   c.set_abi( N(fscio.token), fscio_token_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(fscio.token), N(create), N(fscio.token), mutable_variant_object()
              ("issuer",       "fscio" )
   ...
   cr = c.push_action( N(fscio.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "dan" )
