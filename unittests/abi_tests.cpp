...
auto get_resolver(const abi_def& abi = abi_def())
{
   return [&abi](const account_name &name) -> optional<abi_serializer> {
      return abi_serializer(fscio_contract_abi(abi), max_serialization_time);
   };
}
...
{ try {
   ...
   abi_serializer abis(fscio_contract_abi(abi), max_serialization_time);
...
{ try {
   ...
   auto abi = fscio_contract_abi(fc::json::from_string(my_abi).as<abi_def>());

   abi_serializer abis(abi, max_serialization_time);

   const char *my_other = R"=====(
    {
      "publickey"     :  "FSC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
      "publickey_arr" :  ["FSC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV","FSC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV","FSC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"],
      "asset"         : "100.0000 SYS",
      ...
      "keyweight": {"key":"FSC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"100"},
      "keyweight_arr": [{"key":"FSC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"100"},{"key":"FSC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"200"}],
      "authority": {
         "threshold":"10",
         "keys":[{"key":"FSC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":100},{"key":"FSC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":200}],
...
{ try {
   ...
   auto abi = fscio_contract_abi(fc::json::from_string(typedef_cycle_abi).as<abi_def>());
...
{ try {
   ...
   abi_serializer abis(fscio_contract_abi(abi_def()), max_serialization_time);
...
{ try {
   ...
   const char* test_data = R"=====(
   {
     ...
     "auth" : {
        ...
        "keys" : [ {"key" : "FSC65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im", "weight" : 57005},
                   {"key" : "FSC5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf", "weight" : 57605} ],
   ...
   BOOST_TEST("FSC65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == (std::string)updauth.auth.keys[0].key);
   BOOST_TEST(57005u == updauth.auth.keys[0].weight);
   BOOST_TEST("FSC5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == (std::string)updauth.auth.keys[1].key);
...
{ try {
   ...
   BOOST_TEST("FSC65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == (std::string)newacct.owner.keys[0].key);
   BOOST_TEST(57005u == newacct.owner.keys[0].weight);
   BOOST_TEST("FSC5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == (std::string)newacct.owner.keys[1].key);
   ...
   BOOST_TEST("FSC65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == (std::string)newacct.active.keys[0].key);
   BOOST_TEST(57005u == newacct.active.keys[0].weight);
   BOOST_TEST("FSC5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == (std::string)newacct.active.keys[1].key);
...
{ try {
   ...
   abi_serializer abis(fscio_contract_abi(v.as<abi_def>()), max_serialization_time);
...
{ try {
   ...
   auto abi = fscio_contract_abi(fc::json::from_string(repeat_abi).as<abi_def>());
...
BOOST_AUTO_TEST_CASE(abi_account_name_in_fscio_abi)
{ try {
   // inifinite loop in types
   const char* repeat_abi = R"=====(
   {
...
BOOST_AUTO_TEST_CASE(abi_large_array)
{
   try {
      ...
      // curl http://127.0.0.1:8888/v1/chain/abi_bin_to_json -X POST -d '{"code":"fscio", "action":"hi", "binargs":"ffffffff08"}'
...
{
   using fscio::testing::fc_exception_message_starts_with;

   auto duplicate_variant_abi = R"({
      "version": "fscio::abi/1.1",
      "variants": [
...
BOOST_AUTO_TEST_CASE(version)
{
   try {
      ...
      BOOST_CHECK_THROW( abi_serializer(fc::json::from_string(R"({"version": "fscio::abi/9.0"})").as<abi_def>(), max_serialization_time), unsupported_abi_version_exception );
      abi_serializer(fc::json::from_string(R"({"version": "fscio::abi/1.0"})").as<abi_def>(), max_serialization_time);
      abi_serializer(fc::json::from_string(R"({"version": "fscio::abi/1.1"})").as<abi_def>(), max_serialization_time);
   } FC_LOG_AND_RETHROW()
}
...
{
   using fscio::testing::fc_exception_message_is;
