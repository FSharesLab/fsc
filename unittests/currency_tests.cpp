...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance(N(fscio.token), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(fscio.token), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
      :TESTER(),abi_ser(json::from_string(fscio_token_abi).as<abi_def>(), abi_serializer_max_time)
      {
         create_account( N(fscio.token));
         set_code( N(fscio.token), fscio_token_wast );

         auto result = push_action(N(fscio.token), N(create), mutable_variant_object()
                 ("issuer",       fscio_token)
         ...
         result = push_action(N(fscio.token), N(issue), mutable_variant_object()
                 ("to",       fscio_token)
      ...
      static const std::string fscio_token;
};

const std::string currency_tester::fscio_token = name(N(fscio.token)).to_string();
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(fscio.token), expected.get_symbol(), N(fscio.token));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(fscio.token), N(transfer), mutable_variant_object()
         ("from", fscio_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(fscio.token), N(transfer), mutable_variant_object()
                                    ("from", fscio_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE( test_overspend, currency_tester ) try {
   ...
   // Overspend!
   {
      ...
      BOOST_CHECK_EXCEPTION( push_action(N(alice), N(transfer), data),
                             fscio_assert_message_exception, fscio_assert_message_is("overdrawn balance") );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE( test_input_quantity, currency_tester ) try {
   ...
   {
      auto trace = transfer(fscio_token, N(alice), "100.0000 CUR");
   ...
   {
      BOOST_REQUIRE_THROW(transfer(N(alice), N(carl), "20.50 USD"), fscio_assert_message_exception);
   }
