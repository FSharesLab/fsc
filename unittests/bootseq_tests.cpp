...
#include <fscio.msig/fscio.msig.wast.hpp>
#include <fscio.msig/fscio.msig.abi.hpp>
...
public:
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "fscio_global_state", data, abi_serializer_max_time );
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(fscio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create fscio.msig and fscio.token
        create_accounts({N(fscio.msig), N(fscio.token), N(fscio.ram), N(fscio.ramfee), N(fscio.stake), N(fscio.vpay), N(fscio.bpay), N(fscio.saving) });

        // Set code for the following accounts:
        //  - fscio (code: fscio.bios) (already set by tester constructor)
        //  - fscio.msig (code: fscio.msig)
        //  - fscio.token (code: fscio.token)
        set_code_abi(N(fscio.msig), fscio_msig_wast, fscio_msig_abi);//, &fscio_active_pk);
        set_code_abi(N(fscio.token), fscio_token_wast, fscio_token_abi); //, &fscio_active_pk);

        // Set privileged for fscio.msig and fscio.token
        set_privileged(N(fscio.msig));
        set_privileged(N(fscio.token));

        // Verify fscio.msig and fscio.token is privileged
        const auto& fscio_msig_acc = get<account_object, by_name>(N(fscio.msig));
        BOOST_TEST(fscio_msig_acc.privileged == true);
        const auto& fscio_token_acc = get<account_object, by_name>(N(fscio.token));
        BOOST_TEST(fscio_token_acc.privileged == true);


        // Create SYS tokens in fscio.token, set its manager as fscio
        ...
        create_currency(N(fscio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to fscio.system
        issue(N(fscio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        // Set fscio.system to fscio
        set_code_abi(config::system_account_name, fscio_system_wast, fscio_system_abi);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(fscio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "fscio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), fscio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), fscio_assert_message_exception);
