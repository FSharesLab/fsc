...
BOOST_FIXTURE_TEST_CASE( basic_test, TESTER ) try {
   ...
   transaction_id_type yes_assert_id;
   {
      ...
      BOOST_CHECK_THROW(push_transaction( trx ), fscio_assert_message_exception);
   }
...
BOOST_FIXTURE_TEST_CASE( f32_f64_overflow_tests, tester ) try {
   ...
   auto check = [&](const char *wast_template, const char *op, const char *param) -> bool {
      ...
      try {
         ...
         const auto& receipt = get_transaction_receipt(trx.id());
         return true;
      } catch (fscio::chain::wasm_execution_error &) {
         return false;
      }
   };
   //
...
(module
  ...
  (import "env" "fscio_assert" (func $fscio_assert (param i32 i32)))
   (table 0 anyfunc)
   (memory $0 1)
   ...
   while (count < 4) {
      ...
      try {
         ...
         pass = true;
         count++;
      } catch( fscio::chain::leeway_deadline_exception& ) {
...
BOOST_FIXTURE_TEST_CASE( big_memory, TESTER ) try {
   ...
   string biggest_memory_wast_f = fc::format_string(biggest_memory_wast, fc::mutable_variant_object(
                                          "MAX_WASM_PAGES", fscio::chain::wasm_constraints::maximum_linear_memory/(64*1024)));
   ...
   string too_big_memory_wast_f = fc::format_string(too_big_memory_wast, fc::mutable_variant_object(
                                          "MAX_WASM_PAGES_PLUS_ONE", fscio::chain::wasm_constraints::maximum_linear_memory/(64*1024)+1));
   BOOST_CHECK_THROW(set_code(N(bigmem), too_big_memory_wast_f.c_str()), fscio::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( table_init_tests, TESTER ) try {
   ...
   BOOST_CHECK_THROW(set_code(N(tableinit), too_big_table), fscio::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( memory_init_border, TESTER ) try {
   ...
   BOOST_CHECK_THROW(set_code(N(memoryborder), memory_init_toolong), fscio::chain::wasm_execution_error);
   BOOST_CHECK_THROW(set_code(N(memoryborder), memory_init_negative), fscio::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( nested_limit_test, TESTER ) try {
   ...
   }
   {
      ...
      for(unsigned int i = 0; i < 1024; ++i)
         ss << ")";
      ss << "))";
      BOOST_CHECK_THROW(set_code(N(nested), ss.str().c_str()), fscio::chain::wasm_execution_error);
   }
...
BOOST_FIXTURE_TEST_CASE( lotso_globals, TESTER ) try {
   ...
   .c_str()), fscio::chain::wasm_execution_error);
...
BOOST_FIXTURE_TEST_CASE( offset_check, TESTER ) try {
   ...
   for(const string& s : loadops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << fscio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(drop (" << s << " offset=" << fscio::chain::wasm_constraints::maximum_linear_memory-2 << " (i32.const 0)))";
   ...
   for(const vector<string>& o : storeops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << fscio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(" << o[0] << " offset=" << fscio::chain::wasm_constraints::maximum_linear_memory-2 << " (i32.const 0) (" << o[1] << ".const 0))";
      ss << ") (export \"apply\" (func $apply)) )";
   ...
   for(const string& s : loadops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << fscio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(drop (" << s << " offset=" << fscio::chain::wasm_constraints::maximum_linear_memory+4 << " (i32.const 0)))";
      ss << ") (export \"apply\" (func $apply)) )";

      BOOST_CHECK_THROW(set_code(N(offsets), ss.str().c_str()), fscio::chain::wasm_execution_error);
      produce_block();
   }
   for(const vector<string>& o : storeops) {
      std::stringstream ss;
      ss << "(module (memory $0 " << fscio::chain::wasm_constraints::maximum_linear_memory/(64*1024) << ") (func $apply (param $0 i64) (param $1 i64) (param $2 i64)";
      ss << "(" << o[0] << " offset=" << fscio::chain::wasm_constraints::maximum_linear_memory+4 << " (i32.const 0) (" << o[1] << ".const 0))";
      ss << ") (export \"apply\" (func $apply)) )";
...
// abi_serializer::to_variant failed because fscio_system_abi modified via set_abi.
// This test also verifies that chain_initializer::fsc_contract_abi() does not conflict
// with fscio_system_abi as they are not allowed to contain duplicates.
BOOST_FIXTURE_TEST_CASE(fscio_abi, TESTER) try {
   produce_blocks(2);
   ...
   // verify to_variant works on fsc native contract type: newaccount
...
BOOST_FIXTURE_TEST_CASE( check_table_maximum, TESTER ) try {
   ...
   BOOST_CHECK_THROW(push_transaction(trx), fscio::chain::wasm_execution_error);
   }
   ...
   signed_transaction trx;
   action act;
   act.name = fscio::chain::wasm_constraints::maximum_table_elements+54334;
   act.account = N(tbl);
   act.authorization = vector<permission_level>{{N(tbl),config::active_name}};
   trx.actions.push_back(act);
      set_transaction_headers(trx);
   ...
   BOOST_CHECK_EXCEPTION(push_transaction(trx), fscio::chain::wasm_execution_error, [](const fscio::chain::wasm_execution_error &e) {return true;});
   }
} FC_LOG_AND_RETHROW()
...
BOOST_FIXTURE_TEST_CASE(net_usage_tests, tester ) try {
   ...
   (import "env" "fscio_assert" (func $fscio_assert (param i32 i32)))
      ...
      produce_blocks(1);
      signed_transaction trx;
      auto wasm = ::fscio::chain::wast_to_wasm(code);
