...
BOOST_AUTO_TEST_SUITE(database_tests)
   ...
   BOOST_AUTO_TEST_CASE(undo_test) {
      try {
         ...
         fscio::chain::database& db = const_cast<fscio::chain::database&>( test.control->db() );
   ...
   BOOST_AUTO_TEST_CASE(get_blocks) {
      try {
         ...
         auto calc_exp_last_irr_block_num = [&](uint32_t head_block_num) -> uint32_t {
            ...
            const auto max_reversible_rounds = FSC_PERCENT(producers_size, config::percent_100 - config::irreversible_threshold_percent);
