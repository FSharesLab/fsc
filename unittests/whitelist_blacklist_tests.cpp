...
#include <fscio/testing/tester_network.hpp>
...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(fscio.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(fscio.token), fscio_token_wast);
         chain->set_abi(N(fscio.token), fscio_token_abi);
         chain->push_action( N(fscio.token), N(create), N(fscio.token), mvo()
              ( "issuer", "fscio.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(fscio.token), N(issue), N(fscio.token), mvo()
              ( "to", "fscio.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(fscio.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(fscio.token), N(alice)};
   test.init();

   test.transfer( N(fscio.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(fscio.token), N(bob)};
   test.init();

   test.transfer( N(fscio.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(fscio.token) );
   ...
   test.chain->set_code(N(bob), fscio_token_wast);
   test.chain->set_abi(N(bob), fscio_token_abi);

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), fscio_token_wast);
   test.chain->set_abi(N(charlie), fscio_token_abi);
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(fscio.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_fscio ) { try {
   whitelist_blacklist_tester<tester> tester1;
   tester1.init();
   tester1.chain->produce_blocks();
   tester1.chain->set_code(config::system_account_name, fscio_token_wast);
   tester1.chain->produce_blocks();
   tester1.shutdown();
...
BOOST_AUTO_TEST_CASE( blacklist_onerror ) { try {
   ...
   BOOST_CHECK_EXCEPTION( tester1.chain->produce_blocks(), fc::exception,
                          fc_exception_message_is("action 'fscio::onerror' is on the action blacklist")
                        );
...
BOOST_AUTO_TEST_CASE( actor_blacklist_inline_deferred ) { try {
   ...
   auto auth = authority(fscio::testing::base_tester::get_public_key("alice", "active"));
   auth.accounts.push_back( permission_level_weight{{N(alice), config::fscio_code_name}, 1} );

   tester1.chain->push_action( N(fscio), N(updateauth), N(alice), mvo()
      ( "account", "alice" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auth = authority(fscio::testing::base_tester::get_public_key("bob", "active"));
   auth.accounts.push_back( permission_level_weight{{N(alice), config::fscio_code_name}, 1} );
   auth.accounts.push_back( permission_level_weight{{N(bob), config::fscio_code_name}, 1} );

   tester1.chain->push_action( N(fscio), N(updateauth), N(bob), mvo()
      ( "account", "bob" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auth = authority(fscio::testing::base_tester::get_public_key("charlie", "active"));
   auth.accounts.push_back( permission_level_weight{{N(charlie), config::fscio_code_name}, 1} );

   tester1.chain->push_action( N(fscio), N(updateauth), N(charlie), mvo()
      ( "account", "charlie" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auto log_trxs = [&]( const transaction_trace_ptr& t) {
      ...
      if( act.account == N(fscio) && act.name == N(onblock) ) return;
