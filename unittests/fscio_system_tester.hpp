...
namespace fscio_system {

class fscio_system_tester : public TESTER {
public:

   fscio_system_tester()
   : fscio_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   fscio_system_tester(Lambda setup) {
      setup(*this);
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(fscio.token) );
         abi_def abi;
         BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
         token_abi_ser.set_abi(abi, abi_serializer_max_time);
      }

      create_currency( N(fscio.token), config::system_account_name, core_from_string("10000000000.0000") );
      issue(config::system_account_name,      core_from_string("1000000000.0000"));
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( "fscio" ) );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance("fscio")  + get_balance("fscio.ramfee") + get_balance("fscio.stake") + get_balance("fscio.ram") );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(fscio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(fscio.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(fscio.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = fscio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(fscio.token), symbol_code, N(stat), symbol_code );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(fscio.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "fscio", "fscio.msig", core_from_string("5000.0000") ) );
         produce_block();
         ...
         set_code( N(fscio.msig), fscio_msig_wast );
         set_abi( N(fscio.msig), fscio_msig_abi );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( N(fscio.msig) );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      //stake more than 15% of total FSC supply to activate chain
      transfer( "fscio", "alice1111111", core_from_string("650000000.0000"), "fscio" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::fscio_code_name}, 1},
...
inline uint64_t M( const string& fsc_str ) {
   return core_from_string( fsc_str ).get_amount();
}
