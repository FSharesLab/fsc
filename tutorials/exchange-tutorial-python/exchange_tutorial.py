...
def monitor_exchange():
    ...
    results = clfsc('get actions tokenxchange {} 0 -j'.format(action_num))
...
def transfer(to, quantity):
    if quantity[:-4] != ' SYS':
        quantity += ' SYS'
    results = clfsc('transfer tokenxchange {} "{}" {} -j'.format(to, quantity, KEY_TO_INTERNAL_ACCOUNT))
