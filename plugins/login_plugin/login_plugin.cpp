...
#include <fscio/login_plugin/login_plugin.hpp>
...
login_plugin::start_login_request(const login_plugin::start_login_request_params& params) {
   my->expire_requests();
   FSC_ASSERT(params.expiration_time > fc::time_point::now(), fc::timeout_exception,
              "Requested expiration time ${expiration_time} is in the past",
              ("expiration_time", params.expiration_time));
   FSC_ASSERT(my->requests.size() < my->max_login_requests, fc::timeout_exception, "Too many pending login requests");
