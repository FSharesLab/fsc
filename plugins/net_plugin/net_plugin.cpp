...
#include <fscio/net_plugin/net_plugin.hpp>
#include <fscio/net_plugin/protocol.hpp>
...
using namespace fscio::chain::plugin_interface::compat;
...
namespace fscio {
   ...
   using fscio::chain::transaction_id_type;
...
#define peer_elog( PEER, FORMAT, ... ) \
  ...
  FC_MULTILINE_MACRO_END
   ...
   struct update_request_time {
      void operator() (struct fscio::peer_block_state &bs) {
         bs.requested_time = time_point::now();
      }
   } set_request_time;

   typedef multi_index_container<
      fscio::peer_block_state,
      indexed_by<
         ordered_unique< tag<by_id>, member<fscio::peer_block_state, block_id_type, &fscio::peer_block_state::id >, sha256_less >,
         ordered_unique< tag<by_block_num>, member<fscio::peer_block_state, uint32_t, &fscio::peer_block_state::block_num > >
         >
      > peer_block_state_index;


   struct update_known_by_peer {
      void operator() (fscio::peer_block_state& bs) {
         bs.is_known = true;
      }
   } set_is_known;
   ...
   public:
      ...
      void fill_out_buffer( std::vector<boost::asio::const_buffer>& bufs ) {
         if( _sync_write_queue.size() > 0 ) { // always send msgs from sync_write_queue first
            ...
            FSC_ASSERT( _write_queue_size == 0, plugin_exception, "write queue size expected to be zero" );
         }
      }
   ...
   struct msg_handler : public fc::visitor<void> {
      ...
      void operator()( const signed_block& msg ) const {
         FSC_ASSERT( false, plugin_config_exception, "operator()(signed_block&&) should be called" );
      }
      void operator()( signed_block& msg ) const {
         FSC_ASSERT( false, plugin_config_exception, "operator()(signed_block&&) should be called" );
      }
      void operator()( const packed_transaction& msg ) const {
         FSC_ASSERT( false, plugin_config_exception, "operator()(packed_transaction&&) should be called" );
      }
            ...
            [this,weak_conn]( boost::system::error_code ec, std::size_t bytes_transferred ) {
               ...
               try {
                  if( !ec ) {
                     ...
                     FSC_ASSERT(bytes_transferred <= conn->pending_message_buffer.bytes_to_write(), plugin_exception, "");
   ...
   void net_plugin_impl::handle_message(const connection_ptr& c, const packed_transaction_ptr& trx) {
      ...
      controller& cc = my_impl->chain_plug->chain();
      if( cc.get_read_mode() == fscio::db_read_mode::READ_ONLY ) {
   ...
   void net_plugin::set_program_options( options_description& /*cli*/, options_description& cfg )
   {
      cfg.add_options()
         ...
         ( "agent-name", bpo::value<string>()->default_value("\"FSC Test Agent\""), "The name supplied to identify this node amongst the peers.")
   ...
   void net_plugin::plugin_initialize( const variables_map& options ) {
      ...
      try {
         ...
         if( my->allowed_connections & net_plugin_impl::Specified )
            FSC_ASSERT( options.count( "peer-key" ),
         ...
         FSC_ASSERT( my->chain_plug, chain::missing_chain_plugin_exception, ""  );
