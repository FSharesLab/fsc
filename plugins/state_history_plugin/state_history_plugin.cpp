...
#include <fscio/state_history_plugin/state_history_log.hpp>
#include <fscio/state_history_plugin/state_history_serialization.hpp>
...
struct state_history_plugin_impl : std::enable_shared_from_this<state_history_plugin_impl> {
   ...
   void listen() {
      ...
      auto check_ec = [&](const char* what) {
         if (!ec)
            return;
         elog("${w}: ${m}", ("w", what)("m", ec.message()));
         FSC_ASSERT(false, plugin_exception, "unable to open listen socket");
      };
   ...
   static bool is_onblock(const transaction_trace_ptr& p) {
      ...
      if (act.account != fscio::chain::config::system_account_name || act.name != N(onblock) ||
          act.authorization.size() != 1)
         return false;
      auto& auth = act.authorization[0];
      return auth.actor == fscio::chain::config::system_account_name &&
             auth.permission == fscio::chain::config::active_name;
   }
   ...
   void store_traces(const block_state_ptr& block_state) {
      ...
      for (auto& r : block_state->block->transactions) {
         ...
         FSC_ASSERT(it != cached_traces.end() && it->second->receipt, plugin_exception,
      ...
      FSC_ASSERT(traces_bin.size() == (uint32_t)traces_bin.size(), plugin_exception, "traces is too big");
   ...
   void store_chain_state(const block_state_ptr& block_state) {
      ...
      auto get_table_id = [&](uint64_t tid) -> const table_id_object& {
         ...
         FSC_ASSERT(it != removed_table_id.end(), chain::plugin_exception, "can not found table id ${tid}",
                    ("tid", tid));
         return *it->second;
      };
      ...
      FSC_ASSERT(deltas_bin.size() == (uint32_t)deltas_bin.size(), plugin_exception, "deltas is too big");
      state_history_log_header header{.block_num    = block_state->block->block_num(),
                                      .block_id     = block_state->block->id(),
...
void state_history_plugin::plugin_initialize(const variables_map& options) {
   try {
      FSC_ASSERT(options.at("disable-replay-opts").as<bool>(), plugin_exception,
