...
class state_history_log {
 ...
 public:
   ...
   void write_entry(const state_history_log_header& header, const chain::block_id_type& prev_id, F write_payload) {
      FSC_ASSERT(_begin_block == _end_block || header.block_num <= _end_block, chain::plugin_exception,
      ...
      if (_begin_block != _end_block && header.block_num > _begin_block) {
         if (header.block_num == _end_block) {
            FSC_ASSERT(prev_id == last_block_id, chain::plugin_exception, "missed a fork change in ${name}.log",
                       ("name", name));
         } else {
            state_history_log_header prev;
            get_entry(header.block_num - 1, prev);
            FSC_ASSERT(prev_id == prev.block_id, chain::plugin_exception, "missed a fork change in ${name}.log",
                       ("name", name));
         }
      }
      ...
      FSC_ASSERT(end == pos + sizeof(header) + header.payload_size, chain::plugin_exception,
   ...
   std::fstream& get_entry(uint32_t block_num, state_history_log_header& header) {
      FSC_ASSERT(block_num >= _begin_block && block_num < _end_block, chain::plugin_exception,
 ...
 private:
   ...
   void recover_blocks(uint64_t size) {
      ...
      log.sync();
      FSC_ASSERT(get_last_block(pos), chain::plugin_exception, "recover ${name}.log failed", ("name", name));
   }
   ...
   void open_log() {
      ...
      if (size >= sizeof(state_history_log_header)) {
         ...
         log.seekg(0);
         log.read((char*)&header, sizeof(header));
         FSC_ASSERT(header.version == 0 && sizeof(header) + header.payload_size + sizeof(uint64_t) <= size,
         ...
         FSC_ASSERT(!size, chain::plugin_exception, "corrupt ${name}.log (5)", ("name", name));
   ...
   void open_index() {
      ...
      while (pos < size) {
         state_history_log_header header;
         FSC_ASSERT(pos + sizeof(header) <= size, chain::plugin_exception, "corrupt ${name}.log (6)", ("name", name));
         log.seekg(pos);
         log.read((char*)&header, sizeof(header));
         uint64_t suffix_pos = pos + sizeof(header) + header.payload_size;
         uint64_t suffix;
         FSC_ASSERT(suffix_pos + sizeof(suffix) <= size, chain::plugin_exception, "corrupt ${name}.log (7)",
                    ("name", name));
         ...
         FSC_ASSERT(suffix == pos, chain::plugin_exception, "corrupt ${name}.log (8)", ("name", name));
