#include <fscio/history_plugin/history_plugin.hpp>
#include <fscio/history_plugin/account_control_history_object.hpp>
#include <fscio/history_plugin/public_key_history_object.hpp>
...
CHAINBASE_SET_INDEX_TYPE(fscio::account_history_object, fscio::account_history_index)
CHAINBASE_SET_INDEX_TYPE(fscio::action_history_object, fscio::action_history_index)
...
namespace fscio {
      ...
      try {
         if( options.count( "filter-on" )) {
            ...
            for( auto& s : fo ) {
               if( s == "*" || s == "\"*\"" ) {
                  ...
                  wlog( "--filter-on * enabled. This can fill shared_mem, causing nodfsc to stop." );
                  break;
               }
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               FSC_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-on", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
               FSC_ASSERT( fe.receiver.value, fc::invalid_arg_exception,
         ...
         if( options.count( "filter-out" )) {
            ...
            for( auto& s : fo ) {
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               FSC_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-out", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
               FSC_ASSERT( fe.receiver.value, fc::invalid_arg_exception,
   ...
   namespace history_apis {
      read_only::get_actions_result read_only::get_actions( const read_only::get_actions_params& params )const {
        ...
        FSC_ASSERT( end >= start, chain::plugin_exception, "end position is earlier than start position" );
      ...
      read_only::get_transaction_result read_only::get_transaction( const read_only::get_transaction_params& p )const {
         ...
         } FSC_RETHROW_EXCEPTIONS(transaction_id_type_exception, "Invalid transaction ID: ${transaction_id}", ("transaction_id", p.id))
         ...
         if( !in_history && !p.block_num_hint ) {
            FSC_THROW(tx_not_found, "Transaction ${id} not found in history and no block hint was given", ("id",p.id));
         }
         ...
         if( in_history ) {
            ...
            if (!found) {
               FSC_THROW(tx_not_found, "Transaction ${id} not found in history or in block number ${n}", ("id",p.id)("n", *p.block_num_hint));
            }
         }
