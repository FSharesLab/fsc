 ...
 */
#include <fscio/wallet_plugin/wallet_manager.hpp>
#include <fscio/wallet_plugin/wallet.hpp>
#include <fscio/wallet_plugin/se_wallet.hpp>
...
void wallet_manager::set_timeout(const std::chrono::seconds& t) {
   ...
   FSC_ASSERT(timeout_time >= now && timeout_time.time_since_epoch().count() > 0, invalid_lock_timeout_exception, "Overflow on timeout_time, specified ${t}, now ${now}, timeout_time ${timeout_time}",
...
std::string wallet_manager::create(const std::string& name) {
   ...
   FSC_ASSERT(valid_filename(name), wallet_exception, "Invalid filename, path not allowed in wallet name ${n}", ("n", name));

   auto wallet_filename = dir / (name + file_ext);

   if (fc::exists(wallet_filename)) {
      FSC_THROW(chain::wallet_exist_exception, "Wallet with name: '${n}' already exists at ${path}", ("n", name)("path",fc::path(wallet_filename)));
   }
   ...
   // This can happen if the wallet file is removed while fsc-walletd is running.
...
void wallet_manager::open(const std::string& name) {
   ...
   if (!wallet->load_wallet_file()) {
      FSC_THROW(chain::wallet_nonexistent_exception, "Unable to open file: ${f}", ("f", wallet_filename.string()));
   }

   // If we have name in our map then remove it since we want the emplace below to replace.
   // This can happen if the wallet file is added while fsc-walletd is running.
   auto it = wallets.find(name);
   if (it != wallets.end()) {
      wallets.erase(it);
   }
...
map<public_key_type,private_key_type> wallet_manager::list_keys(const string& name, const string& pw) {
   ...
   if (wallets.count(name) == 0)
      FSC_THROW(chain::wallet_nonexistent_exception, "Wallet not found: ${w}", ("w", name));
   auto& w = wallets.at(name);
   if (w->is_locked())
      FSC_THROW(chain::wallet_locked_exception, "Wallet is locked: ${w}", ("w", name));
...
flat_set<public_key_type> wallet_manager::get_public_keys() {
   check_timeout();
   FSC_ASSERT(!wallets.empty(), wallet_not_available_exception, "You don't have any wallet!");
   ...
   FSC_ASSERT(!is_all_wallet_locked, wallet_locked_exception, "You don't have any unlocked wallet!");
   return result;
}
...
void wallet_manager::unlock(const std::string& name, const std::string& password) {
   ...
   auto& w = wallets.at(name);
   if (!w->is_locked()) {
      FSC_THROW(chain::wallet_unlocked_exception, "Wallet is already unlocked: ${w}", ("w", name));
      return;
   }
   w->unlock(password);
}
...
wallet_manager::sign_transaction(const chain::signed_transaction& txn, const flat_set<public_key_type>& keys, const chain::chain_id_type& id) {
   ...
   for (const auto& pk : keys) {
      ...
      if (!found) {
         FSC_THROW(chain::wallet_missing_pub_key_exception, "Public key not found in unlocked wallets ${k}", ("k", pk));
      }
   }
...
wallet_manager::sign_digest(const chain::digest_type& digest, const public_key_type& key) {
   ...
   FSC_THROW(chain::wallet_missing_pub_key_exception, "Public key not found in unlocked wallets ${k}", ("k", key));
}
...
void wallet_manager::initialize_lock() {
   //This is technically somewhat racy in here -- if multiple kfscd are in this function at once.
   ...
   lock_path = dir / "wallet.lock";
   {
      ...
      FSC_ASSERT(!x.fail(), wallet_exception, "Failed to open wallet lock file at ${f}", ("f", lock_path.string()));
   }
   ...
   if(!wallet_dir_lock->try_lock()) {
      ...
      FSC_THROW(wallet_exception, "Failed to lock access to wallet directory; is another kfscd running?");
   }
}
