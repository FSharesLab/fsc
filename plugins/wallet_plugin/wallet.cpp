...
namespace fscio { namespace wallet {
...
public:
   ...
   private_key_type get_private_key(const public_key_type& id)const
   {
      ...
      FSC_ASSERT( has_key, chain::key_nonexistent_exception, "Key doesn't exist!" );
      return *has_key;
   }
   ...
   bool import_key(string wif_key)
   {
      ...
      fscio::chain::public_key_type wif_pub_key = priv.get_public_key();
      ...
      FSC_THROW( chain::key_exist_exception, "Key already in wallet" );
   }
   ...
   bool remove_key(string key)
   {
      ...
      FSC_THROW( chain::key_nonexistent_exception, "Key not in wallet" );
   }
   ...
   string create_key(string key_type)
   {
      ...
      if(key_type == "K1")
         ...
         FSC_THROW(chain::unsupported_key_type_exception, "Key type \"${kt}\" not supported by software wallet", ("kt", key_type));
   ...
   void save_wallet_file(string wallet_filename = "")
   {
      ...
      try
      {
         ...
         if (!outfile) {
            ...
            FSC_THROW(wallet_exception, "Unable to open file: ${fn}", ("fn", wallet_filename));
         }
...
{
   FSC_ASSERT(!is_locked(), wallet_locked_exception, "Unable to import key on a locked wallet");
...
{
   FSC_ASSERT(!is_locked(), wallet_locked_exception, "Unable to remove key from a locked wallet");
...
{
   FSC_ASSERT(!is_locked(), wallet_locked_exception, "Unable to create key on a locked wallet");
...
{ try {
   FSC_ASSERT( !is_locked(), wallet_locked_exception, "Unable to lock a locked wallet" );
   encrypt_keys();
...
} FSC_RETHROW_EXCEPTIONS(chain::wallet_invalid_password_exception,
...
{
   if( !is_new() )
      FSC_ASSERT( !is_locked(), wallet_locked_exception, "The wallet must be unlocked before the password can be set" );
...
{
   FSC_ASSERT(!is_locked(), wallet_locked_exception, "Unable to list public keys of a locked wallet");
   return my->_keys;
}

flat_set<public_key_type> soft_wallet::list_public_keys() {
   FSC_ASSERT(!is_locked(), wallet_locked_exception, "Unable to list private keys of a locked wallet");
...
pair<public_key_type,private_key_type> soft_wallet::get_private_key_from_password( string account, string role, string password )const {
   ...
   FSC_ASSERT( seed.size(), wallet_exception, "seed should not be empty" );
