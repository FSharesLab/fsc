...
#include <fscio/wallet_plugin/yubihsm.h>
...
struct yubihsm_wallet_impl {
   ...
   public_key_type create() {
      ...
      try {
         if((rc = api.util_generate_key_ec(session, &new_key_id, "kfscd created key", authkey_domains, &creation_caps, YH_ALGO_EC_P256)))
