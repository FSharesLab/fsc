...
#include <fscio/wallet_plugin/macos_user_auth.h>
...
#define STR(A) #A
   ...
   bool remove_key(string public_key) {
      ...
      macos_user_auth(auth_callback, &prom, CFSTR("remove a key from your FSCIO wallet"));
      if(!fut.get())
...
static void check_signed() {
   ...
   if(is_valid != errSecSuccess) {
      ...
      FSC_THROW(secure_enclave_exception, "");
   }
}
...
se_wallet::se_wallet() : my(new detail::se_wallet_impl()) {
   ...
   FSC_THROW(secure_enclave_exception, "Secure Enclave not supported on this hardware");
}
...
void se_wallet::lock() {
   FSC_ASSERT(!is_locked(), wallet_locked_exception, "You can not lock an already locked wallet");
   my->locked = true;
}
...
void se_wallet::unlock(string password) {
   promise<bool> prom;
   future<bool> fut = prom.get_future();
   macos_user_auth(detail::auth_callback, &prom, CFSTR("unlock your FSCIO wallet"));
   if(!fut.get())
      FC_THROW_EXCEPTION(chain::wallet_invalid_password_exception, "Local user authentication failed");
   my->locked = false;
}
...
bool se_wallet::remove_key(string key) {
   FSC_ASSERT(!is_locked(), wallet_locked_exception, "You can not remove a key from a locked wallet");
