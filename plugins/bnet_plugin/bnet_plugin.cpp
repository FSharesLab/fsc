...
#include <fscio/bnet_plugin/bnet_plugin.hpp>
...
using fscio::public_key_type;
using fscio::chain_id_type;
using fscio::block_id_type;
using fscio::block_timestamp_type;
using std::string;
using fscio::sha256;
using fscio::signed_block_ptr;
using fscio::packed_transaction_ptr;
using std::vector;
...
namespace fscio {
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              FSC_THROW(block_validate_exception, "bad block" );
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           FSC_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         FSC_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
   ...
   void session::on( const packed_transaction_ptr& p ) {
      ...
      if (!p) {
        ...
        FSC_THROW(transaction_exception, "bad transaction");
      }
