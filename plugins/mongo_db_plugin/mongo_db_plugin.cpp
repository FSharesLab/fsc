 ...
 */
#include <fscio/mongo_db_plugin/mongo_db_plugin.hpp>
...
void mongo_db_plugin_impl::applied_transaction( const chain::transaction_trace_ptr& t ) {
   try {
      ...
      //
      // Allow logging traces if node is a producer for testing purposes, so a single nodfsc can do both for testing.
      //
         ...
         if(account) {
            ...
            if( view.find( "abi" ) != view.end()) {
               ...
               if( n == chain::config::system_account_name ) {
                  // redefine fscio setabi.abi from bytes to abi_def
                           ...
                           abis.add_specialized_unpack_pack( "abi_def",
                                 std::make_pair<abi_serializer::unpack_function, abi_serializer::pack_function>(
                                       []( fc::datastream<const char*>& stream, bool is_array, bool is_optional ) -> fc::variant {
                                          FSC_ASSERT( !is_array && !is_optional, chain::mongo_db_exception, "unexpected abi_def");
                                          chain::bytes temp;
                                       ...
                                       []( const fc::variant& var, fc::datastream<char*>& ds, bool is_array, bool is_optional ) {
                                          FSC_ASSERT( false, chain::mongo_db_exception, "never called" );
                                       }
                                 ) );
                        }
                     }
...
void mongo_db_plugin_impl::_process_accepted_transaction( const chain::transaction_metadata_ptr& t ) {
   ...
   try {
      ...
      if( !_trans.update_one( make_document( kvp( "trx_id", trx_id_str ) ),
         ...
         FSC_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert trans ${id}", ("id", trx_id) );
      }
   } catch( ... ) {
   ...
   if( write_atraces ) {
      try {
         if( !bulk_action_traces.execute() ) {
            FSC_ASSERT( false, chain::mongo_db_insert_fail,
      ...
      try {
         if( update_blocks_via_block_num ) {
            if( !_block_states.update_one( make_document( kvp( "block_num", b_int32{static_cast<int32_t>(block_num)} ) ),
               ...
               FSC_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block_state ${num}", ("num", block_num) );
            }
         } else {
            if( !_block_states.update_one( make_document( kvp( "block_id", block_id_str ) ),
                                           make_document( kvp( "$set", block_state_doc.view() ) ), update_opts ) ) {
               FSC_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block_state ${bid}", ("bid", block_id) );
            }
         }
      } catch( ... ) {
      ...
      try {
         if( update_blocks_via_block_num ) {
            if( !_blocks.update_one( make_document( kvp( "block_num", b_int32{static_cast<int32_t>(block_num)} ) ),
               ...
               FSC_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block ${num}", ("num", block_num) );
            }
         } else {
            if( !_blocks.update_one( make_document( kvp( "block_id", block_id_str ) ),
                                     make_document( kvp( "$set", block_doc.view() ) ), update_opts ) ) {
               FSC_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block ${bid}", ("bid", block_id) );
            }
         }
      } catch( ... ) {
...
void mongo_db_plugin_impl::_process_irreversible_block(const chain::block_state_ptr& bs)
{
   ...
   if( store_transactions ) {
      ...
      if( transactions_in_block ) {
         try {
            if( !bulk.execute() ) {
               FSC_ASSERT( false, chain::mongo_db_insert_fail, "Bulk transaction insert failed for block: ${bid}", ("bid", block_id) );
            }
         } catch( ... ) {
...
void mongo_db_plugin_impl::remove_pub_keys( const account_name& name, const permission_name& permission )
{
   ...
   try {
      auto result = _pub_keys.delete_many( make_document( kvp( "account", name.to_string()),
                                                         kvp( "permission", permission.to_string())));
      if( !result ) {
         FSC_ASSERT( false, chain::mongo_db_update_fail,
...
void create_account( mongocxx::collection& accounts, const name& name, std::chrono::milliseconds& now ) {
   ...
   try {
      if( !accounts.update_one( make_document( kvp( "name", name_str )), update.view(), update_opts )) {
         FSC_ASSERT( false, chain::mongo_db_update_fail, "Failed to insert account ${n}", ("n", name));
      }
   } catch (...) {
            ...
            try{
               ...
               try {
                  if( !_accounts.update_one( make_document( kvp( "_id", account->view()["_id"].get_oid())),
                                             update_from.view())) {
                     FSC_ASSERT( false, chain::mongo_db_update_fail, "Failed to udpdate account ${n}", ("n", setabi.account));
                  }
               } catch( ... ) {
      ...
      if( accounts.count( make_document()) == 0 ) {
         ...
         try {
            if( !accounts.insert_one( doc.view())) {
               FSC_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert account ${n}",
...
void mongo_db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         "The target queue size between nodfsc and MongoDB plugin thread.")
         ...
         "MongoDB URI connection string, see: https://docs.mongodb.com/master/reference/connection-string/."
               " If not specified then plugin is disabled. Default database 'FSC' is used if not specified in URI."
               " Example: mongodb://127.0.0.1:27017/FSC")
      ...
      if( options.count( "mongodb-uri" )) {
         ...
         if( options.at( "replay-blockchain" ).as<bool>() || options.at( "hard-replay-blockchain" ).as<bool>() || options.at( "delete-all-blocks" ).as<bool>() ) {
            if( options.at( "mongodb-wipe" ).as<bool>()) {
               ...
               FSC_ASSERT( false, chain::plugin_config_exception, "--mongodb-wipe required with --replay-blockchain, --hard-replay-blockchain, or --delete-all-blocks"
                                 " --mongodb-wipe will remove all FSC collections from mongodb." );
            }
         }

         if( options.count( "abi-serializer-max-time-ms") == 0 ) {
            FSC_ASSERT(false, chain::plugin_config_exception, "--abi-serializer-max-time-ms required as default value not appropriate for parsing full blocks");
         }
         ...
         if( options.count( "mongodb-abi-cache-size" )) {
            ...
            FSC_ASSERT( my->abi_cache_size > 0, chain::plugin_config_exception, "mongodb-abi-cache-size > 0 required" );
         }
         ...
         if( options.count( "mongodb-filter-on" )) {
            ...
            for( auto& s : fo ) {
               ...
               FSC_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --mongodb-filter-on", ("s", s));
         ...
         if( options.count( "mongodb-filter-out" )) {
            ...
            for( auto& s : fo ) {
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               FSC_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --mongodb-filter-out", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
         ...
         wlog( "fscio::mongo_db_plugin configured, but no --mongodb-uri specified." );
