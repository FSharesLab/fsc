...
#include <fscio/http_plugin/local_endpoint.hpp>
      ...
      public:
         ...
         ssl_context_ptr on_tls_init(websocketpp::connection_hdl hdl) {
            ...
            try {
               ...
               if (!ecdh)
                  FSC_THROW(chain::http_exception, "Failed to set NID_secp384r1");
               if(SSL_CTX_set_tmp_ecdh(ctx->native_handle(), (EC_KEY*)ecdh) != 1)
                  FSC_THROW(chain::http_exception, "Failed to set ECDH PFS");
               ...
               if(SSL_CTX_set_cipher_list(ctx->native_handle(), \
                  ...
                  FSC_THROW(chain::http_exception, "Failed to set HTTPS cipher list");
