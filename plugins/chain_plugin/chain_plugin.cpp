...
#include <fscio/chain/fork_database.hpp>
#include <fscio/chain/block_log.hpp>
#include <fscio/chain/exceptions.hpp>
#include <fscio/chain/authorization_manager.hpp>
#include <fscio/chain/producer_object.hpp>
#include <fscio/chain/config.hpp>
#include <fscio/chain/wasm_interface.hpp>
#include <fscio/chain/resource_limits.hpp>
#include <fscio/chain/reversible_block_object.hpp>
#include <fscio/chain/controller.hpp>
#include <fscio/chain/generated_transaction_object.hpp>
#include <fscio/chain/snapshot.hpp>
...
std::ostream& operator<<(std::ostream& osm, fscio::chain::db_read_mode m) {
   if ( m == fscio::chain::db_read_mode::SPECULATIVE ) {
      osm << "speculative";
   } else if ( m == fscio::chain::db_read_mode::HEAD ) {
      osm << "head";
   } else if ( m == fscio::chain::db_read_mode::READ_ONLY ) {
      osm << "read-only";
   } else if ( m == fscio::chain::db_read_mode::IRREVERSIBLE ) {
      osm << "irreversible";
   }
...
void validate(boost::any& v,
              ...
              fscio::chain::db_read_mode* /* target_type */,
              int)
{
  ...
  if ( s == "speculative" ) {
     v = boost::any(fscio::chain::db_read_mode::SPECULATIVE);
  } else if ( s == "head" ) {
     v = boost::any(fscio::chain::db_read_mode::HEAD);
  } else if ( s == "read-only" ) {
     v = boost::any(fscio::chain::db_read_mode::READ_ONLY);
  } else if ( s == "irreversible" ) {
     v = boost::any(fscio::chain::db_read_mode::IRREVERSIBLE);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}

std::ostream& operator<<(std::ostream& osm, fscio::chain::validation_mode m) {
   if ( m == fscio::chain::validation_mode::FULL ) {
      osm << "full";
   } else if ( m == fscio::chain::validation_mode::LIGHT ) {
      osm << "light";
   }
...
void validate(boost::any& v,
              const std::vector<std::string>& values,
              fscio::chain::validation_mode* /* target_type */,
              int)
{
  using namespace boost::program_options;
  ...
  if ( s == "full" ) {
     v = boost::any(fscio::chain::validation_mode::FULL);
  } else if ( s == "light" ) {
     v = boost::any(fscio::chain::validation_mode::LIGHT);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}
...
using namespace fscio::chain::config;
using namespace fscio::chain::plugin_interface;
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         ("wasm-runtime", bpo::value<fscio::chain::wasm_interface::vm_type>()->value_name("wavm/wabt"), "Override default WASM runtime")
         ...
         ("read-mode", boost::program_options::value<fscio::chain::db_read_mode>()->default_value(fscio::chain::db_read_mode::SPECULATIVE),
         ...
         ("validation-mode", boost::program_options::value<fscio::chain::validation_mode>()->default_value(fscio::chain::validation_mode::FULL),
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      try {
         genesis_state gs; // Check if FSCIO_ROOT_KEY is bad
      } catch ( const fc::exception& ) {
         elog( "FSCIO_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
               ("root_key", genesis_state::fscio_root_key));
         throw;
      }
      ...
      if( options.count( "action-blacklist" )) {
         ...
         for( const auto& a : acts ) {
            auto pos = a.find( "::" );
            FSC_ASSERT( pos != std::string::npos, plugin_config_exception, "Invalid entry in action-blacklist: '${a}'", ("a", a));
      ...
      if( options.count("checkpoint") ) {
         ...
         for( const auto& cp : cps ) {
            ...
            if( itr != my->loaded_checkpoints.end() ) {
               FSC_ASSERT( itr->second == item.second,
      ...
      if( options.count( "chain-threads" )) {
         ...
         FSC_ASSERT( my->chain_config->thread_pool_size > 0, plugin_config_exception,
      ...
      FSC_ASSERT( my->chain_config->sig_cpu_bill_pct >= 0 && my->chain_config->sig_cpu_bill_pct <= 100, plugin_config_exception,
      ...
      if( options.count( "extract-genesis-json" ) || options.at( "print-genesis-json" ).as<bool>()) {
         ...
         FSC_THROW( extract_genesis_state_exception, "extracted genesis state from blocks.log" );
      }
      ...
      if( options.count("export-reversible-blocks") ) {
         ...
         FSC_THROW( node_management_success, "exported reversible blocks" );
      }
      ...
      if( options.at( "delete-all-blocks" ).as<bool>()) {
         ...
         FSC_THROW( fixed_reversible_db_exception, "fixed corrupted reversible blocks database" );
         ...
         FSC_THROW( node_management_success, "imported reversible blocks" );
      }
      ...
      if (options.count( "snapshot" )) {
         ...
         FSC_ASSERT( fc::exists(*my->snapshot_path), plugin_config_exception,
         ...
         FSC_ASSERT( options.count( "genesis-json" ) == 0 &&  options.count( "genesis-timestamp" ) == 0,
                 plugin_config_exception,
         ...
         FSC_ASSERT( !fc::exists(shared_mem_path),
                 plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" )) {
            ...
            FSC_ASSERT( log_genesis.compute_chain_id() == my->chain_config->genesis.compute_chain_id(),
                    plugin_config_exception,
         ...
         if( options.count( "genesis-json" )) {
            ...
            FSC_ASSERT( fc::is_regular_file( genesis_file ),
                        plugin_config_exception,
         ...
         if( !existing_genesis ) {
            ...
            FSC_ASSERT( my->chain_config->genesis == *existing_genesis, plugin_config_exception,
      ...
      if ( options.count("read-mode") ) {
         ...
         FSC_ASSERT( my->chain_config->read_mode != db_read_mode::IRREVERSIBLE, plugin_config_exception, "irreversible mode not currently supported." );
      }
      ...
      my->pre_accepted_block_connection = my->chain->pre_accepted_block.connect([this](const signed_block_ptr& blk) {
         ...
         if( itr != my->loaded_checkpoints.end() ) {
            auto id = blk->id();
            FSC_ASSERT( itr->second == id, checkpoint_exception,
...
void chain_apis::read_write::validate() const {
   FSC_ASSERT( db.get_read_mode() != chain::db_read_mode::READ_ONLY, missing_chain_api_plugin_exception, "Not allowed, node in read-only mode" );
}
...
bool chain_plugin::recover_reversible_blocks( const fc::path& db_dir, uint32_t cache_size,
   ...
   if( new_db_dir ) {
      ...
      FSC_ASSERT( reversible_dir_name != ".", invalid_reversible_blocks_dir, "Invalid path to reversible directory" );
      backup_dir = reversible_dir.parent_path() / reversible_dir_name.append("-").append( now );

      FSC_ASSERT( !fc::exists(backup_dir),
   ...
   try {
      for( ; itr != ubi.end(); ++itr ) {
         FSC_ASSERT( itr->blocknum == end + 1, gap_in_reversible_blocks_db,
   ...
   try {
      while( reversible_blocks.tellg() < end_pos ) {
         ...
         if( start == 0 ) {
            ...
            FSC_ASSERT( num == end + 1, gap_in_reversible_blocks_db,
...
chain::chain_id_type chain_plugin::get_chain_id()const {
   FSC_ASSERT( my->chain_id.valid(), chain_id_type_exception, "chain ID has not been initialized yet" );
   return *my->chain_id;
}
...
void chain_plugin::handle_db_exhaustion() {
   ...
   //return 1 -- it's what programs/nodfsc/main.cpp considers "BAD_ALLOC"
   std::_Exit(1);
}
...
uint64_t read_only::get_table_index_name(const read_only::get_table_rows_params& p, bool& primary) {
   ...
   FSC_ASSERT( index == table, chain::contract_table_query_exception, "Unsupported table name: ${n}", ("n", p.table) );
   ...
   if (p.index_position.empty() || p.index_position == "first" || p.index_position == "primary" || p.index_position == "one") {
      ...
      try {
         ...
         FSC_ASSERT( false, chain::contract_table_query_exception, "Invalid index_position: ${p}", ("p", p.index_position));
      }
      if (pos < 2) {
         primary = true;
         pos = 0;
...
uint64_t convert_to_type(const string& str, const string& desc) {
   ...
   if (str.find(',') != string::npos) { // fix #6274 only match formats like 4,FSC
      try {
         auto symb = fscio::chain::symbol::from_string(str);
         return symb.value();
      } catch( ... ) { }
   }
   
   try {
      return ( fscio::chain::string_to_symbol( 0, str.c_str() ) >> 8 );
   } catch( ... ) {
      FSC_ASSERT( false, chain_type_exception, "Could not convert ${desc} string '${str}' to any of the following: "
...
double convert_to_type(const string& str, const string& desc) {
   ...
   FSC_ASSERT( !std::isnan(val), chain::contract_table_query_exception,
...
abi_def get_abi( const controller& db, const name& account ) {
   ...
   FSC_ASSERT(code_accnt != nullptr, chain::account_query_exception, "Fail to retrieve account for ${account}", ("account", account) );
   abi_def abi;
...
string get_table_type( const abi_def& abi, const name& table_name ) {
   ...
   FSC_ASSERT( false, chain::contract_table_query_exception, "Table ${table} is not specified in the ABI", ("table",table_name) );
}

read_only::get_table_rows_result read_only::get_table_rows( const read_only::get_table_rows_params& p )const {
   const abi_def abi = fscio::chain_apis::get_abi( db, p.code );

   bool primary = false;
   auto table_with_index = get_table_index_name( p, primary );
   if( primary ) {
      FSC_ASSERT( p.table == table_with_index, chain::contract_table_query_exception, "Invalid table name ${t}", ( "t", p.table ));
      auto table_type = get_table_type( abi, p.table );
      if( table_type == KEYi64 || p.key_type == "i64" || p.key_type == "name" ) {
         return get_table_rows_ex<key_value_index>(p,abi);
      }
      FSC_ASSERT( false, chain::contract_table_query_exception,  "Invalid table type ${type}", ("type",table_type)("abi",abi));
   } else {
      FSC_ASSERT( !p.key_type.empty(), chain::contract_table_query_exception, "key type required for non-primary index" );
      ...
      FSC_ASSERT(false, chain::contract_table_query_exception,  "Unsupported secondary index type: ${t}", ("t", p.key_type));
   }
}
...
vector<asset> read_only::get_currency_balance( const read_only::get_currency_balance_params& p )const {
   ...
   walk_key_value_table(p.code, p.account, N(accounts), [&](const key_value_object& obj){
      FSC_ASSERT( obj.value.size() >= sizeof(asset), chain::asset_type_exception, "Invalid data on table");

      asset cursor;
      fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
      fc::raw::unpack(ds, cursor);

      FSC_ASSERT( cursor.get_symbol().valid(), chain::asset_type_exception, "Invalid asset");
...
fc::variant read_only::get_currency_stats( const read_only::get_currency_stats_params& p )const {
   ...
   uint64_t scope = ( fscio::chain::string_to_symbol( 0, boost::algorithm::to_upper_copy(p.symbol).c_str() ) >> 8 );

   walk_key_value_table(p.code, scope, N(stat), [&](const key_value_object& obj){
      FSC_ASSERT( obj.value.size() >= sizeof(read_only::get_currency_stats_result), chain::asset_type_exception, "Invalid data on table");
...
fc::variant get_global_row( const database& db, const abi_def& abi, const abi_serializer& abis, const fc::microseconds& abi_serializer_max_time_ms, bool shorten_abi_errors ) {
   ...
   FSC_ASSERT(table_type == read_only::KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table global", ("type",table_type));
   ...
   FSC_ASSERT(table_id, chain::contract_table_query_exception, "Missing table global");
   ...
   FSC_ASSERT(it != kv_index.end(), chain::contract_table_query_exception, "Missing row in table global");
...
read_only::get_producers_result read_only::get_producers( const read_only::get_producers_params& p ) const {
   const abi_def abi = fscio::chain_apis::get_abi(db, config::system_account_name);
   const auto table_type = get_table_type(abi, N(producers));
   const abi_serializer abis{ abi, abi_serializer_max_time };
   FSC_ASSERT(table_type == KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table producers", ("type",table_type));
   ...
   FSC_ASSERT(table_id && secondary_table_id, chain::contract_table_query_exception, "Missing producers table");
      ...
      if (!p.lower_bound.empty()) {
         try {
            ...
            try {
               ...
               if (itr == by_txid.end()) {
                  FSC_THROW(transaction_exception, "Unknown Transaction ID: ${txid}", ("txid", txid));
               }
...
fc::variant read_only::get_block(const read_only::get_block_params& params) const {
   ...
   FSC_ASSERT(!params.block_num_or_id.empty() && params.block_num_or_id.size() <= 64, chain::block_id_type_exception, "Invalid Block number or ID, must be greater than 0 and less than 64 characters" );
   try {
   ...
   } FSC_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))

   FSC_ASSERT( block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));
...
fc::variant read_only::get_block_header_state(const get_block_header_state_params& params) const {
   ...
   FSC_ASSERT( b, unknown_block_exception, "Could not find reversible block: ${block}", ("block", params.block_num_or_id));
...
void read_write::push_transaction(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
   ...
   try {
      ...
      } FSC_RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid packed transaction")
...
void read_write::push_transactions(const read_write::push_transactions_params& params, next_function<read_write::push_transactions_results> next) {
   try {
      FSC_ASSERT( params.size() <= 1000, too_many_tx_at_once, "Attempt to push too many transactions at once" );
...
read_only::get_code_results read_only::get_code( const get_code_params& params )const {
   ...
   FSC_ASSERT( params.code_as_wasm, unsupported_feature, "Returning WAST from get_code is no longer supported" );
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   while( perm != permissions.end() && perm->owner == params.account_name ) {
      ...
      if( perm->parent._id ) {
         ...
         if( p ) {
            FSC_ASSERT(perm->owner == p->owner, invalid_parent_permission, "Invalid parent permission");
            parent = p->name;
         }
      }
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      const auto token_code = N(fscio.token);
...
read_only::abi_json_to_bin_result read_only::abi_json_to_bin( const read_only::abi_json_to_bin_params& params )const try {
   ...
   FSC_ASSERT(code_account != nullptr, contract_query_exception, "Contract can't be found ${contract}", ("contract", params.code));

   abi_def abi;
   if( abi_serializer::to_abi(code_account->abi, abi) ) {
      abi_serializer abis( abi, abi_serializer_max_time );
      auto action_type = abis.get_action_type(params.action);
      FSC_ASSERT(!action_type.empty(), action_validate_exception, "Unknown action ${action} in contract ${contract}", ("action", params.action)("contract", params.code));
      try {
         result.binargs = abis.variant_to_binary( action_type, params.args, abi_serializer_max_time, shorten_abi_errors );
      } FSC_RETHROW_EXCEPTIONS(chain::invalid_action_args_exception,
                                "'${args}' is invalid args for action '${action}' code '${code}'. expected '${proto}'",
                                ("args", params.args)("action", params.action)("code", params.code)("proto", action_abi_to_variant(abi, action_type)))
   } else {
      FSC_ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", params.code));
   }
   return result;
...
read_only::get_required_keys_result read_only::get_required_keys( const get_required_keys_params& params )const {
   ...
   } FSC_RETHROW_EXCEPTIONS(chain::transaction_type_exception, "Invalid transaction")
...
chain::symbol read_only::extract_core_symbol()const {
   ...
   // The following code makes assumptions about the contract deployed on fscio account (i.e. the system contract) and how it stores its data.
   const auto& d = db.db();
   const auto* t_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple( N(fscio), N(fscio), N(rammarket) ));
   if( t_id != nullptr ) {
      const auto &idx = d.get_index<key_value_index, by_scope_primary>();
      auto it = idx.find(boost::make_tuple( t_id->id, fscio::chain::string_to_symbol_c(4,"RAMCORE") ));
      if( it != idx.end() ) {
...
FC_REFLECT( fscio::chain_apis::detail::ram_market_exchange_state_t, (ignore1)(ignore2)(ignore3)(core_symbol)(ignore4) )
