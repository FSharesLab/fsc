 ...
 */
#include <fscio/txn_test_gen_plugin/txn_test_gen_plugin.hpp>
...
namespace fscio { namespace detail {
  struct txn_test_gen_empty {};
}}

FC_REFLECT(fscio::detail::txn_test_gen_empty, );
...
#define INVOKE_V_R_R_R(api_handle, call_name, in_param0, in_param1, in_param2) \
     ...
     fscio::detail::txn_test_gen_empty result;
   ...
   [this](string, string body, url_response_callback cb) mutable { \
      ...
      auto result_handler = [cb, body](const fc::exception_ptr& e) {\
         if (e) {\
            ...
            cb(http_response_code, fc::json::to_string(fscio::detail::txn_test_gen_empty())); \
         }\
      };\
      INVOKE \
   }\
...
struct txn_test_gen_plugin_impl {
   ...
   void create_test_accounts(const std::string& init_name, const std::string& init_priv_key, const std::function<void(const fc::exception_ptr&)>& next) {
      ...
      try {
         ...
         abi_def currency_abi_def = fc::json::from_string(fscio_token_abi).as<abi_def>();

         controller& cc = app().get_plugin<chain_plugin>().chain();
         auto chainid = app().get_plugin<chain_plugin>().get_chain_id();
         auto abi_serializer_max_time = app().get_plugin<chain_plugin>().get_abi_serializer_max_time();

         abi_serializer fscio_token_serializer{fc::json::from_string(fscio_token_abi).as<abi_def>(), abi_serializer_max_time};
         ...
         //create some test accounts
         {
            ...
            //create "A" account
            {
            auto owner_auth   = fscio::chain::authority{1, {{txn_text_receiver_A_pub_key, 1}}, {}};
            auto active_auth  = fscio::chain::authority{1, {{txn_text_receiver_A_pub_key, 1}}, {}};
            ...
            //create "B" account
            {
            auto owner_auth   = fscio::chain::authority{1, {{txn_text_receiver_B_pub_key, 1}}, {}};
            auto active_auth  = fscio::chain::authority{1, {{txn_text_receiver_B_pub_key, 1}}, {}};
            ...
            {
            auto owner_auth   = fscio::chain::authority{1, {{txn_text_receiver_C_pub_key, 1}}, {}};
            auto active_auth  = fscio::chain::authority{1, {{txn_text_receiver_C_pub_key, 1}}, {}};
         ...
         //set txn.test.t contract to fscio.token & initialize it
         {
            signed_transaction trx;

            vector<uint8_t> wasm = wast_to_wasm(std::string(fscio_token_wast));
...

            {
               setabi handler;
               handler.account = newaccountC;
               handler.abi = fc::raw::pack(json::from_string(fscio_token_abi).as<abi_def>());
               trx.actions.emplace_back( vector<chain::permission_level>{{newaccountC,"active"}}, handler);
            }

            {
               ...
               act.data = fscio_token_serializer.variant_to_binary("create", fc::json::from_string("{\"issuer\":\"txn.test.t\",\"maximum_supply\":\"1000000000.0000 CUR\"}}"), abi_serializer_max_time);
            ...
            }
            {
               ...
               act.name = N(issue);
               act.authorization = vector<permission_level>{{newaccountC,config::active_name}};
               act.data = fscio_token_serializer.variant_to_binary("issue", fc::json::from_string("{\"to\":\"txn.test.t\",\"quantity\":\"600.0000 CUR\",\"memo\":\"\"}"), abi_serializer_max_time);
               trx.actions.push_back(act);
            }
            {
               action act;
               ...
               act.name = N(transfer);
               act.authorization = vector<permission_level>{{newaccountC,config::active_name}};
               act.data = fscio_token_serializer.variant_to_binary("transfer", fc::json::from_string("{\"from\":\"txn.test.t\",\"to\":\"txn.test.a\",\"quantity\":\"200.0000 CUR\",\"memo\":\"\"}"), abi_serializer_max_time);
               trx.actions.push_back(act);
            }
            {
               action act;
               ...
               act.name = N(transfer);
               act.authorization = vector<permission_level>{{newaccountC,config::active_name}};
               act.data = fscio_token_serializer.variant_to_binary("transfer", fc::json::from_string("{\"from\":\"txn.test.t\",\"to\":\"txn.test.b\",\"quantity\":\"200.0000 CUR\",\"memo\":\"\"}"), abi_serializer_max_time);
               trx.actions.push_back(act);
            }
   ...
   void start_generation(const std::string& salt, const uint64_t& period, const uint64_t& batch_size) {
      ...
      act_a_to_b.data = fscio_token_serializer.variant_to_binary("transfer", 
      ...
      act_b_to_a.data = fscio_token_serializer.variant_to_binary("transfer", 
