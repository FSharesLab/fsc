 ...
 */
#include <fscio/producer_plugin/producer_plugin.hpp>
...
class producer_plugin_impl : public std::enable_shared_from_this<producer_plugin_impl> {
   public:
      ...
      uint32_t _production_skip_flags              = 0; //fscio::chain::skip_nothing;
      ...
      fc::microseconds                                          _kfscd_provider_timeout_us;
   ...
   //                  ilog( "${n} confirmed", ("n",name(producer)) );
      ...
      void on_incoming_block(const signed_block_ptr& block) {
         ...
         FSC_ASSERT( block->timestamp < (fc::time_point::now() + fc::seconds( 7 )), block_from_the_future,
...
void new_chain_banner(const fscio::chain::controller& db)
{
   std::cerr << "\n"
      "*******************************\n"
      "*                             *\n"
   ...
   boost::program_options::options_description& config_file_options)
{
   ...
   producer_options.add_options()
         ...
         ("signature-provider", boost::program_options::value<vector<string>>()->composing()->multitoken()->default_value({std::string(default_priv_key.get_public_key()) + "=KEY:" + std::string(default_priv_key)}, std::string(default_priv_key.get_public_key()) + "=KEY:" + std::string(default_priv_key)),
          ...
          "   KFSCD:<data>    \tis the URL where kfscd is available and the approptiate wallet(s) are unlocked")
         ("kfscd-provider-timeout", boost::program_options::value<int32_t>()->default_value(5),
          "Limits the maximum time (in milliseconds) that is allowd for sending blocks to a kfscd provider for signing")
...
chain::signature_type producer_plugin::sign_compact(const chain::public_key_type& key, const fc::sha256& digest) const
{
  if(key != chain::public_key_type()) {
    auto private_key_itr = my->_signature_providers.find(key);
    FSC_ASSERT(private_key_itr != my->_signature_providers.end(), producer_priv_key_not_found, "Local producer has no private key in config.ini corresponding to public key ${key}", ("key", key));
...
static producer_plugin_impl::signature_provider_type
make_kfscd_signature_provider(const std::shared_ptr<producer_plugin_impl>& impl, const string& url_str, const public_key_type pubkey) {
   fc::url kfscd_url;
   if(boost::algorithm::starts_with(url_str, "unix://"))
      ...
      kfscd_url = fc::url("unix", url_str.substr(7), ostring(), ostring(), ostring(), ostring(), ovariant_object(), fc::optional<uint16_t>());
   else
      kfscd_url = fc::url(url_str);
   std::weak_ptr<producer_plugin_impl> weak_impl = impl;

   return [weak_impl, kfscd_url, pubkey]( const chain::digest_type& digest ) {
      auto impl = weak_impl.lock();
      if (impl) {
         fc::variant params;
         fc::to_variant(std::make_pair(digest, pubkey), params);
         auto deadline = impl->_kfscd_provider_timeout_us.count() >= 0 ? fc::time_point::now() + impl->_kfscd_provider_timeout_us : fc::time_point::maximum();
         return app().get_plugin<http_client_plugin>().get_client().post_sync(kfscd_url, params, deadline).as<chain::signature_type>();
      } else {
...
{ try {
   ...
   FSC_ASSERT( my->chain_plug, plugin_config_exception, "chain_plugin not found" );
   my->_options = &options;
   ...
   if( options.count("signature-provider") ) {
      ...
      for (const auto& key_spec_pair : key_spec_pairs) {
         try {
            ...
            FSC_ASSERT(delim != std::string::npos, plugin_config_exception, "Missing \"=\" in the key spec pair");
            auto pub_key_str = key_spec_pair.substr(0, delim);
            auto spec_str = key_spec_pair.substr(delim + 1);

            auto spec_delim = spec_str.find(":");
            FSC_ASSERT(spec_delim != std::string::npos, plugin_config_exception, "Missing \":\" in the key spec pair");
            ...
            } else if (spec_type_str == "KFSCD") {
               my->_signature_providers[pubkey] = make_kfscd_signature_provider(my, spec_data, pubkey);
            }
   ...
   my->_kfscd_provider_timeout_us = fc::milliseconds(options.at("kfscd-provider-timeout").as<int32_t>());
   ...
   FSC_ASSERT( thread_pool_size > 0, plugin_config_exception,
   ...
   if( options.count( "snapshots-dir" )) {
      ...
      FSC_ASSERT( fc::is_directory(my->_snapshots_dir), snapshot_directory_not_found_exception,
...
{ try {
   ...
   FSC_ASSERT( my->_producers.empty() || chain.get_read_mode() == chain::db_read_mode::SPECULATIVE, plugin_config_exception,
              "node cannot have any producer-name configured because block production is impossible when read_mode is not \"speculative\"" );

   FSC_ASSERT( my->_producers.empty() || chain.get_validation_mode() == chain::validation_mode::FULL, plugin_config_exception,
   ...
   if (!my->_producers.empty()) {
      ...
      if (my->_production_enabled) {
         ...
         //_production_skip_flags |= fscio::chain::skip_undo_history_check;
      }
   }
...
producer_plugin::snapshot_information producer_plugin::create_snapshot() const {
   ...
   FSC_ASSERT( !fc::is_regular_file(snapshot_path), snapshot_exists_exception,
...
void producer_plugin_impl::schedule_production_loop() {
   ...
   if (result == start_block_result::failed) {
      ...
      if (deadline > fc::time_point::now()) {
         ...
         FSC_ASSERT( chain.pending_block_state(), missing_pending_block_state, "producing without pending_block_state, start_block succeeded" );
         ...
         FSC_ASSERT( chain.pending_block_state(), missing_pending_block_state, "producing without pending_block_state" );
      ...
      FSC_ASSERT( chain.pending_block_state(), missing_pending_block_state, "speculating without pending_block_state" );
      const auto& pbs = chain.pending_block_state();
...
void producer_plugin_impl::produce_block() {
   ...
   FSC_ASSERT(_pending_block_mode == pending_block_mode::producing, producer_exception, "called produce_block while not actually producing");
   chain::controller& chain = chain_plug->chain();
   const auto& pbs = chain.pending_block_state();
   const auto& hbs = chain.head_block_state();
   FSC_ASSERT(pbs, missing_pending_block_state, "pending_block_state does not exist but it should, another plugin may have corrupted it");
   auto signature_provider_itr = _signature_providers.find( pbs->block_signing_key );

   FSC_ASSERT(signature_provider_itr != _signature_providers.end(), producer_priv_key_not_found, "Attempting to produce a block for which we don't have the private key");
