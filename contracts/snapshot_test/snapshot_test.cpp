...
namespace snapshot_test {
   ...
   struct main_record {
      ...
      FSCLIB_SERIALIZE( main_record, (id)(index_f64)(index_f128)(index_i64)(index_i128)(index_i256) )
   };
   ...
   struct increment {
      ...
      FSCLIB_SERIALIZE(increment, (value))
   };

   using multi_index_type = fscio::multi_index<N(data), main_record,
...
namespace multi_index_test {
   extern "C" {
      ...
      void apply( uint64_t self, uint64_t code, uint64_t action ) {
         require_auth(code);
         fscio_assert(action == N(increment), "unsupported action");
