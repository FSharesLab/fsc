...
void test_db::primary_i64_general(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   // find
   {
      ...
      fscio_assert(itr_next == itr_next_expected && prim == N(allyson), "primary_i64_general - db_find_i64" );
      itr_next = db_next_i64(itr_next, &prim);
      itr_next_expected = db_find_i64(receiver, receiver, table1, N(bob));
      fscio_assert(itr_next == itr_next_expected && prim == N(bob), "primary_i64_general - db_next_i64");
   }
   ...
   // next
   {
      ...
      fscio_assert(end_itr < 0, "primary_i64_general - db_next_i64");
      // prim didn't change
      fscio_assert(prim == 0, "primary_i64_general - db_next_i64");
   }
   ...
   // previous
   {
      ...
      fscio_assert(itr_prev == itr_prev_expected && prim == N(bob), "primary_i64_general - db_previous_i64");

      itr_prev = db_previous_i64(itr_prev, &prim);
      itr_prev_expected = db_find_i64(receiver, receiver, table1, N(allyson));
      fscio_assert(itr_prev == itr_prev_expected && prim == N(allyson), "primary_i64_general - db_previous_i64");

      itr_prev = db_previous_i64(itr_prev, &prim);
      itr_prev_expected = db_find_i64(receiver, receiver, table1, N(alice));
      fscio_assert(itr_prev == itr_prev_expected && prim == N(alice), "primary_i64_general - db_previous_i64");

      itr_prev = db_previous_i64(itr_prev, &prim);
      fscio_assert(itr_prev < 0 && prim == N(alice), "primary_i64_general - db_previous_i64");
   }

   // remove
   {
      int itr = db_find_i64(receiver, receiver, table1, N(alice));
      fscio_assert(itr >= 0, "primary_i64_general - db_find_i64");
      db_remove_i64(itr);
   ...
   // get
   {
      ...
      value[buffer_len] = '\0';
      std::string s(value);
      fscio_assert(uint32_t(len) == buffer_len, "primary_i64_general - db_get_i64");
      fscio_assert(s == "bob's", "primary_i64_general - db_get_i64  - 5");
      ...
      fscio_assert(sfull == "bob's info", "primary_i64_general - db_get_i64 - full");
   }
   ...
   // update
   {
      ...
      fscio_assert(sret == "bob's new info", "primary_i64_general - db_update_i64");
   }
}
...
void test_db::primary_i64_lowerbound(uint64_t receiver, uint64_t code, uint64_t action)
{
...

   {
      ...
      fscio_assert(lb == db_find_i64(receiver, receiver, table, N(alice)), err.c_str());
   }
   {
      int lb = db_lowerbound_i64(receiver, receiver, table, N(billy));
      fscio_assert(lb == db_find_i64(receiver, receiver, table, N(bob)), err.c_str());
   }
   {
      int lb = db_lowerbound_i64(receiver, receiver, table, N(frank));
      fscio_assert(lb == db_find_i64(receiver, receiver, table, N(joe)), err.c_str());
   }
   {
...
void test_db::primary_i64_upperbound(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   const std::string err = "primary_i64_upperbound";
   {
      ...
      fscio_assert(ub == db_find_i64(receiver, receiver, table, N(allyson)), err.c_str());
   }
   {
      int ub = db_upperbound_i64(receiver, receiver, table, N(billy));
      fscio_assert(ub == db_find_i64(receiver, receiver, table, N(bob)), err.c_str());
   }
   {
      int ub = db_upperbound_i64(receiver, receiver, table, N(frank));
      fscio_assert(ub == db_find_i64(receiver, receiver, table, N(joe)), err.c_str());
   }
   {
      int ub = db_upperbound_i64(receiver, receiver, table, N(joe));
      fscio_assert(ub < 0, err.c_str());
   }
   {
...
void test_db::idx64_general(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   // find_primary
   {
      ...
      fscio_assert(itr < 0 && sec == 0, "idx64_general - db_idx64_find_primary");
      itr = db_idx64_find_primary(receiver, receiver, table, &sec, 110);
      fscio_assert(itr >= 0 && sec == N(joe), "idx64_general - db_idx64_find_primary");
      uint64_t prim_next = 0;
      int itr_next = db_idx64_next(itr, &prim_next);
      fscio_assert(itr_next < 0 && prim_next == 0, "idx64_general - db_idx64_find_primary");
   }
   ...
   // iterate forward starting with charlie
   {
      ...
      fscio_assert(itr >= 0 && sec == N(charlie), "idx64_general - db_idx64_find_primary");

      uint64_t prim_next = 0;
      int itr_next = db_idx64_next(itr, &prim_next);
      fscio_assert(itr_next >= 0 && prim_next == 976, "idx64_general - db_idx64_next");
      secondary_type sec_next = 0;
      int itr_next_expected = db_idx64_find_primary(receiver, receiver, table, &sec_next, prim_next);
      fscio_assert(itr_next == itr_next_expected && sec_next == N(emily), "idx64_general - db_idx64_next");

      itr_next = db_idx64_next(itr_next, &prim_next);
      fscio_assert(itr_next >= 0 && prim_next == 110, "idx64_general - db_idx64_next");
      itr_next_expected = db_idx64_find_primary(receiver, receiver, table, &sec_next, prim_next);
      fscio_assert(itr_next == itr_next_expected && sec_next == N(joe), "idx64_general - db_idx64_next");
   ...
   // iterate backward staring with second bob
   {
      ...
      fscio_assert(itr >= 0 && sec == N(bob), "idx64_general - db_idx64_find_primary");

      uint64_t prim_prev = 0;
      int itr_prev = db_idx64_previous(itr, &prim_prev);
      fscio_assert(itr_prev >= 0 && prim_prev == 540, "idx64_general - db_idx64_previous");

      secondary_type sec_prev = 0;
      int itr_prev_expected = db_idx64_find_primary(receiver, receiver, table, &sec_prev, prim_prev);
      fscio_assert(itr_prev == itr_prev_expected && sec_prev == N(bob), "idx64_general - db_idx64_previous");

      itr_prev = db_idx64_previous(itr_prev, &prim_prev);
      fscio_assert(itr_prev >= 0 && prim_prev == 650, "idx64_general - db_idx64_previous");
      itr_prev_expected = db_idx64_find_primary(receiver, receiver, table, &sec_prev, prim_prev);
      fscio_assert(itr_prev == itr_prev_expected && sec_prev == N(allyson), "idx64_general - db_idx64_previous");

      itr_prev = db_idx64_previous(itr_prev, &prim_prev);
      fscio_assert(itr_prev >= 0 && prim_prev == 265, "idx64_general - db_idx64_previous");
      itr_prev_expected = db_idx64_find_primary(receiver, receiver, table, &sec_prev, prim_prev);
      fscio_assert(itr_prev == itr_prev_expected && sec_prev == N(alice), "idx64_general - db_idx64_previous");
   ...
   // find_secondary
   {
      ...
      fscio_assert(itr >= 0 && prim == 540, "idx64_general - db_idx64_find_secondary");

      sec = N(emily);
      itr = db_idx64_find_secondary(receiver, receiver, table, &sec, &prim);
      fscio_assert(itr >= 0 && prim == 976, "idx64_general - db_idx64_find_secondary");
   ...
   // update and remove
   {
      ...
      fscio_assert(sec_itr == itr && sec == new_name, "idx64_general - db_idx64_update");
      db_idx64_remove(itr);
      int itrf = db_idx64_find_primary(receiver, receiver, table, &sec, ssn);
      fscio_assert(itrf < 0, "idx64_general - db_idx64_remove");
   }
}
...
void test_db::idx64_lowerbound(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   const std::string err = "idx64_lowerbound";
   {
      ...
      fscio_assert(lb_prim == ssn && lb_sec == N(alice), err.c_str());
      fscio_assert(lb == db_idx64_find_primary(receiver, receiver, table, &lb_sec, ssn), err.c_str());
   }
   {
      ...
      int lb = db_idx64_lowerbound(receiver, receiver, table, &lb_sec, &lb_prim);
      fscio_assert(lb_prim == ssn && lb_sec == N(bob), err.c_str());
      fscio_assert(lb == db_idx64_find_primary(receiver, receiver, table, &lb_sec, ssn), err.c_str());
   }
   {
      ...
      int lb = db_idx64_lowerbound(receiver, receiver, table, &lb_sec, &lb_prim);
      fscio_assert(lb_prim == ssn && lb_sec == N(joe), err.c_str());
      fscio_assert(lb == db_idx64_find_primary(receiver, receiver, table, &lb_sec, ssn), err.c_str());
   }
   {
      ...
      uint64_t lb_prim = 0;
      int lb = db_idx64_lowerbound(receiver, receiver, table, &lb_sec, &lb_prim);
      fscio_assert(lb_prim == 0 && lb_sec == N(kevin), err.c_str());
      fscio_assert(lb < 0, "");
   }
}
...
void test_db::idx64_upperbound(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   const std::string err = "idx64_upperbound";
   {
      ...
      fscio_assert(ub_prim == allyson_ssn && ub_sec == N(allyson), "");
      fscio_assert(ub == db_idx64_find_primary(receiver, receiver, table, &ub_sec, allyson_ssn), err.c_str());
   }
   {
      ...
      int ub = db_idx64_upperbound(receiver, receiver, table, &ub_sec, &ub_prim);
      fscio_assert(ub_prim == bob_ssn && ub_sec == N(bob), "");
      fscio_assert(ub == db_idx64_find_primary(receiver, receiver, table, &ub_sec, bob_ssn), err.c_str());
   }
   {
      secondary_type ub_sec = N(joe);
      uint64_t ub_prim = 0;
      int ub = db_idx64_upperbound(receiver, receiver, table, &ub_sec, &ub_prim);
      fscio_assert(ub_prim == 0 && ub_sec == N(joe), err.c_str());
      fscio_assert(ub < 0, err.c_str());
   }
   {
      ...
      uint64_t ub_prim = 0;
      int ub = db_idx64_upperbound(receiver, receiver, table, &ub_sec, &ub_prim);
      fscio_assert(ub_prim == 0 && ub_sec == N(kevin), err.c_str());
      fscio_assert(ub < 0, err.c_str());
   }
}
...
{
   (void)code;(void)action;
   auto act = fscio::get_action(1, 0);
   auto ia = fscio::unpack<invalid_access_action>(act.data);
   ...
   if( ia.store ) {
      ...
      //fscio::print("test_invalid_access: stored ", value_to_store, "\n");
   } else {
      fscio_assert( itr >= 0, "test_invalid_access: could not find row" );
      switch( ia.index ) {
         case 1:
         break;
         case 0:
         default:
            fscio_assert( db_get_i64( itr, &value, sizeof(value) ) == sizeof(value),
                          "test_invalid_access: value in primary table was incorrect size" );
         break;
      }
      //fscio::print("test_invalid_access: expected ", ia.val, " and retrieved ", value, "\n");
      fscio_assert( value == ia.val, "test_invalid_access: value did not match" );
   }
}
...
void test_db::idx_double_nan_lookup_fail(uint64_t receiver, uint64_t, uint64_t) {
   auto act = fscio::get_action(1, 0);
   auto lookup_type = fscio::unpack<uint32_t>(act.data);
   ...
   switch( lookup_type ) {
      ...
      break;
      default:
         fscio_assert( false, "idx_double_nan_lookup_fail: unexpected lookup_type" );
   }
}
...
void test_db::misaligned_secondary_key256_tests(uint64_t /* receiver */, uint64_t, uint64_t) {
   auto key = fscio::key256::make_from_word_sequence<uint64_t>(0ULL, 0ULL, 0ULL, 42ULL);
   char* ptr = (char*)(&key);
   ptr += 1;
