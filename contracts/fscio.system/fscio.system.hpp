...
namespace fsciosystem {
   ...
   typedef fscio::multi_index< N(namebids), name_bid,
                               indexed_by<N(highbid), const_mem_fun<name_bid, uint64_t, &name_bid::by_high_bid>  >
                               >  name_bid_table;


   struct fscio_global_state : fscio::blockchain_parameters {
   ...
   struct voter_info {
      ...
      time                        reserved2 = 0;
      fscio::asset                reserved3;

      uint64_t primary_key()const { return owner; }

      // explicit serialization macro is not necessary, used here only to improve compilation time
      FSCLIB_SERIALIZE( voter_info, (owner)(proxy)(producers)(staked)(last_vote_weight)(proxied_vote_weight)(is_proxy)(reserved1)(reserved2)(reserved3) )
   };

   typedef fscio::multi_index< N(voters), voter_info>  voters_table;


   typedef fscio::multi_index< N(producers), producer_info,
                               indexed_by<N(prototalvote), const_mem_fun<producer_info, double, &producer_info::by_votes>  >
                               >  producers_table;

   typedef fscio::singleton<N(global), fscio_global_state> global_state_singleton;
