...
namespace fsciosystem {
    ...
    */
   typedef fscio::multi_index< N(userres), user_resources>      user_resources_table;
   typedef fscio::multi_index< N(delband), delegated_bandwidth> del_bandwidth_table;
   typedef fscio::multi_index< N(refunds), refund_request>      refunds_table;
   ...
   void system_contract::buyram( account_name payer, account_name receiver, asset quant )
   {
      ...
      INLINE_ACTION_SENDER(fscio::token, transfer)( N(fscio.token), {payer,N(active)},
         { payer, N(fscio.ram), quant_after_fee, std::string("buy ram") } );

      if( fee.amount > 0 ) {
         INLINE_ACTION_SENDER(fscio::token, transfer)( N(fscio.token), {payer,N(active)},
                                                       { payer, N(fscio.ramfee), fee, std::string("ram fee") } );
      }
   ...
   void system_contract::sellram( account_name account, int64_t bytes ) {
      ...
      fscio_assert( bytes > 0, "cannot sell negative byte" );
      ...
      INLINE_ACTION_SENDER(fscio::token, transfer)( N(fscio.token), {N(fscio.ram),N(active)},
                                                       { N(fscio.ram), account, asset(tokens_out), std::string("sell ram") } );

      auto fee = ( tokens_out.amount + 199 ) / 200; /// .5% fee (round up)
      // since tokens_out.amount was asserted to be at least 2 earlier, fee.amount < tokens_out.amount
      
      if( fee > 0 ) {
         INLINE_ACTION_SENDER(fscio::token, transfer)( N(fscio.token), {account,N(active)},
            { account, N(fscio.ramfee), asset(fee), std::string("sell ram fee") } );
      }
   }
   ...
   void validate_b1_vesting( int64_t stake ) {
      ...
      fscio_assert( max_claimable - claimable <= stake, "b1 can only claim their tokens over 10 years" );
   }
   ...
   {
      require_auth( from );
      fscio_assert( stake_net_delta != asset(0) || stake_cpu_delta != asset(0), "should stake non-zero amount" );
      ...
      // update stake delegated from "from" to "receiver"
      {
         ...
         fscio_assert( asset(0) <= itr->net_weight, "insufficient staked net bandwidth" );
         fscio_assert( asset(0) <= itr->cpu_weight, "insufficient staked cpu bandwidth" );
      ...
      // update totals of "receiver"
      {
         ...
         fscio_assert( asset(0) <= tot_itr->net_weight, "insufficient staked total net bandwidth" );
         fscio_assert( asset(0) <= tot_itr->cpu_weight, "insufficient staked total cpu bandwidth" );
      ...
      if ( N(fscio.stake) != source_stake_from ) { //for fscio both transfer and refund make no sense
         ...
         if( is_delegating_to_self || is_undelegating ) {
            if ( req != refunds_tbl.end() ) { //need to update refund
               ...
               fscio_assert( asset(0) <= req->net_amount, "negative net refund amount" ); //should never happen
               fscio_assert( asset(0) <= req->cpu_amount, "negative cpu refund amount" ); //should never happen
         ...
         if ( asset(0) < transfer_amount ) {
            INLINE_ACTION_SENDER(fscio::token, transfer)( N(fscio.token), {source_stake_from, N(active)},
               { source_stake_from, N(fscio.stake), asset(transfer_amount), std::string("stake bandwidth") } );
         }
      }
      ...
      // update voting power
      {
         ...
         fscio_assert( 0 <= from_voter->staked, "stake for voting cannot be negative");
         if( from == N(b1) ) {
   ...
   {
      fscio_assert( stake_cpu_quantity >= asset(0), "must stake a positive amount" );
      fscio_assert( stake_net_quantity >= asset(0), "must stake a positive amount" );
      fscio_assert( stake_net_quantity + stake_cpu_quantity > asset(0), "must stake a positive amount" );
   ...
   {
      fscio_assert( asset() <= unstake_cpu_quantity, "must unstake a positive amount" );
      fscio_assert( asset() <= unstake_net_quantity, "must unstake a positive amount" );
      fscio_assert( asset() < unstake_cpu_quantity + unstake_net_quantity, "must unstake a positive amount" );
      fscio_assert( _gstate.total_activated_stake >= min_activated_stake,
   ...
   void system_contract::refund( const account_name owner ) {
      ...
      fscio_assert( req->request_time + refund_delay <= now(), "refund is not available yet" );
      // Until now() becomes NOW, the fact that now() is the timestamp of the previous block could in theory
      // allow people to get their tokens earlier than the 3 day delay if the unstake happened immediately after many
      // consecutive missed blocks.

      INLINE_ACTION_SENDER(fscio::token, transfer)( N(fscio.token), {N(fscio.stake),N(active)},
                                                    { N(fscio.stake), req->owner, req->net_amount + req->cpu_amount, std::string("unstake") } );
