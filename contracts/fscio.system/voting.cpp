...
namespace fsciosystem {
    ...
    *
    */
   void system_contract::regproducer( const account_name producer, const fscio::public_key& producer_key, const std::string& url, uint16_t location ) {
   ...
   void system_contract::update_votes( const account_name voter_name, const account_name proxy, const std::vector<account_name>& producers, bool voting ) {
      //validate input
      if ( proxy ) {
         fscio_assert( producers.size() == 0, "cannot vote for producers and proxy at same time" );
         fscio_assert( voter_name != proxy, "cannot proxy to self" );
         require_recipient( proxy );
      } else {
         fscio_assert( producers.size() <= 30, "attempt to vote for too many producers" );
         for( size_t i = 1; i < producers.size(); ++i ) {
            fscio_assert( producers[i-1] < producers[i], "producer votes must be unique and sorted" );
         }
      }

      auto voter = _voters.find(voter_name);
      fscio_assert( voter != _voters.end(), "user must stake before they can vote" ); /// staking creates voter object
      fscio_assert( !proxy || !voter->is_proxy, "account registered as a proxy is not allowed to use a proxy" );
      ...
      if ( voter->last_vote_weight > 0 ) {
         if( voter->proxy ) {
            ...
            fscio_assert( old_proxy != _voters.end(), "old proxy not found" ); //data corruption
      ...
      if( proxy ) {
         ...
         fscio_assert( new_proxy != _voters.end(), "invalid proxy specified" ); //if ( !voting ) { data corruption } else { wrong vote }
         fscio_assert( !voting || new_proxy->is_proxy, "proxy not found" );
      ...
      for( const auto& pd : producer_deltas ) {
         ...
         if( pitr != _producers.end() ) {
            fscio_assert( !voting || pitr->active() || !pd.second.second /* not from new set */, "producer is not currently registered" );
            _producers.modify( pitr, 0, [&]( auto& p ) {
               ...
               //fscio_assert( p.total_votes >= 0, "something bad happened" );
            });
         } else {
            fscio_assert( !pd.second.second /* not from new set */, "producer is not registered" ); //data corruption
         }
      }
   ...
   void system_contract::regproxy( const account_name proxy, bool isproxy ) {
      ...
      if ( pitr != _voters.end() ) {
         fscio_assert( isproxy != pitr->is_proxy, "action has no effect" );
         fscio_assert( !isproxy || !pitr->proxy, "account that uses a proxy is not allowed to become a proxy" );
   ...
   void system_contract::propagate_weight_change( const voter_info& voter ) {
      fscio_assert( voter.proxy == 0 || !voter.is_proxy, "account registered as a proxy is not allowed to use a proxy" );
