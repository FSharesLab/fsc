...
namespace proxy {
   ...
   namespace configs {
      ...
      bool get(config &out, const account_name &self) {
         ...
         if (it != -1) {
            ...
            fscio_assert(size == sizeof(config), "Wrong record size");
            return true;
         } else {
            return false;
         }
   ...
   void apply_transfer(uint64_t receiver, account_name /* code */, const T& transfer) {
      ...
      fscio_assert(get_res, "Attempting to use unconfigured proxy");
      if (transfer.from == self) {
         fscio_assert(transfer.to == code_config.owner,  "proxy may only pay its owner" );
      } else {
         fscio_assert(transfer.to == self, "proxy is not involved in this transfer");
         ...
         transaction out;
         out.actions.emplace_back(permission_level{self, N(active)}, N(fscio.token), N(transfer), new_transfer);
   ...
   void apply_setowner(uint64_t receiver, set_owner params) {
      ...
      fscio::print("Setting owner to: ", name{params.owner}, " with delay: ", params.delay, "\n");
      configs::store(code_config, self);
   }

   template<size_t ...Args>
   void apply_onerror(uint64_t receiver, const onerror& error ) {
      fscio::print("starting onerror\n");
      const auto self = receiver;
      config code_config;
      fscio_assert(configs::get(code_config, self), "Attempting use of unconfigured proxy");

      auto id = code_config.next_id++;
      configs::store(code_config, self);

      fscio::print("Resending Transaction: ", error.sender_id, " as ", id, "\n");
...
extern "C" {
    ...
    void apply( uint64_t receiver, uint64_t code, uint64_t action ) {
      if( code == N(fscio) && action == N(onerror) ) {
         apply_onerror( receiver, onerror::from_current_action() );
      } else if( code == N(fscio.token) ) {
         if( action == N(transfer) ) {
            apply_transfer(receiver, code, unpack_action_data<fscio::token::transfer_args>());
         }
