...
#include <fsciolib/vector.hpp>

namespace identity {
   using fscio::action_meta;
      ...
      public:
        ...
        using identity_base::identity_base;
         ...
         void create( const account_name creator, const uint64_t identity ) {
            ...
            fscio_assert( itr == t.end(), "identity already exists" );
            fscio_assert( identity != 0, "identity=0 is not allowed" );
                               ...
                               const vector<certvalue>& values )
         {
            ...
            idents_table t( _self, _self );
            fscio_assert( t.find( identity ) != t.end(), "identity does not exist" );
            ...
            for( const auto& value : values ) {
               ...
               if (value.confidence) {
                  fscio_assert(value.type.size() <= 32, "certrow::type should be not longer than 32 bytes");
                  ...
                  if (value.property == N(owner)) {
                     fscio_assert(sizeof(account_name) == value.data.size(), "data size doesn't match account_name size");
...
FSCIO_ABI( identity::contract, (create)(certprop)(settrust) );
