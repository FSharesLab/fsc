   ...
   void apply( uint64_t /* receiver */, uint64_t code, uint64_t action ) {
       ...
       if( code == N(asserter) ) {
          if( action == N(procassert) ) {
             assertdef def = fscio::unpack_action_data<assertdef>();

             // maybe assert?
             fscio_assert((uint32_t)def.condition, def.message.c_str());
          } else if( action == N(provereset) ) {
             fscio_assert(global_variable == 45, "Global Variable Initialized poorly");
             global_variable = 100;
          }
       }
    }
