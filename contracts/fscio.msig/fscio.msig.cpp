...
void multisig::propose() {
   ...
   fscio_assert( trx_header.expiration >= fscio::time_point_sec(now()), "transaction expired" );
   //fscio_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, proposer );
   fscio_assert( proptable.find( proposal_name ) == proptable.end(), "proposal with the same name exists" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      fscio_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < fscio::time_point_sec(now()), "cannot cancel until expiration" );
   }
...
FSCIO_ABI( fscio::multisig, (propose)(approve)(unapprove)(cancel)(exec) )
