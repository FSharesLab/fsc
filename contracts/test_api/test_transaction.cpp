...
void copy_data(char* data, size_t data_len, fscio::vector<char>& data_out) {
...
void test_transaction::send_action() {
   ...
   action act(fscio::vector<permission_level>{{N(testapi), N(active)}}, test_action);
   act.send();
}
...
void test_transaction::send_action_large() {
   ...
   act.send();
   fscio_assert(false, "send_message_large() should've thrown an error");
}
...
void test_transaction::test_tapos_block_prefix() {
   ...
   fscio_assert( tbp == tapos_block_prefix(), "tapos_block_prefix does not match" );
}
...
void test_transaction::test_tapos_block_num() {
   ...
   fscio_assert( tbn == tapos_block_num(), "tapos_block_num does not match" );
}
...
void test_transaction::test_read_transaction() {
   ...
   fscio_assert( size == read, "read_transaction failed");
   sha256(buf, read, &h);
   printhex( &h, sizeof(h) );
}
...
void test_transaction::test_transaction_size() {
   ...
   fscio_assert( trans_size == transaction_size(), "transaction size does not match" );
}
...
void test_transaction::send_transaction_empty(uint64_t receiver, uint64_t, uint64_t) {
   ...
   fscio_assert(false, "send_transaction_empty() should've thrown an error");
}
...
void test_transaction::assert_false_error_handler(const fscio::transaction& dtrx) {
   fscio_assert(dtrx.actions.size() == 1, "transaction should only have one action");
   fscio_assert(dtrx.actions[0].account == N(testapi), "transaction has wrong code");
   fscio_assert(dtrx.actions[0].name == WASM_TEST_ACTION("test_action", "assert_false"), "transaction has wrong name");
   fscio_assert(dtrx.actions[0].authorization.size() == 1, "action should only have one authorization");
   fscio_assert(dtrx.actions[0].authorization[0].actor == N(testapi), "action's authorization has wrong actor");
   fscio_assert(dtrx.actions[0].authorization[0].permission == N(active), "action's authorization has wrong permission");
}
...
void test_transaction::send_transaction_large(uint64_t receiver, uint64_t, uint64_t) {
   ...
   fscio_assert(false, "send_transaction_large() should've thrown an error");
}
...
void test_transaction::cancel_deferred_transaction_success() {
   ...
   fscio_assert( (bool)r, "transaction was not found" );
}

void test_transaction::cancel_deferred_transaction_not_found() {
   using namespace fscio;
   auto r = cancel_deferred( 0xffffffffffffffff ); //use the same id (0) as in send_deferred_transaction
   fscio_assert( !r, "transaction was canceled, whild should not be found" );
}
...
void test_transaction::send_cf_action_fail() {
   ...
   act.send_context_free();
   fscio_assert(false, "send_cfa_action_fail() should've thrown an error");
}
...
void test_transaction::new_feature() {
   fscio_assert(false == is_feature_active((int64_t)N(newfeature)), "we should not have new features unless hardfork");
}
