...
struct check_auth_msg {
   ...
   FSCLIB_SERIALIZE( check_auth_msg, (account)(permission)(pubkeys)  )
};
...
struct test_permission_last_used_msg {
   ...
   FSCLIB_SERIALIZE( test_permission_last_used_msg, (account)(permission)(last_used_time) )
};
...
void test_permission::test_permission_last_used(uint64_t /* receiver */, uint64_t code, uint64_t action) {
   ...
   fscio_assert( get_permission_last_used(params.account, params.permission) == params.last_used_time, "unexpected last used permission time" );
}
...
void test_permission::test_account_creation_time(uint64_t /* receiver */, uint64_t code, uint64_t action) {
   ...
   fscio_assert( get_account_creation_time(params.account) == params.last_used_time, "unexpected account creation time" );
}
