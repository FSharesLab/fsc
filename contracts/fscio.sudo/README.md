...
Execute a transaction while bypassing regular authorization checks (requires authorization of fscio.sudo which needs to be a privileged account).

### fscio.sudo::exec    executer trx
...
## 2. Installing the fscio.sudo contract

The fscio.sudo contract needs to be installed on a privileged account to function. It is recommended to use the account `fscio.sudo`.

First, the account `fscio.sudo` needs to be created. Since it has the restricted `fscio.` prefix, only a privileged account can create this account. So this guide will use the `fscio` account to create the `fscio.sudo` account. On typical live blockchain configurations, the `fscio` account can only be controlled by a supermajority of the current active block producers. So, this guide will use the `fscio.msig` contract to help coordinate the approvals of the proposed transaction that creates the `fscio.sudo` account.

The `fscio.sudo` account also needs to have sufficient RAM to host the contract and sufficient CPU and network bandwidth to deploy the contract. This means that the creator of the account (`fscio`) needs to gift sufficient RAM to the new account and delegate (preferably with transfer) sufficient bandwidth to the new account. To pull this off the `fscio` account needs to have enough of the core system token (the `SYS` token will be used within this guide) in its liquid balance. So prior to continuing with the next steps of this guide, the active block producers of the chain who are coordinating this process need to ensure that a sufficient amount of core system tokens that they are authorized to spend is placed in the liquid balance of the `fscio` account.

This guide will be using clfsc to carry out the process.

### 2.1 Create the fscio.sudo account

#### 2.1.1 Generate the transaction to create the fscio.sudo account

The transaction to create the `fscio.sudo` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the clfsc command to propose the transaction to the fscio.msig contract.
...
```
$ clfsc system newaccount -s -j -d --transfer --stake-net "1.000 SYS" --stake-cpu "1.000 SYS" --buy-ram-kbytes 50 fscio fscio.sudo FSC8MMUW11TAdTDxqdSwSqJodefSoZbFhcprndomgLi9MeR2o8MT4 > generated_account_creation_trx.json
726964ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea305500c80000"} arg: {"code":"fscio","action":"buyrambytes","args":{"payer":"fscio","receiver":"fscio.sudo","bytes":51200}}
726967ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea3055102700000000000004535953000000001027000000000000045359530000000001"} arg: {"code":"fscio","action":"delegatebw","args":{"from":"fscio","receiver":"fscio.sudo","stake_net_quantity":"1.0000 SYS","stake_cpu_quantity":"1.0000 SYS","transfer":true}}
...
Fourth, generate a transaction containing the `fscio::setpriv` action which will make the `fscio.sudo` account privileged:
```
$ clfsc push action -s -j -d fscio setpriv '{"account": "fscio.sudo", "is_priv": 1}' -p fscio > generated_setpriv_trx.json
...
Next, the action JSONs of the previously generated transactions will be used to construct a unified transaction which will eventually be proposed with the fscio.msig contract. A good way to get started is to make a copy of the generated_newaccount_trx.json file (call the copied file create_sudo_account_trx.json) and edit the first three fields so it looks something like the following:
```
...
#### 2.1.2 Propose the transaction to create the fscio.sudo account
...
```
$ clfsc multisig propose_trx createsudo producer_permissions.json create_sudo_account_trx.json blkproducera
executed transaction: bf6aaa06b40e2a35491525cb11431efd2b5ac94e4a7a9c693c5bf0cfed942393  744 bytes  772 us
#    fscio.msig <= fscio.msig::propose          {"proposer":"blkproducera","proposal_name":"createsudo","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.1.3 Review and approve the transaction to create the fscio.sudo account
...
```
$ clfsc multisig review blkproducera createsudo > create_sudo_account_trx_to_review.json
...
```
$ clfsc multisig propose_trx -j -s -d createsudo '[]' create_sudo_account_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_create_sudo_trx_serialized.hex
...
```
$ clfsc multisig approve blkproducera createsudo '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: 03a907e2a3192aac0cd040c73db8273c9da7696dc7960de22b1a479ae5ee9f23  128 bytes  472 us
#    fscio.msig <= fscio.msig::approve          {"proposer":"blkproducera","proposal_name":"createsudo","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.1.4 Execute the transaction to create the fscio.sudo account

When the necessary approvals are collected (in this example, with 21 block producers, at least 15 of their approvals were required), anyone can push the `fscio.msig::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the deferred transaction that is generated by the fscio.msig contract).

```
$ clfsc multisig exec blkproducera createsudo blkproducera
executed transaction: 7ecc183b99915cc411f96dde7c35c3fe0df6e732507f272af3a039b706482e5a  160 bytes  850 us
#    fscio.msig <= fscio.msig::exec             {"proposer":"blkproducera","proposal_name":"createsudo","executer":"blkproducera"}
warning: transaction executed locally, but may not be confirmed by the network yet
```

Anyone can now verify that the `fscio.sudo` was created:
```
$ clfsc get account fscio.sudo
privileged: true
permissions:
...
### 2.2 Deploy the fscio.sudo contract

#### 2.2.1  Generate the transaction to deploy the fscio.sudo contract

The transaction to deploy the contract to the `fscio.sudo` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the clfsc command to propose the transaction to the fscio.msig contract.

The easy way to generate this transaction is using clfsc:
```
$ clfsc set contract -s -j -d fscio.sudo contracts/fscio.sudo/ > deploy_sudo_contract_trx.json
Reading WAST/WASM from contracts/fscio.sudo/fscio.sudo.wasm...
...
#### 2.2.2 Propose the transaction to deploy the fscio.sudo contract
...
```
$ clfsc multisig propose_trx deploysudo producer_permissions.json deploy_sudo_contract_trx.json blkproducera
executed transaction: 9e50dd40eba25583a657ee8114986a921d413b917002c8fb2d02e2d670f720a8  4312 bytes  871 us
#    fscio.msig <= fscio.msig::propose          {"proposer":"blkproducera","proposal_name":"deploysudo","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.2.3 Review and approve the transaction to deploy the fscio.sudo contract

Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.

The proposed transaction can be reviewed using the `clfsc multisig review` command:
```
$ clfsc multisig review blkproducera deploysudo > deploy_sudo_contract_trx_to_review.json
...
```
$ clfsc multisig propose_trx -j -s -d deploysudo '[]' deploy_sudo_contract_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_deploy_sudo_trx_serialized.hex
...
When an approver (e.g. `blkproducerb`) is satisfied with the proposed transaction, they can simply approve it:
```
$ clfsc multisig approve blkproducera deploysudo '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: d1e424e05ee4d96eb079fcd5190dd0bf35eca8c27dd7231b59df8e464881abfd  128 bytes  483 us
#    fscio.msig <= fscio.msig::approve          {"proposer":"blkproducera","proposal_name":"deploysudo","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.2.4 Execute the transaction to create the fscio.sudo account

When the necessary approvals are collected (in this example, with 21 block producers, at least 15 of their approvals were required), anyone can push the `fscio.msig::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the deferred transaction that is generated by the fscio.msig contract).

```
$ clfsc multisig exec blkproducera deploysudo blkproducera
executed transaction: e8da14c6f1fdc3255b5413adccfd0d89b18f832a4cc18c4324ea2beec6abd483  160 bytes  1877 us
#    fscio.msig <= fscio.msig::exec             {"proposer":"blkproducera","proposal_name":"deploysudo","executer":"blkproducera"}
```

Anyone can now verify that the `fscio.sudo` contract was deployed correctly.

```
$ clfsc get code -a retrieved-fscio.sudo.abi fscio.sudo
code hash: 1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c
saving abi to retrieved-fscio.sudo.abi
$ sha256sum contracts/fscio.sudo/fscio.sudo.wasm
1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c  contracts/fscio.sudo/fscio.sudo.wasm
```

If the two hashes match then the local WebAssembly code is the one deployed on the blockchain. The retrieved ABI, which was stored in the file retrieved-fscio.sudo.abi, can then be compared to the original ABI of the contract (contracts/fscio.sudo/fscio.sudo.abi) to ensure they are semantically the same.

## 3. Using the fscio.sudo contract

### 3.1 Example: Updating owner authority of an arbitrary account

This example will demonstrate how to use the deployed fscio.sudo contract together with the fscio.msig contract to allow a greater than two-thirds supermajority of block producers of an FSCIO blockchain to change the owner authority of an arbitrary account. The example will use clfsc: in particular, the `clfsc multisig` command, the `clfsc set account permission` sub-command, and the `clfsc sudo exec` sub-command. However, the guide also demonstrates what to do if the `clfsc sudo exec` sub-command is not available.
...
The next step is to generate the transaction containing the `fscio.sudo::exec` action. This action will contain the transaction in update_alice_owner_trx.json as part of its action payload data.

```
$ clfsc sudo exec -s -j -d blkproducera update_alice_owner_trx.json > sudo_update_alice_owner_trx.json
```
...
If the `clfsc sudo` command is not available, there is an alternative way to generate the above transaction. There is no need to continue reading the remaining of sub-section 3.1.1 if the sudo_update_alice_owner_trx.json file was already generated with content similar to the above using the `clfsc sudo exec` sub-command method.
...
Then generate the template for the transaction containing the `fscio.sudo::exec` action:
```
$ clfsc push action -s -j -d fscio.sudo exec '{"executer": "blkproducera", "trx": ""}' > sudo_update_alice_owner_trx.json
$ cat sudo_update_alice_owner_trx.json
{
...
```
$ clfsc multisig propose_trx updatealice producer_permissions.json sudo_update_alice_owner_trx.json blkproducera
...
Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.
```
$ clfsc multisig review blkproducera updatealice > sudo_update_alice_owner_trx_to_review.json
...
```
$ clfsc multisig propose_trx -j -s -d updatealice '[]' sudo_update_alice_owner_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_sudo_update_alice_owner_trx_serialized.hex
