...
void tic_tac_toe::create(const account_name& challenger, const account_name& host) {
   require_auth(host);
   fscio_assert(challenger != host, "challenger shouldn't be the same as host");

   // Check if game already exists
   games existing_host_games(_self, host);
   auto itr = existing_host_games.find( challenger );
   fscio_assert(itr == existing_host_games.end(), "game already exists");
...
void tic_tac_toe::restart(const account_name& challenger, const account_name& host, const account_name& by) {
   ...
   games existing_host_games(_self, host);
   auto itr = existing_host_games.find( challenger );
   fscio_assert(itr != existing_host_games.end(), "game doesn't exists");

   // Check if this game belongs to the action sender
   fscio_assert(by == itr->host || by == itr->challenger, "this is not your game!");
...
void tic_tac_toe::move(const account_name& challenger, const account_name& host, const account_name& by, const uint16_t& row, const uint16_t& column ) {
   ...
   fscio_assert(itr->winner == N(none), "the game has ended!");
   // Check if this game belongs to the action sender
   fscio_assert(by == itr->host || by == itr->challenger, "this is not your game!");
   // Check if this is the  action sender's turn
   fscio_assert(by == itr->turn, "it's not your turn yet!");


   // Check if user makes a valid movement
   fscio_assert(is_valid_movement(row, column, itr->board), "not a valid movement!");
...
FSCIO_ABI( tic_tac_toe, (create)(restart)(close)(move))
