...
namespace fscio {
   ...
   public:
      ...
      FSCLIB_SERIALIZE( transaction_header, (expiration)(ref_block_num)(ref_block_prefix)(net_usage_words)(max_cpu_usage_ms)(delay_sec) )
   };
   ...
   inline action get_action( uint32_t type, uint32_t index ) {
      ...
      fscio_assert( s > 0, "get_action size failed" );
      size_t size = static_cast<size_t>(s);
      char* buffer = (char*)( max_stack_buffer_size < size ? malloc(size) : alloca(size) );
      auto size2 = ::get_action( type, index, buffer, size );
      fscio_assert( size == static_cast<size_t>(size2), "get_action failed" );
