...
namespace fscio {
   ...
   struct action {
      ...
      void send_context_free() const {
         fscio_assert( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
      ...
      template<typename T>
      T data_as() {
         fscio_assert( name == T::get_name(), "Invalid name" );
         fscio_assert( account == T::get_account(), "Invalid account" );
...
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::fscio::string_to_name(#NAME) )
 ...
 * @param NAME - The name of the action
 */
#define ACTION( CODE, NAME ) struct NAME : ::fscio::action_meta<CODE, ::fscio::string_to_name(#NAME) >
