...
#define N(X) ::fscio::string_to_name(#X)
namespace fscio {
...
// Helper macro for FSCIO_API
#define FSCIO_API_CALL( r, OP, elem ) \
   case ::fscio::string_to_name( BOOST_PP_STRINGIZE(elem) ): \
      fscio::execute_action( &thiscontract, &OP::elem ); \
      break;

// Helper macro for FSCIO_ABI
#define FSCIO_API( TYPE,  MEMBERS ) \
   BOOST_PP_SEQ_FOR_EACH( FSCIO_API_CALL, TYPE, MEMBERS )
 ...
 * To be able to use this macro, the contract needs to be derived from fscio::contract
 * 
 ...
 * Example:
 * @code
 * FSCIO_ABI( fscio::bios, (setpriv)(setalimits)(setglimits)(setprods)(reqauth) )
 * @endcode
 */
#define FSCIO_ABI( TYPE, MEMBERS ) \
extern "C" { \
   void apply( uint64_t receiver, uint64_t code, uint64_t action ) { \
      auto self = receiver; \
      if( action == N(onerror)) { \
         /* onerror is only valid if it is for the "fscio" code account and authorized by "fscio"'s "active permission */ \
         fscio_assert(code == N(fscio), "onerror action's are only valid from the \"fscio\" system account"); \
      } \
      if( code == self || action == N(onerror) ) { \
         TYPE thiscontract( self ); \
         switch( action ) { \
            FSCIO_API( TYPE, MEMBERS ) \
         } \
