...
#include <fsciolib/fixed_key.hpp>
 ...
 *       FSCLIB_SERIALIZE( record, (primary)(secondary) )
 *     };
 *    public:
 ...
 *
 *
 *  FSCIO Multi-Index API provides a C++ interface to the FSCIO database. It is patterned after Boost Multi Index Container.
 ...
 *      FSCLIB_SERIALIZE( record, (primary)(secondary_1)(secondary_2)(secondary_3)(secondary_4)(secondary_5) )
 *    };
 *    public:
 *      mycontract( account_name self ):contract(self){}
 *      void myaction() {
...
class multi_index
{
   private:
      ...
      struct index {
         public:
            ...
            struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
               public:
                  ...
                  const_iterator& operator++() {
                     ...
                     fscio_assert( _item != nullptr, "cannot increment end iterator" );
                  ...
                  const_iterator& operator--() {
                     ...
                     if( !_item ) {
                        ...
                        fscio_assert( ei != -1, "cannot decrement end iterator when the index is empty" );
                        prev_itr = secondary_index_db_functions<secondary_key_type>::db_idx_previous( ei , &prev_pk );
                        fscio_assert( prev_itr >= 0, "cannot decrement end iterator when the index is empty" );
                     } else {
                        if( _item->__iters[Number] == -1 ) {
                           secondary_key_type temp_secondary_key;
                           auto idxitr = secondary_index_db_functions<secondary_key_type>::db_idx_find_primary(_idx->get_code(), _idx->get_scope(), _idx->name(), _item->primary_key(), temp_secondary_key);
                        ...
                        fscio_assert( prev_itr >= 0, "cannot decrement iterator at beginning of index" );
                     }
            ...
            const_iterator require_find( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               auto lb = lower_bound( secondary );
               fscio_assert( lb != cend(), error_msg );
               fscio_assert( secondary == secondary_extractor_type()(*lb), error_msg );
               return lb;
            }
            ...
            const T& get( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               ...
               fscio_assert( result != cend(), error_msg );
               return *result;
            }
            ...
            const_iterator iterator_to( const T& obj ) {
               ...
               fscio_assert( objitem.__idx == _multidx, "object passed to iterator_to is not in multi_index" );
            ...
            void modify( const_iterator itr, uint64_t payer, Lambda&& updater ) {
               fscio_assert( itr != cend(), "cannot pass end iterator to modify" );

               _multidx->modify( *itr, payer, std::forward<Lambda&&>(updater) );
            }

            const_iterator erase( const_iterator itr ) {
               fscio_assert( itr != cend(), "cannot pass end iterator to erase" );
      ...
      const item& load_object_by_primary_iterator( int32_t itr )const {
         ...
         fscio_assert( size >= 0, "error reading iterator" );
   ...
   public:
       ...
       *
       *  Notes
       *  The `fscio::multi_index` template has template parameters `<uint64_t TableName, typename T, typename... Indices>`, where:
       ...
       *       FSCLIB_SERIALIZE( address, (account_name)(first_name)(last_name)(street)(city)(state) )
       *    };
       *    public:
       *      addressbook(account_name self):contract(self) {}
       *      typedef fscio::multi_index< N(address), address > address_index;
       *      void myaction() {
       ...
       *        fscio_assert(addresses.get_code() == N(dan), "Codes don't match.");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *      void myaction() {
       *        address_index addresses(N(dan), N(dan)); // code, scope
       *        fscio_assert(addresses.get_code() == N(dan), "Scopes don't match");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
         ...
         const_iterator& operator--() {
            ...
            if( !_item ) {
               ...
               fscio_assert( ei != -1, "cannot decrement end iterator when the table is empty" );
               prev_itr = db_previous_i64( ei , &prev_pk );
               fscio_assert( prev_itr >= 0, "cannot decrement end iterator when the table is empty" );
            } else {
               prev_itr = db_previous_i64( _item->__primary_itr, &prev_pk );
               fscio_assert( prev_itr >= 0, "cannot decrement iterator at beginning of table" );
            }
       ...
       *        fscio_assert(itr == addresses.cbegin(), "Only address is not at front.");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        fscio_assert(itr == addresses.begin(), "Only address is not at front.");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        fscio_assert(itr != addresses.cend(), "Address for account doesn't exist");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        fscio_assert(itr != addresses.end(), "Address for account doesn't exist");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        fscio_assert(itr->account_name == N(dan), "Incorrect Last Record ");
       *        itr++;
       *        fscio_assert(itr->account_name == N(brendan), "Incorrect Second Last Record");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        itr--;
       *        fscio_assert(itr->account_name == N(brendan), "Incorrect First Record ");
       *        itr--;
       *        fscio_assert(itr->account_name == N(dan), "Incorrect Second Record");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *       FSCLIB_SERIALIZE( address, (account_name)(first_name)(last_name)(street)(city)(state)(zip) )
       *    };
       *    public:
       *      addressbook(account_name self):contract(self) {}
       *      typedef fscio::multi_index< N(address), address, indexed_by< N(zip), const_mem_fun<address, uint64_t, &address::by_zip> > address_index;
       *      void myaction() {
       *        address_index addresses(_self, _self);  // code, scope
       *        // add to table, first argument is account to bill for storage
       *        addresses.emplace(payer, [&](auto& address) {
       ...
       *        fscio_assert(itr->account_name == N(brendan), "Incorrect First Lower Bound Record ");
       *        itr++;
       *        fscio_assert(itr->account_name == N(dan), "Incorrect Second Lower Bound Record");
       *        itr++;
       *        fscio_assert(itr == zip_index.end(), "Incorrect End of Iterator");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        fscio_assert(itr->account_name == N(dan), "Incorrect First Upper Bound Record ");
       *        itr++;
       *        fscio_assert(itr == zip_index.end(), "Incorrect End of Iterator");
       *      }
       *  }
       ...
       *       FSCLIB_SERIALIZE( address, (key)(first_name)(last_name)(street)(city)(state) )
       *    };
       *    public:
       *      addressbook(account_name self):contract(self) {}
       *      typedef fscio::multi_index< N(address), address > address_index;
      ...
      uint64_t available_primary_key()const {
         ...
         fscio_assert( _next_primary_key < no_available_primary_key, "next primary key in table is at autoincrement limit");
       ...
       *        fscio_assert(itr->account_name == N(dan), "Incorrect Record ");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        fscio_assert(iterator_to(user) == itr, "Invalid iterator");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      const_iterator iterator_to( const T& obj )const {
         const auto& objitem = static_cast<const item&>(obj);
         fscio_assert( objitem.__idx == this, "object passed to iterator_to is not in multi_index" );
         return {this, &objitem};
      }
      /**
      ...
      const_iterator emplace( uint64_t payer, Lambda&& constructor ) {
         ...
         fscio_assert( _code == current_receiver(), "cannot create objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        fscio_assert(itr != addresses.end(), "Address for account not found");
      ...
      template<typename Lambda>
      void modify( const_iterator itr, uint64_t payer, Lambda&& updater ) {
         fscio_assert( itr != end(), "cannot pass end iterator to modify" );
       ...
       *          address.state = "CA";
       *        });
       *        fscio_assert(itr->city == "San Luis Obispo", "Address not modified");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      void modify( const T& obj, uint64_t payer, Lambda&& updater ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         fscio_assert( objitem.__idx == this, "object passed to modify is not in multi_index" );
         auto& mutableitem = const_cast<item&>(objitem);
         fscio_assert( _code == current_receiver(), "cannot modify objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         fscio_assert( pk == obj.primary_key(), "updater cannot change primary key when modifying an object" );
       ...
       *        });
       *        auto user = addresses.get(N(dan));
       *        fscio_assert(user.first_name == "Daniel", "Couldn't get him.");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        fscio_assert(itr != addresses.end(), "Couldn't get him.");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        fscio_assert(itr != addresses.end(), "Address for account not found");
       *        addresses.erase( itr );
       *        fscio_assert(itr != addresses.end(), "Address not erased properly");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
       */
      const_iterator erase( const_iterator itr ) {
         fscio_assert( itr != end(), "cannot pass end iterator to erase" );
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        fscio_assert(itr != addresses.end(), "Record is not found");
       *        addresses.erase(*itr);
       *        itr = addresses.find(N(dan));
       *        fscio_assert(itr == addresses.end(), "Record is not deleted");
       *      }
       *  }
       *  FSCIO_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      void erase( const T& obj ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         fscio_assert( objitem.__idx == this, "object passed to erase is not in multi_index" );
         fscio_assert( _code == current_receiver(), "cannot erase objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         fscio_assert( itr2 != _items_vector.rend(), "attempt to remove object that was not in multi_index" );
