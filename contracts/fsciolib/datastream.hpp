...
#include <fsciolib/memory.h>
#include <fsciolib/vector.hpp>
#include <boost/container/flat_set.hpp>
#include <boost/container/flat_map.hpp>
#include <fsciolib/varint.hpp>
#include <array>
#include <set>
#include <map>
#include <string>
   ...
   public:
      ...
      */
      inline bool read( char* d, size_t s ) {
        fscio_assert( size_t(_end - _pos) >= (size_t)s, "read" );
        memcpy( d, _pos, s );
        _pos += s;
        return true;
      }
      ...
      */
      inline bool write( const char* d, size_t s ) {
        fscio_assert( _end - _pos >= (int32_t)s, "write" );
...
DataStream& operator >> ( DataStream& ds, T (&v)[N] ) {
   unsigned_int s;
   ds >> s;
   fscio_assert( N == s.value, "T[] size and unpacked size don't match");
   for( uint32_t i = 0; i < N; ++i )
      ds >> v[i];
   return ds;
}
