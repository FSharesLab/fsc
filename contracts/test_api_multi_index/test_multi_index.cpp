...
namespace _test_multi_index {
   ...
   struct record_idx64 {
      ...
      FSCLIB_SERIALIZE( record_idx64, (id)(sec) )
   };
   ...
   struct record_idx128 {
      ...
      FSCLIB_SERIALIZE( record_idx128, (id)(sec) )
   };
   ...
   struct record_idx256 {
      ...
      FSCLIB_SERIALIZE( record_idx256, (id)(sec) )
   };
   ...
   struct record_idx_double {
      ...
      FSCLIB_SERIALIZE( record_idx_double, (id)(sec) )
   };
   ...
   struct record_idx_long_double {
      ...
      FSCLIB_SERIALIZE( record_idx_long_double, (id)(sec) )
   };
   ...
   void idx64_check_without_storing(uint64_t receiver)
   {
      ...
      // find by primary key
      {
         ...
         fscio_assert(itr == table.end(), "idx64_general - table.find() of non-existing primary key");

         itr = table.find(976);
         fscio_assert(itr != table.end() && itr->sec == N(emily), "idx64_general - table.find() of existing primary key");

         ++itr;
         fscio_assert(itr == table.end(), "idx64_general - increment primary iterator to end");
         
         itr = table.require_find(976);
         fscio_assert(itr != table.end() && itr->sec == N(emily), "idx64_general - table.require_find() of existing primary key");
      ...
      // iterate forward starting with charlie
      {
         ...
         fscio_assert(itr != secondary_index.end() && itr->sec == N(charlie), "idx64_general - secondary_index.lower_bound()");

         ++itr;
         fscio_assert(itr != secondary_index.end() && itr->id == 976 && itr->sec == N(emily), "idx64_general - increment secondary iterator");

         ++itr;
         fscio_assert(itr != secondary_index.end() && itr->id == 110 && itr->sec == N(joe), "idx64_general - increment secondary iterator again");

         ++itr;
         fscio_assert(itr == secondary_index.end(), "idx64_general - increment secondary iterator to end");
      }

      // iterate backward starting with second bob
      {
         auto pk_itr = table.find(781);
         fscio_assert(pk_itr != table.end() && pk_itr->sec == N(bob), "idx64_general - table.find() of existing primary key");

         auto itr = secondary_index.iterator_to(*pk_itr);
         fscio_assert(itr->id == 781 && itr->sec == N(bob), "idx64_general - iterator to existing object in secondary index");

         --itr;
         fscio_assert(itr != secondary_index.end() && itr->id == 540 && itr->sec == N(bob), "idx64_general - decrement secondary iterator");

         --itr;
         fscio_assert(itr != secondary_index.end() && itr->id == 650 && itr->sec == N(allyson), "idx64_general - decrement secondary iterator again");

         --itr;
         fscio_assert(itr == secondary_index.begin() && itr->id == 265 && itr->sec == N(alice), "idx64_general - decrement secondary iterator to beginning");
      }
      ...
      // iterate backward starting with emily using const_reverse_iterator
      {
         ...
         for( ; itr != secondary_index.rend(); ++itr ) {
            fscio_assert(pk_itr != pks.end(), "idx64_general - unexpected continuation of secondary index in reverse iteration");
            fscio_assert(*pk_itr == itr->id, "idx64_general - primary key mismatch in reverse iteration");
            ++pk_itr;
         }
         fscio_assert( pk_itr == pks.end(), "idx64_general - did not iterate backwards through secondary index properly" );
      }

      // require_find secondary key
      {
         auto itr = secondary_index.require_find(N(bob));
         fscio_assert(itr != secondary_index.end(), "idx64_general - require_find must never return end iterator");
         fscio_assert(itr->id == 540, "idx64_general - require_find test");
         
         ++itr;
         fscio_assert(itr->id == 781, "idx64_general - require_find secondary key test");
      }
      ...
      // modify and erase
      {
         ...
         fscio_assert(itr1 != table.end() && itr1->sec == N(billy), "idx64_general - table.modify()");

         table.erase(itr1);
         auto itr2 = table.find(ssn);
         fscio_assert( itr2 == table.end(), "idx64_general - table.erase()");
      }
   }
   ...
   void idx64_require_find_fail(uint64_t receiver)
   {
      ...
      fscio_assert(itr != table.end(), "table not loaded");
   ...
   void idx64_require_find_sk_fail(uint64_t receiver)
   {
      ...
      fscio_assert(itr != sec_index.end(), "table not loaded");
   ...
   void idx128_check_without_storing(uint64_t receiver)
   {
...

      {
         ...
         fscio_assert( itr->primary_key() == 0 && itr->get_secondary() == multiplier*0, "idx128_general - secondary key sort" );
         ++itr;
         fscio_assert( itr->primary_key() == 1 && itr->get_secondary() == multiplier*1, "idx128_general - secondary key sort" );
         ++itr;
         fscio_assert( itr->primary_key() == 2 && itr->get_secondary() == multiplier*2, "idx128_general - secondary key sort" );
         ++itr;
         fscio_assert( itr->primary_key() == 4 && itr->get_secondary() == multiplier*4, "idx128_general - secondary key sort" );
         ++itr;
         fscio_assert( itr->primary_key() == 3 && itr->get_secondary() == multiplier*6, "idx128_general - secondary key sort" );
         ++itr;
         fscio_assert( itr == secondary_index.end(), "idx128_general - secondary key sort" );
      }
...
void test_multi_index::idx128_autoincrement_test(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   {
      fscio_assert( r.primary_key() == expected_key, "idx128_autoincrement_test - unexpected primary key" );
      --expected_key;
   }
   fscio_assert( expected_key == static_cast<uint64_t>(-1), "idx128_autoincrement_test - did not iterate through secondary index properly" );

   auto itr = table.find(3);
   fscio_assert( itr != table.end(), "idx128_autoincrement_test - could not find object with primary key of 3" );
   ...
   */
   ...
   fscio_assert( table.available_primary_key() == 101, "idx128_autoincrement_test - next_primary_key was not correct after record modify" );
}
...
void test_multi_index::idx128_autoincrement_test_part1(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   for( const auto& r : table.get_index<N(bysecondary)>() )
   {
      fscio_assert( r.primary_key() == expected_key, "idx128_autoincrement_test_part1 - unexpected primary key" );
      --expected_key;
   }
   fscio_assert( expected_key == 0, "idx128_autoincrement_test_part1 - did not iterate through secondary index properly" );
...
void test_multi_index::idx128_autoincrement_test_part2(uint64_t receiver, uint64_t code, uint64_t action)
{
...

   {
      ...
      fscio_assert( table.available_primary_key() == 3, "idx128_autoincrement_test_part2 - did not recover expected next primary key");
   }
   ...
   for( const auto& r : table.get_index<N(bysecondary)>() )
   {
      fscio_assert( r.primary_key() == expected_key, "idx128_autoincrement_test_part2 - unexpected primary key" );
      --expected_key;
   }
   fscio_assert( expected_key == static_cast<uint64_t>(-1), "idx128_autoincrement_test_part2 - did not iterate through secondary index properly" );

   auto itr = table.find(3);
   fscio_assert( itr != table.end(), "idx128_autoincrement_test_part2 - could not find object with primary key of 3" );
   ...
   fscio_assert( table.available_primary_key() == 101, "idx128_autoincrement_test_part2 - next_primary_key was not correct after record update" );
}
...
void test_multi_index::idx256_general(uint64_t receiver, uint64_t code, uint64_t action)
{
...

   {
      ...
      fscio_assert( itr->primary_key() == 1 && itr->get_secondary() == fourtytwo, "idx256_general - primary key sort" );
      ++itr;
      fscio_assert( itr->primary_key() == 2 && itr->get_secondary() == onetwothreefour, "idx256_general - primary key sort" );
      ++itr;
      fscio_assert( itr->primary_key() == 3 && itr->get_secondary() == fourtytwo, "idx256_general - primary key sort" );
      ++itr;
      fscio_assert( itr == table.end(), "idx256_general - primary key sort" );
   }
   ...
   fscio_assert( lower1->id == 1, "idx256_general - lower_bound" );
   ...
   fscio_assert( lower2->id == 2, "idx256_general - lower_bound" );
...

   {
      ...
      fscio_assert( itr->primary_key() == 1, "idx256_general - secondary key sort" );
      ++itr;
      fscio_assert( itr->primary_key() == 3, "idx256_general - secondary key sort" );
      ++itr;
      fscio_assert( itr->primary_key() == 2, "idx256_general - secondary key sort" );
      ++itr;
      fscio_assert( itr == secidx.end(), "idx256_general - secondary key sort" );
   }
   ...
   fscio_assert( upper->id == 2, "idx256_general - upper_bound" );
   fscio_assert( upper->id == secidx.get(onetwothreefour).id, "idx256_general - secondary index get" );
...

   {
      ...
      fscio_assert( itr->primary_key() == 3 && itr->get_secondary() == fourtytwo, "idx256_general - primary key sort after remove" );
      ++itr;
      fscio_assert( itr->primary_key() == 2 && itr->get_secondary() == onetwothreefour, "idx256_general - primary key sort after remove" );
      ++itr;
      fscio_assert( itr == table.rend(), "idx256_general - primary key sort after remove" );
   }
}
...
void test_multi_index::idx_double_general(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   for( const auto& obj : secidx ) {
      fscio_assert( obj.primary_key() == expected_key, "idx_double_general - unexpected primary key" );

      double prod = obj.sec * obj.id;

      print(" id = ", obj.id, ", sec = ", obj.sec, ", sec * id = ", prod, "\n");

      fscio_assert( std::abs(prod - expected_product) <= tolerance,
                    "idx_double_general - product of secondary and id not equal to expected_product within tolerance" );

      --expected_key;
   }
   fscio_assert( expected_key == 0, "idx_double_general - did not iterate through secondary index properly" );

   {
      auto itr = secidx.lower_bound( expected_product / 5.5 );
      fscio_assert( std::abs(1.0 / itr->sec - 5000000.0) <= tolerance, "idx_double_general - lower_bound" );

      itr = secidx.upper_bound( expected_product / 5.0 );
      fscio_assert( std::abs(1.0 / itr->sec - 4000000.0) <= tolerance, "idx_double_general - upper_bound" );
...
void test_multi_index::idx_long_double_general(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   uint64_t expected_key = 10;
   for( const auto& obj : secidx ) {
      fscio_assert( obj.primary_key() == expected_key, "idx_long_double_general - unexpected primary key" );
   ...
   fscio_assert( expected_key == 0, "idx_long_double_general - did not iterate through secondary index properly" );

   {
      auto itr = secidx.lower_bound( expected_product / 5.5l );
      fscio_assert( std::abs(1.0l / itr->sec - 5000000.0l) <= tolerance, "idx_long_double_general - lower_bound" );

      itr = secidx.upper_bound( expected_product / 5.0l );
      fscio_assert( std::abs(1.0l / itr->sec - 4000000.0l) <= tolerance, "idx_long_double_general - upper_bound" );
...
void test_multi_index::idx64_pass_pk_ref_to_other_table(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   fscio_assert(table1_pk_itr != table1.end() && table1_pk_itr->sec == N(bob), "idx64_pass_pk_ref_to_other_table - table.find() of existing primary key");
...
void test_multi_index::idx64_pass_sk_ref_to_other_table(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   auto table1_pk_itr = table1.find(781);
   fscio_assert(table1_pk_itr != table1.end() && table1_pk_itr->sec == N(bob), "idx64_pass_sk_ref_to_other_table - table.find() of existing primary key");
...
void test_multi_index::idx64_modify_primary_key(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   auto pk_itr = table.find(781);
   fscio_assert(pk_itr != table.end() && pk_itr->sec == N(bob), "idx64_modify_primary_key - table.find() of existing primary key");
...
void test_multi_index::idx64_run_out_of_avl_pk(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   fscio_assert(table.available_primary_key() == static_cast<uint64_t>(-3), "idx64_run_out_of_avl_pk - incorrect available primary key");
...
void test_multi_index::idx64_sk_cache_pk_lookup(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   fscio_assert(sk_itr != sec_index.end() && sk_itr->id == 540, "idx64_sk_cache_pk_lookup - sec_index.find() of existing secondary key");

   auto pk_itr = table.iterator_to(*sk_itr);
   auto prev_itr = --pk_itr;
   fscio_assert(prev_itr->id == 265 && prev_itr->sec == N(alice), "idx64_sk_cache_pk_lookup - previous record");
}
...
void test_multi_index::idx64_pk_cache_sk_lookup(uint64_t receiver, uint64_t code, uint64_t action)
{
   ...
   fscio_assert(pk_itr != table.end() && pk_itr->sec == N(bob), "idx64_pk_cache_sk_lookup - table.find() of existing primary key");

   auto sec_index = table.get_index<N(bysecondary)>();
   auto sk_itr = sec_index.iterator_to(*pk_itr);
   auto next_itr = ++sk_itr;
   fscio_assert(next_itr->id == 781 && next_itr->sec == N(bob), "idx64_pk_cache_sk_lookup - next record");
}
