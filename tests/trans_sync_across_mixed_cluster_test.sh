...
killAll()
{
  programs/fscio-launcher/fscio-launcher -k 15
}

cleanup()
{
    rm -rf etc/fscio/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/clfsc/clfsc get info)"
  verifyErrorCode "clfsc get info"
...
# stand up nodfsc cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodfsc \"--plugin fscio::wallet_api_plugin\" $launcherOpts
programs/fscio-launcher/fscio-launcher --nodfsc "--plugin fscio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clfsc/clfsc --port $port2 get block 1)"
    verifyErrorCode "clfsc get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clfsc/clfsc create key)"
verifyErrorCode "clfsc create key"
...
PASSWORD_INITA="$(programs/clfsc/clfsc wallet create --name inita)"
verifyErrorCode "clfsc wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clfsc/clfsc wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clfsc wallet import"
programs/clfsc/clfsc wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clfsc wallet import"
programs/clfsc/clfsc wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clfsc wallet import"
programs/clfsc/clfsc wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clfsc wallet import"
...
ACCOUNT_INFO="$(programs/clfsc/clfsc create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clfsc create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clfsc/clfsc get account testera)"
verifyErrorCode "clfsc get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clfsc/clfsc transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clfsc transfer"
   ...
   TRANS_INFO="$(programs/clfsc/clfsc --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clfsc get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
