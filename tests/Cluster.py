...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="fscio-ignition-wd/bootlog.txt"
    __configDir="etc/fscio/"
    __dataDir="var/lib/"
    __fileDivider="================================================================="

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kfscd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="FSCtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kfscd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: fsc server host
        port: fsc server port
        walletHost: fsc wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures fsc mongo plugin
        ...
        self.fscioAccount=self.defProducerAccounts["fscio"]= Account("fscio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, extraNodfscArgs=None, useBiosBootFile=True, specificExtraNodfscArgs=None):
        """Launch cluster.
        ...
        extraNodfscArgs: string of arguments to pass through to each nodoes instance (via --nodfsc flag on launcher)
        ...
        specificExtraNodfscArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodfsc flags on launcher), example: { "5" : "--plugin fscio::test_control_api_plugin" }
        """
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodfsc default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        nodfscArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodfscArgs += " --plugin fscio::wallet_api_plugin"
        if self.enableMongo:
            nodfscArgs += " --plugin fscio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodfscArgs is not None:
            assert(isinstance(extraNodfscArgs, str))
            nodfscArgs += extraNodfscArgs
        if Utils.Debug:
            nodfscArgs += " --contracts-console"

        if nodfscArgs:
            cmdArr.append("--nodfsc")
            cmdArr.append(nodfscArgs)

        if specificExtraNodfscArgs is not None:
            assert(isinstance(specificExtraNodfscArgs, dict))
            for nodeNum,arg in specificExtraNodfscArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-nodfsc")
                cmdArr.append(arg)
        ...
        self.fscioAccount=self.defProducerAccounts["fscio"]
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on fsc server port %d" % (
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on fsc node port: %d" % (node.port))
                return False
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.fscioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse fscio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(totalNodes, biosHost, biosPort, walletMgr, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by fscio-launcher."""
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down fsc cluster.")
            return None
        ...
        fscioName="fscio"
        fscioKeys=producerKeys[fscioName]
        fscioAccount=Account(fscioName)
        fscioAccount.ownerPrivateKey=fscioKeys["private"]
        fscioAccount.ownerPublicKey=fscioKeys["public"]
        fscioAccount.activePrivateKey=fscioKeys["private"]
        fscioAccount.activePublicKey=fscioKeys["public"]
        producerKeys.pop(fscioName)

        if not walletMgr.importKey(fscioAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (fscioName))
            return None
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"fscio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init fscio transfer")
            opts="--permission fscio@active"
            if name != "fscio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from fscio.token to %s." % (name))
                    return None
    ...
    def bootstrap(totalNodes, prodCount, totalProducers, biosHost, biosPort, walletMgr, onlyBios=False):
        ...
        trans=biosNode.publishContract(fscioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, fscioAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission fscio@active"
                    myTrans=biosNode.pushMessage("fscio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("fscio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but fscio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "fscio"
            ret=Utils.waitForBool(lam)
            if not ret:
                Utils.Print("ERROR: Block production handover failed.")
                return None

        fscioTokenAccount=copy.deepcopy(fscioAccount)
        fscioTokenAccount.name="fscio.token"
        trans=biosNode.createAccount(fscioTokenAccount, fscioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (fscioTokenAccount.name))
            return None

        fscioRamAccount=copy.deepcopy(fscioAccount)
        fscioRamAccount.name="fscio.ram"
        trans=biosNode.createAccount(fscioRamAccount, fscioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (fscioRamAccount.name))
            return None

        fscioRamfeeAccount=copy.deepcopy(fscioAccount)
        fscioRamfeeAccount.name="fscio.ramfee"
        trans=biosNode.createAccount(fscioRamfeeAccount, fscioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (fscioRamfeeAccount.name))
            return None

        fscioStakeAccount=copy.deepcopy(fscioAccount)
        fscioStakeAccount.name="fscio.stake"
        trans=biosNode.createAccount(fscioStakeAccount, fscioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (fscioStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(fscioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=fscioTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (fscioTokenAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to fscio contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (fscioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to fscio contract.")
            return None
        ...
        Utils.Print("Verify fscio issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountFscBalanceStr(fscioAccount.name)
        if expectedAmount != actualAmount:
            Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                        (expectedAmount, actualAmount))
            return None

        contract="fscio.system"
        contractDir="contracts/%s" % (contract)
        wasmFile="%s.wasm" % (contract)
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (fscioAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (fscioAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (fscioTokenAccount.name, name))
                return None
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodfsc instances. allInstances will kill all nodfsc instances running on the system."""
        ...
        if allInstances:
            # ocassionally the launcher cannot kill the fsc server
            cmd="pkill -9 %s" % (Utils.FscServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down fsc cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodfsc instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodfsc instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
