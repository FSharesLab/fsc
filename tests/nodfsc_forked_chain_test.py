...
try:
    ...
    specificExtraNodfscArgs={}
    # producer nodes will be mapped to 0 through totalProducerNodes-1, so the number totalProducerNodes will be the non-producing node
    specificExtraNodfscArgs[totalProducerNodes]="--plugin fscio::test_control_api_plugin"
    ...
    if cluster.launch(prodCount=prodCount, onlyBios=False, topo="bridge", pnodes=totalProducerNodes,
                      ...
                      useBiosBootFile=False, specificExtraNodfscArgs=specificExtraNodfscArgs) is False:
        Utils.cmdError("launcher")
        Utils.errorExit("Failed to stand up fsc cluster.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.fscioAccount,accounts[0],accounts[1],accounts[2],accounts[3],accounts[4]])
    ...
    for account in accounts:
        ...
        trans=node.createInitializeAccount(account, cluster.fscioAccount, stakedDeposit=0, waitForTransBlock=True, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.fscioAccount.name, account.name))
        node.transferFunds(cluster.fscioAccount, account, transferAmount, "test transfer", waitForTransBlock=True)
