...
try:
    ...
    if not dontLaunch:
        ...
        if walletMgr.launch() is False:
            ...
            errorExit("Failed to stand up fsc walletd.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.fscioAccount,cluster.defproduceraAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.fscioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.fscioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.fscioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.fscioAccount, buyRAM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.fscioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.fscioAccount, buyRAM=1000000, waitForTransBlock=True, exitOnError=True)
