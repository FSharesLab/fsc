...
try:
    ...
    if not dontLaunch:
        walletAccounts.append(cluster.fscioAccount)
    ...
    contractDir="contracts/fscio.token"
    wasmFile="fscio.token.wasm"
    abiFile="fscio.token.abi"
    ...
    if not enableMongo:
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get FSC account failed", raw=True)
