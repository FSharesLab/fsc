...
# fscio should have the same key as defproducera
fscio = copy.copy(defproduceraAccount)
fscio.name = "fscio"
...
wasmFile="fscio.system.wasm"
abiFile="fscio.system.abi"
Print("\nPush system contract %s %s" % (wasmFile, abiFile))
trans=node0.publishContract(fscio.name, wasmFile, abiFile, waitForTransBlock=True)
if trans is None:
    Utils.errorExit("Failed to publish fscio.system.")
else:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, fscio)
