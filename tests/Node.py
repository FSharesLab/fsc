...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, walletMgr=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="FSCtest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def getBlock(self, blockNum, silentErrors=False, exitOnError=False):
        ...
        if not self.enableMongo:
            ...
            return self.processClfscCmd(cmd, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError, exitMsg=msg)
        else:
    ...
    def getTransaction(self, transId, silentErrors=False, exitOnError=False, delayedRetry=True):
        ...
        if not self.enableMongo:
            ...
            for i in range(0,(int(60/timeout) - 1)):
                trans=self.processClfscCmd(cmd, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnErrorForDelayed, exitMsg=msg)
    ...
    def createInitializeAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False, stakeNet=100, stakeCPU=100, buyRAM=10000, exitOnError=False):
        ...
        trans=self.processClfscCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getFscAccount(self, name, exitOnError=False):
        ...
        if not self.enableMongo:
            ...
            return self.processClfscCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg)
        else:
    ...
    def getTable(self, contract, scope, table, exitOnError=False):
        ...
        return self.processClfscCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getCurrencyBalance(self, contract, account, symbol=CORE_SYMBOL, exitOnError=False):
        ...
        return self.processClfscCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg, returnType=ReturnType.raw)
    ...
    # Get actions mapped to an account (clfsc get actions)
    ...
    def getAccountFscBalanceStr(self, scope):
        """Returns SYS currency0000 account balance from clfsc get table command. Returned balance is string following syntax "98.0311 SYS". """
        assert isinstance(scope, str)
        amount=self.getTableAccountBalance("fscio.token", scope)
        if Utils.Debug: Utils.Print("getNodeAccountFscBalance %s %s" % (scope, amount))
        assert isinstance(amount, str)
        return amount

    def getAccountFscBalance(self, scope):
        """Returns SYS currency0000 account balance from clfsc get table command. Returned balance is an integer e.g. 980311. """
    ...
    def setPermission(self, account, code, pType, requirement, waitForTransBlock=False, exitOnError=False):
        ...
        trans=self.processClfscCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError)
        self.trackCmdTransaction(trans)
    ...
    def delegatebw(self, fromAccount, netQuantity, cpuQuantity, toAccount=None, transferTo=False, waitForTransBlock=False, exitOnError=False):
        ...
        trans=self.processClfscCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
        self.trackCmdTransaction(trans)
    ...
    def processClfscCmd(self, cmd, cmdDesc, silentErrors=True, exitOnError=False, exitMsg=None, returnType=ReturnType.json):
    ...
    def getInfo(self, silentErrors=False, exitOnError=False):
        cmdDesc = "get info"
        info=self.processClfscCmd(cmdDesc, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError)
        if info is None:
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by clfsc get info."""
        if not self.enableMongo:
