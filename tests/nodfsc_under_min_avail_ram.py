...
###############################################################
# nodfsc_voting_test
# --dump-error-details <Upon error print etc/fscio/node_*/config.ini and var/lib/node_*/stderr.log to stdout>
...
try:
    ...
    maxRAMValue=1010
    extraNodfscArgs=" %s %d %s %d " % (minRAMFlag, minRAMValue, maxRAMFlag, maxRAMValue)
    if cluster.launch(onlyBios=False, pnodes=totalNodes, totalNodes=totalNodes, totalProducers=totalNodes, extraNodfscArgs=extraNodfscArgs, useBiosBootFile=False) is False:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.fscioAccount])
    ...
    # create accounts via fscio as otherwise a bid is needed
    for account in accounts:
        Print("Create new account %s via %s" % (account.name, cluster.fscioAccount.name))
        trans=nodes[0].createInitializeAccount(account, cluster.fscioAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
        transferAmount="70000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.fscioAccount.name, account.name))
        nodes[0].transferFunds(cluster.fscioAccount, account, transferAmount, "test transfer")
    ...
    Print("Create new account %s via %s" % (contractAccount.name, cluster.fscioAccount.name))
    trans=nodes[0].createInitializeAccount(contractAccount, cluster.fscioAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
    transferAmount="90000000.0000 {0}".format(CORE_SYMBOL)
    Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.fscioAccount.name, contractAccount.name))
    nodes[0].transferFunds(cluster.fscioAccount, contractAccount, transferAmount, "test transfer")
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    timeOutCount+=1
                    if timeOutCount>=3:
                       Print("Failed to push create action to fscio contract for %d consecutive times, looks like nodfsc already exited." % (timeOutCount))
                       keepProcessing=False
                       break
                    Print("Failed to push create action to fscio contract. sleep for 60 seconds")
                    time.sleep(60)
                else:
                    timeOutCount=0
                time.sleep(1)
