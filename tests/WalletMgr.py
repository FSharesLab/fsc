...
class WalletMgr(object):
    __walletLogOutFile="test_kfscd_out.log"
    __walletLogErrFile="test_kfscd_err.log"
    __walletDataDir="test_wallet_0"
    __MaxPort=9999

    # pylint: disable=too-many-arguments
    # walletd [True|False] True=Launch wallet(kfscd) process; False=Manage launch process externally.
    def __init__(self, walletd, nodfscPort=8888, nodfscHost="localhost", port=9899, host="localhost"):
        self.walletd=walletd
        self.nodfscPort=nodfscPort
        self.nodfscHost=nodfscHost
        self.port=port
        self.host=host
        self.wallets={}
    ...
    def getArgs(self):
        return " --url http://%s:%d%s %s" % (self.nodfscHost, self.nodfscPort, self.getWalletEndpointArgs(), Utils.MiscFscClientArgs)
    ...
    def launch(self):
        if not self.walletd:
            Utils.Print("ERROR: Wallet Manager wasn't configured to launch kfscd")
            return False
    ...
    def killall(self, allInstances=False):
        """Kill kfsc instances. allInstances will kill all kfsc instances running on the system."""
        if self.__walletPid:
