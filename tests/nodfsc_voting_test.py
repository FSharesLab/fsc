...
try:
    ...
    for account in accounts:
        ...
        trans=node.createInitializeAccount(account, cluster.fscioAccount, stakedDeposit=0, waitForTransBlock=False, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.fscioAccount.name, account.name))
        node.transferFunds(cluster.fscioAccount, account, transferAmount, "test transfer")
