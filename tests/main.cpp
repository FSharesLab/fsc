...
//extern uint32_t FSC_TESTING_GENESIS_TIMESTAMP;
...
boost::unit_test::test_suite* init_unit_test_suite(int argc, char* argv[]) {
   ...
   /*
   const char* genesis_timestamp_str = getenv("FSC_TESTING_GENESIS_TIMESTAMP");
   if( genesis_timestamp_str != nullptr )
   {
      FSC_TESTING_GENESIS_TIMESTAMP = std::stoul( genesis_timestamp_str );
   }
   std::cout << "FSC_TESTING_GENESIS_TIMESTAMP is " << FSC_TESTING_GENESIS_TIMESTAMP << std::endl;
   */
   return nullptr;
}
